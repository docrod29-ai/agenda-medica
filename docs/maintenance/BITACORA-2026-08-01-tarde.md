...
- **v1090** — REG-208. El cruce alergia↔fármaco del copiloto era el CONSUMIDOR QUE FALTABA del campo de alergias: no partía el campo a mano —por eso el guardián de REG-144, que buscaba un quinto `split`, no lo veía— sino que lo leía ENTERO y buscaba el fármaco dentro con un `includes`. La negación va pegada a UN alérgeno y el `includes` los mira todos a la vez: «Niega alergia a penicilina» + amoxicilina → CRÍTICA. Reproducido con el motor real antes de tocar nada: 4 de 9 frases de consultorio daban crítica falsa, y `alergenosDe` acertaba en las 9. El campo «Niega penicilina. Alérgico a sulfas» disparaba LAS DOS familias. Duele porque ese aviso NO SE PLIEGA (y hace bien, es lo más grave de la pantalla): la única salida del médico era borrar el texto del expediente para poder trabajar — el desenlace exacto que la cabecera de `alergias.ts` describe como el fallo a evitar. Ahora se lee alérgeno por alérgeno por `alergenosDe`, y de paso entra `alergiasEstructuradas` (hoy no la llena nadie, pero una importación la activa el mismo día y el paciente MEJOR documentado era el único sin cruce). NO toca FAMILIAS_ALERGIA ni el nivel del aviso ni qué bloquea la firma. VERIFICADO que puede ponerse rojo: revertido, 8 de 14 casos fallan.
