 ...
 * viene, esa mención no es contradicción.
 *
 * ── PERO UNA MENCIÓN BIEN ESCRITA NO ABSUELVE A LAS DEMÁS (REG-192) ──────────
 *
 * Hasta el 7-ago-2026 esto miraba **sólo la primera aparición**. Una nota que
 * escribía «niega diabetes» en el interrogatorio y luego «diabetes mellitus tipo
 * 2 descontrolada, se inicia metformina» en la impresión diagnóstica pasaba en
 * silencio: la primera mención, correcta, silenciaba a la segunda. Ahora cada
 * aparición se juzga sola — el recorrido vive en `buscar-en-la-nota.ts`.
 */
export function contradicciones(negadas: readonly Negada[], textoNota: string): Contradiccion[] {
  ...
  for (const n of negadas) {
    ...
    const enLaNota = primeraMencionSinEncuadre(textoNota, formas, NIEGA_EN_LINEA)
    if (enLaNota) out.push({ ...n, enLaNota })
  }
  return out
}
