...
- **v1151** — REG-270 (auditoría del motor de temporalidad, backlog EVAL-002: se construyó en v1027-v1030 y nunca tuvo corpus). `padecimientosEn` busca cada forma como SUBCADENA y dos eran cabezas sin calificador: «derrame» y «trombosis». Medido con el motor real: «tuvo derrame pleural derecho hace tres años» → **evento vascular cerebral**; «trombosis arterial» → trombosis venosa. El aviso no sobra, FABRICA un hecho clínico de otro órgano y otra gravedad, y lo pone delante del médico en el segundo de firmar — y un derrame pleural es de lo más frecuente que ve un infectólogo. El módulo declaraba TRES veces que «sólo señala de menos, nunca de más»: los 30 casos del golden comprobaban que cazara lo que debía, ninguno que NO cazara lo que no. Estrechado (única dirección segura) + guardián `CABEZAS_QUE_NO_NOMBRAN_SOLAS` para que la siguiente forma suelta no lo reintroduzca en silencio. NINGUNA cifra ni criterio clínico nuevo. Queda vivo y DECLARADO: las cabezas sueltas de `CRONICAS` («infarto» → cardiopatía, «tiroides» → hipotiroidismo) hacen lo mismo, pero viven en `negaciones.ts` donde el vocabulario decide además qué cuenta como negación — backlog SAFE-003.
