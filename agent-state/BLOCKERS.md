...
| B-12 | El disparo programado de V7 exige leer `docs/ai/NEXUSMED_AUTONOMOUS_MEDICAL_INTELLIGENCE_MASTER_LOOP_V7.md` como «la especificación autoritativa» — **ese archivo no existe** en el repositorio ni aparece en ningún commit de su historia (comprobado el 9-ago con `git log --all --diff-filter=A`). No hay ninguna otra referencia a él en el código ni en `docs/`. Que el Dr. confirme si nunca se escribió, si vivía fuera del repo, o si el nombre cambió | Que el dueño diga dónde vive la directiva V7 (o confirme que nunca existió como documento aparte) | El programa sigue funcionando: `agent-state/V7-ITERACION.md` + `agent-state/BACKLOG.json` + `docs/audit/regression-ledger.md` ya hacían de especificación operativa de facto — es lo que las 27 versiones anteriores (v1120→v1163) usaron para decidir qué tocar. Este disparo continuó igual, sin inventar una directiva nueva |
