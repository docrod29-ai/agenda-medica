...
export const REQUISITOS: readonly Requisito[] = Object.freeze([
  ...
  R({
    ...
    estado: 'PARTIAL',
    queFalta: 'REG-574 cerró la mitad que faltaba: los otros DOS programas en vuelo ya no son invisibles. El tablero de Ausculta custodia 78 requisitos y está bien vigilado, pero custodiaba UN programa de TRES — V9 (experiencia del paciente) y V10 (excelencia visual) viven en agent-state/ y el tablero no los mencionaba, así que «quedan N accionables» era cierto del censo y falso del producto. Ahora se cuentan en docs/product/PROGRAMAS-EN-VUELO.md, con guardián. NO se fusionan a propósito: V10 es el carril de Product Excellence y el §20 del directivo prohíbe rehacer su trabajo. Al reconciliar V9 se midieron sus 10 abiertos UNO POR UNO contra el árbol y CINCO ya estaban hechos sin marcar. REG-589 cerró lo que faltaba de las CIFRAS: la sección titulada «Compuertas medidas en este SHA — no citadas de memoria» citaba de memoria un trinquete de 96 con el techo en 95, y 10 844 casos con el árbol en 12 019. Cuarta vez este mes —con REG-572, REG-576 y REG-586— de que la garantía mejor explicada es la que nadie fue a comprobar. Ahora los TECHOS se derivan de los cuatro archivos que ya los tenían (lint-techo, techos-de-diseno, MASTER_STATE, invariantes-clinicos) y el guardián compara byte a byte. El RESULTADO de correr la suite NO se deriva a propósito: exige correrla, y una corrida de tres minutos dentro de un generador de documentación es algo que nadie ejecuta — así que sigue siendo una FOTO, se llama así, lleva fecha y dice que si no cuadra con lo de hoy gana lo de hoy. FALTA: la prosa de cada WS sigue a mano, y debe — es criterio, no un grep; lo que ya no puede envejecer son las cifras. Y no se comprueba que la foto sea reciente, que exigiría correr la suite desde el guardián.',
    artefactos: [
  ...
  R({
    ...
    estado: 'PROVEN',
    evidencia: 'REG-407 lo cerró en la consulta: selector de los CUATRO tipos en cada fila, con etiqueta accesible, bloqueado en nota firmada, y elegirlo marca tipoOrigen: medico — la única vía por la que un diagnóstico pasa a estar firmado por una persona. REG-569 corrigió el censo: esta entrada pedía «la misma elección en las otras superficies» y contra el árbol no se sostenía —hospital y UCI no tienen Diagnostico[] sino una cadena libre, y el expediente enseña notas ya FIRMADAS, donde un selector sería una segunda puerta de escritura—. Construirlo habría sido construir el defecto con el censo dando la orden. De paso apareció lo que sí estaba roto: la receta y la orden elegían el principal con `find(definitivo) ?? dxs[0]`, y ese respaldo no mira tipo, así que un «embarazo descartado» salía impreso como el motivo de la receta; ahora hay una sola puerta (diagnosticoQueSeImprime) que usa estaVigente y no rellena nada si nada califica. REG-592 cerró las DOS preguntas que quedaban, las dos del médico dueño: (D-038) firmar con diagnósticos tipados por el dictado AVISA y NO BLOQUEA —se descartaron bloquear la firma y bloquear sólo el que se imprime—, con guardián nuevo que cae si alguien lo convierte en bloqueo o si quita el aviso; y (D-039) `diagnosticoIngreso` de hospital/UCI SIGUE siendo cadena libre, DIFERIDO mientras Hospital sea ALPHA y no esté a la venta.',
  ...
  R({
    ...
    estado: 'PROVEN',
    evidencia: 'CORRECCIÓN DEL CENSO (REG-403): tres de los cuatro campos que esta entrada pedía YA EXISTÍAN, en el sitio correcto y con otro nombre — `revisado` es `estado: cerrada`, `revisadoPor` es `cerradaPor` y `revisadoEn` es `cerradaEn`. Ponerlos en el panel es lo que la arquitectura prohíbe y `laboratorio/firestore.ts` lo tiene escrito bajo el título «DÓNDE VIVE REVISADO»: crearía una segunda fuente de verdad del mismo hecho. Construirlos habría sido construir el defecto con el censo dando la orden. El cuarto sí faltaba y se cerró: nada registraba que un valor crítico se hubiera comunicado, así que «lo vi» y «localicé a alguien» eran el mismo gesto. REG-593 cerró las DOS preguntas que quedaban, las dos del médico dueño: (D-040) un crítico NO vence —se ofrecieron 1 h, 4 h, 24 h y ninguno—, así que la pregunta al cerrar es la única defensa y hay guardián que cae si alguien la quita; y (D-041) cuentan las cuatro vías, incluido un mensaje enviado, advertido de que puede morir sin acuse (REG-580/438). Se guarda CUÁL fue, en un campo que pregunta una sola cosa —de qué manera consta— para no repetir REG-566, y el progreso del resultado lo distingue: «(hablado)» / «(por mensaje)». El trinquete de conexión cazó que el lector quedaba sin llamador y se conectó ahí.',
  ...
  R({
    ...
    estado: 'PARTIAL',
    queFalta: 'REG-399 creó el contrato: 17 capacidades, cada una con qué decide, QUÉ CUESTA QUE SE EQUIVOQUE, su conjunto (o qué haría falta para que existiera), su métrica y su política de fallo. Un guardián compara el censo contra los `feature` del árbol y el censo se aplica también en ejecución. REG-594: el dueño fijó el PRIMERO el 31-ago-2026 (D-042), el de `nota-consulta`, y lo fijó con DOS ejes porque sus errores no cuestan lo mismo — pérdida ≤ 1 %, alucinación 0 %: un medicamento perdido se nota al leer la nota, uno añadido sale impreso con cédula y nadie lo busca. El tipo `UmbralDecidido` sólo llevaba un número; ahora lleva ejes, con `valor` en el más LAXO a propósito para que quien lea sólo ese campo no se lleve mejor impresión que la real. El guardián se amplió sin bajarlo: un umbral decidido sale de una regla escrita O de una decisión FECHADA del médico, y de ninguna tercera cosa. Falta lo que NO se puede hacer sin el dueño y no se inventó: 14 de los 17 UMBRALES los tiene que fijar un médico —cuánta pérdida de medicamentos es tolerable al extraer una nota es una cifra clínica, y la regla 1 prohíbe inventarlas—. Falta también construir los conjuntos: la mayoría no existe, y el de voz no puede nacer de audio real porque la voz es biométrica. Y la política de fallo se DECLARA; sólo una está comprobada en el código. REG-595 cerró la otra mitad de D-042: el umbral estaba escrito y NO REPROBABA NADA — el arnés medía por un lado, el número vivía por otro y entre los dos no había una función. `aplicarUmbral()` toma el umbral DEL CONTRATO (no una copia; probado con un 50 % armado en la prueba) y da veredicto sobre el corpus oro; vive DENTRO de `ia/evaluacion.ts` porque un arnés que mide y no compara contra el umbral está a medias, y porque los dos trinquetes de conexión rechazaron un módulo aparte —el techo de módulos fuera del camino sólo baja—. Tres huecos que un descuido leería como verde no lo son: umbral pendiente, conjunto VACÍO (sin la guarda, la forma más fácil de estar en verde sería borrar el corpus) y un eje que el arnés no sabe medir; `esVerde()` es el único sitio donde se define verde. Y lo que hay que decir: el 1 % NO se está ejerciendo — con 4 campos esperados el escalón mínimo medible es 25 %, así que hoy la compuerta se comporta como un cero (más estricto, no más laxo) y cada lectura lo declara. Ejercerlo pide ≥ 100 campos esperados, y ese conjunto no existe. REG-596 fijó el SEGUNDO umbral (D-043), el de `transcribir`, con TRES ejes que no se suman: los dos ceros —críticos y sin clasificar— NO los decidió el dueño, salen de `politica-critica.ts` (prohibido, no penalizado); el tercero sí, con la medición delante: error ordinario ≤ 5 %. Y corrigió el censo: el contrato decía «no existe gold de voz… todavía no está» y SÍ EXISTE — `synthetic-data/dialogos-consulta`, 12 diálogos actuados con guion y salida real (novena entrada del censo vieja al ir a construir sobre ella). Medido: 532 palabras, WER crudo 10 % pero normalizado 1,7 %, 5 ordinarios, 0 sin clasificar y 1 CRÍTICO REAL — DLG-004: el motor se comió «Van dos veces este mes» y perdió la cifra y quién la dijo. El dueño eligió trinquete (sellado en 1, sólo baja) en vez de CI rojo; el defecto NO se tapa: queda con nombre en `EL_CRITICO_QUE_SIGUE_ABIERTO` con guardián que cae si se borra, y sigue ABIERTO. La compuerta se mudó a `contratos-de-evaluacion.ts` porque el segundo arnés la necesitaba: dos medidores, un solo tipo `Umbral`, una sola definición de verde. REG-597 fijó el TERCERO (D-044), el de `laboratorio-vision`: valor mal leído 0, unidad mal leída 0, analito perdido ≤ 5 %. Se escribieron 8 hojas sintéticas (`synthetic-data/laboratorio-hojas`) como se imprimen en México y se midió antes de preguntar: 7 de 46 filas no llegan al panel — 15,2 % contra el techo del 5 %, o sea ROJO. La causa NO es la visión: SEIS de las siete son cobertura del catálogo (ácido úrico, neutrófilos, linfocitos, VCM, vitamina D, ferritina; `analitos.ts` cubre 24 y una hoja de rutina trae más), y la séptima es glucosa en mmol/L, que el rango plausible tira entera — el paciente cuyo laboratorio reporte en unidades del SI se queda sin serie de glucosa y nadie se lo dice. Trinquete sellado en 7 con el patrón de D-043: no da por bueno el hueco, la compuerta sigue diciendo `reprueba`. Bajarlo pide añadir analitos con su rango plausible, y ese rango es una cifra que no se inventa: es la siguiente decisión del dueño. Lo que este conjunto NO mide queda declarado: las filas entran perfectas, así que DOS de los tres ejes salen cero por construcción y sólo se ejercen al revés; medir la visión pide imágenes y llamadas de API, la mitad que el dueño dejó para después. Tampoco se le preguntó cuántos analitos INVENTADOS se toleran: se cuenta, se reporta y queda NEEDS_CLINICAL_REVIEW. REG-598 cerró ese rojo POR LA CAUSA, no por el umbral: el dueño entregó su catálogo maestro de plausibilidad (D-045, ~200 analitos, íntegro en `docs/clinical/CATALOGO-PLAUSIBILIDAD-LABORATORIO.md`) y entraron OCHO analitos con SUS números, citados y sin banda de referencia (su §1: el intervalo de referencia lo pone el laboratorio). 15,2 % → 2,2 %, verde. Su §25.2 destapó un defecto que yo iba a cometer: «Neutrófilos 75 %» y «Neutrófilos 7,5 ×10³/µL» son resultados distintos con el MISMO nombre impreso, así que mapear por nombre habría metido el 75 en la serie del absoluto — no un analito perdido sino un VALOR MAL LEÍDO, el eje que él puso en cero. El rango plausible no lo habría cazado (75 cabe en 0–500). Ahora la unidad desambigua y sin unidad NO se adivina. Falta, y es de él: cargar el catálogo entero (~200 analitos), adoptar sus rangos ENSANCHADOS —que sin normalización de unidad haría pasar 7,2 mmol/L como 7,2 mg/dL, con guardián que lo impide— y la arquitectura de sus §25-§36 (LOINC, UCUM, valor original + canónico, doce estados). REG-599 aplicó su §27 y §28: primero se normaliza la unidad, DESPUÉS se juzga el número, y la fila fuera de rango YA NO SE TIRA — se acepta provisionalmente y se marca (su §1), conservando valor y unidad originales (§27.1). Destapó un defecto que nadie había mirado: PCR 84 mg/dL entraba a la serie como 84 mg/L —son 840— y el límite de plausibilidad NO puede cazarlo porque 84 cabe de sobra en 0–600. Una PCR de sepsis dibujada como un resfriado, sin una marca. Sólo hay DOS conversiones y es a propósito: un factor es una equivalencia y la regla 1 las nombra. Vitamina D (§6, literal) y creatinina (§27.1, del ejemplo trabajado). La de la glucosa NO está —18,0182 se sabe de memoria y por eso mismo no se escribe sin fuente— y queda en `CONVERSIONES_QUE_FALTAN`. Y el estado LLEGA a la pantalla: el ámbar se enciende con el estado y no sólo con `noEvaluable` (una ferritina de 2 000 000 en unidad correcta no lo encendía), con el motivo como texto visible y no en un `title` (REG-581). El número cambió de significado, así que ahora son dos: fuera del panel 0 de 46, y SIN GRÁFICA 2 —glucosa y PCR, las dos por falta de factor— con su propio trinquete. REG-600 aplicó su §29: ante un valor imposible se evalúan ×10 ÷10 ×100 ÷100 ×1000 ÷1000 y se SUGIERE revisión, sin tocar el valor y sin botón que lo aplique — el campo ya es editable y la corrección la hace el médico. La mitad del trabajo fue lo que NO se hace: la sugerencia sólo se ofrece con la unidad canónica, porque glucosa 7,2 mmol/L × 10 da 72 —plausible en mg/dL y MAL, son 130—, y una respuesta verosímil a la pregunta equivocada se acepta sin mirar. Con varios candidatos se enseñan todos: en creatinina 120 el «bonito» (1,2) es justo el que puede estar mal si venía en µmol/L (1,36). Sigue sin cubrirse el error de captura que ES plausible (140 tecleado como 145) y el salto imposible respecto del valor previo del paciente. REG-601 cargó el catálogo ENTERO (220 analitos) leído por máquina del documento —un dígito tecleado mal convierte un límite en otro— con guardián del JSON contra el documento y de cada analito contra su fila; el script además comprueba que el documento no se contradiga (repite doce analitos y coinciden). Y al medir apareció un DEFECTO VIVO: «Glucosa urinaria», «LCR glucosa», «Sodio urinario», «LCR leucocitos» y «Creatinina urinaria» caían en las series de SUERO — la última pese a que su patrón excluía «orina», porque no cubría «urinaria». Un sodio urinario de 20 dibujado como sérico se lee como hiponatremia mortal, y no se ve. Ahora la muestra se decide UNA vez sobre el nombre del renglón (§26, §27.3) y un renglón de orina sólo casa con analitos de orina. Cierra media cara de E1-02-H2; la otra (depuración de creatinina) sigue abierta y dicha. Pie de banco cazado por una prueba de UCI: el nombre pelado no puede vivir en `patron`, porque tres sitios lo leen a pelo sobre texto libre sin el filtro. Y la PROSA se sigue leyendo con los 32 de siempre (`ANALITOS_EN_TEXTO`): 219 nombres en un dictado casan solos. Falta: el espécimen como CAMPO desde la lectura de la hoja —hoy se decide por renglón, no por sección—, LOINC/UCUM (§27.2-3); ninguno de los 218 conceptos tiene código. REG-602 cerró `MISSING_UNIT` (§33): un renglón sin unidad se guardaba con `unidadOriginal` rellena con la CANÓNICA — el campo que existe para conservar lo que dijo el laboratorio decía lo que asumimos nosotros, e indistinguible de una hoja que sí la dijo. Ahora se queda vacía, la asumida viaja aparte y el estado lo declara. NO cambió lo demás a propósito: se sigue asumiendo la convencional, la fila sigue graficándose y un pánico sin unidad sigue marcando crítico — dejar de graficarlas vaciaría medio expediente por cautela, y `MISSING_UNIT` no pinta ámbar por lo mismo. Sigue sin distinguirse «la hoja no lo dijo» de «la IA no lo leyó» (pide el §32). REG-603 quitó de en medio la pregunta de los factores: ya no se teclea ninguno. Se calculan por ESCALA (prefijos del SI), por MASA MOLAR (fórmula + pesos atómicos IUPAC) o por VALENCIA (mEq = mmol × |z|), y las dos cifras trabajadas del documento del dueño —creatinina 1,58 del §27.1 y vitamina D 2,496 del §6— se REPRODUCEN sin usarlas: son testigos del método, no su fuente. Un peso atómico mal rompe seis pruebas. Cazó de paso dos cosas: una tabla de escala casera que daba ferritina µg/L → ng/mL = 0,0001 (habría dividido una ferritina entre diez mil, en silencio), y que los electrolitos en mmol/L —lo más común que existe— salían marcados enteros desde REG-599 porque la unidad canónica aquí es mEq/L. 45 de 46 filas del corpus entran ya a la gráfica. NO se derivan los triglicéridos (masa molar CONVENCIONAL, no constante), las unidades de actividad (no son masa) ni calcio/magnesio/fósforo (derivables, pero sin testigo del dueño que lo compruebe). Y los rangos anchos siguen sin adoptarse: con hoja muda, un rango ancho acepta en silencio un valor de otra unidad — el rango estrecho está haciendo el trabajo de `MISSING_UNIT`. El precio concreto es la PCR de 84 mg/dL, que se convierte bien a 840 mg/L y aun así cae fuera del 0–600 NUESTRO, que no tiene fuente citada, frente al 0–1000 del dueño, que sí. Desbloquearlo es dejar de graficar lo mudo, y eso lo decide él. REG-604 cerró el hueco que REG-601 había dejado DECLARADO: la muestra se decidía por el nombre del renglón, y una hoja que pone «Examen general de orina» en la cabecera y luego escribe «Glucosa» a secas seguía cayendo en la serie de suero. No es un caso raro, es el caso normal: el EGO y el LCR llaman a sus renglones igual que una química sanguínea. Medido sobre las dos hojas mudas nuevas, las OCHO filas fallaban: cuatro archivadas en suero (glucosa 250 de orina, creatinina 85 de orina, glucosa 35 de LCR, leucocitos 320 de LCR) y cuatro perdidas (proteína, densidad, pH, proteínas). Las dos primeras son las que duelen y NINGÚN rango las caza porque ambas son plausibles en suero: una glucosuria de 250 archivada como glucemia es una urgencia diabética que nadie tuvo; una glucosa de LCR de 35 —hipoglucorraquia, meningitis bacteriana— es una cifra baja sin importancia. Ahora la lectura de la hoja PIDE la muestra por renglón, con lista blanca cerrada, y la regla que lo hace seguro es que el campo SÓLO RELLENA EL HUECO, NUNCA CONTRADICE: si el nombre nombra una muestra, ésa manda. Es monótona a propósito — un campo capaz de contradecir al nombre convertiría un error de lectura del modelo en una glucosa urinaria archivada como glucemia, o sea reabriría por la puerta de atrás el defecto de REG-601. Y no rompe el foso de «sólo transcribe»: la muestra está IMPRESA, igual que la unidad; el prompt prohíbe con todas las letras DEDUCIRLA del analito, del valor o de las unidades («una glucosa no es de orina por ser alta»). De paso destapó una cautela mal puesta: el patrón pelado descartaba nombres de menos de tres caracteres y con eso el pH urinario se perdía — pero el patrón pelado sólo se consulta DESPUÉS del filtro de espécimen, entre los doce analitos de orina, donde «ph» no es ambiguo. 54 de 54 filas llegan al panel y 53 entran a la gráfica; la única fuera sigue siendo la PCR de 840 mg/L por el rango NUESTRO de 0–600 sin fuente frente al 0–1000 del dueño. NO se comprueba que el modelo lea bien la cabecera (pide imágenes y llamadas de API), no hay campo de confianza (§32: «la hoja no lo dijo» y «el modelo no lo leyó» llegan igual), la muestra es por RENGLÓN y una hoja mixta depende de que cada uno traiga la suya, y LOINC/UCUM sigue abierto: 218 conceptos sin código, y no se asignan de memoria. Quedan 12 umbrales por fijar.',
  ...
  R({
    ...
    estado: 'PARTIAL',
    queFalta: 'REG-391 puso el interruptor y REG-397 el instrumento (el vigilante cuenta pausadas y rendidas y las distingue). REG-580 cerró la pantalla del dead-letter: contar era el primer paso y no el trabajo — con un número no se ve de qué paciente era, ni qué decía, ni por qué murió, ni se puede reintentar. Ahora se listan y se pueden devolver a la cola, con la MISMA puerta que /entregas (`mensajeria.enviar`) y avisando ANTES del botón de que reintentar puede DUPLICAR el mensaje: desde la cola no se distingue si no llegó nunca o si llegó y se perdió el acuse. Los intentos se reinician y las pausas se conservan, porque cuentan cosas distintas. CORRECCIÓN DEL CENSO (REG-586): esta entrada decía que el mensaje REACTIVO del bot «se pierde y no queda en ninguna cola». Que no se encole es una DECISIÓN argumentada —reintentar fuera de la ventana de 24 h exige plantilla aprobada en Meta, trámite del dueño— y sí quedaba constancia: `registrarNoEntregado` escribe desde el helper `send()`, que cubre sus 36 llamadas. El defecto real era peor: `whatsapp_no_entregados` tenía UN ESCRITOR Y CERO LECTORES. Declarada en los tres sitios, respaldada, cerrada al cliente… e invisible, mientras la cabecera del módulo prometía que «un fallo registrado se puede VER». Y REG-580 había puesto una pantalla llamada «No entregados» que, con la cola limpia y el bot fallando, afirmaba que no se había perdido nada. Cerrado: lector con tope que LANZA en vez de devolver [], las dos listas por la misma ruta y la misma capacidad, los del bot SIN botón (no se pueden reintentar, y prometerlo haría que el médico dejara de llamar), y «ninguno» ahora exige que las dos estén vacías. FALTA: (1) encolar y reintentar de verdad, que espera la plantilla aprobada del dueño; (2) una clave de idempotencia de punta a punta con el proveedor, que es la única defensa REAL contra el duplicado (hoy la defensa es que lo decida una persona informada); (3) archivar o descartar lo que el médico decida no reintentar.',
