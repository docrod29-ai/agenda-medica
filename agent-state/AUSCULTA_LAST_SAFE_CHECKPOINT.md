...
## Checkpoint · 5-sep-2026 — **Preservation, Audit & Intelligence Transformation · cuatro P1 cerrados y la receta cerrada en lo verificado, #442 absorbido el test-the-test y la seguridad reportada cerrados (REG-515…531) · tres decisiones del dueño escritas (D-032…034)**
...
LAST_COMPLETED_UNIT=REG-531 — verificación en navegador con el arnés (sonda nueva, pac-006 sembrado): los avisos de REG-520/520/521 se ven a 390 y 1440 sin errores de consola; y destapó que la receta contaba su propia nota como «ya lo toma» — arreglado en receta y consulta. Antes: REG-530 — el error crudo ya no sale al cliente: helper sin acceso al error, 40 rutas migradas, barrido de todas las rutas como guardián. Antes: REG-528 y 528 — sanitize redacta lo que su cabecera prometía (nombres por llave, Stripe por patrón); reclamarCanal en transacción con la carrera provocada. D-E escrita (360dialog: la llave como id de documento exige migración). Antes: REG-525, 525 y 526 — test-the-test: csp-manifest corre tras el build; el prompt se vigila por frases sobre lo emitido; la membresía del servidor tiene por fin una prueba ejecutada contra un doble con id. Antes: REG-513 y REG-514 — port de PR #442 con número nuevo (vista previa medida, captura completa de la sonda); sus pruebas pasan aquí. Antes: REG-524 — terapia duplicada: la misma sustancia en dos renglones (catálogo de dosis: Tempra = paracetamol) o ya vigente, con la suma diaria contra el techo que ya existía; en consulta y receta. Antes: REG-523 (la receta ve el expediente completo), REG-522 (bloqueo ARCO revoca el portal, D-034); D-032 y D-033 escritas en el código que gobiernan
...
LINT=93 (techo apretado en REG-520)
P0_OPEN=(ninguno; cross-tenant refutado en las 99 rutas)
P1_OPEN=(ninguno confirmado). Receta: los cinco hallazgos de medication-safety verificados; cuatro cerrados (REG-520, 518, 520, 521) y el quinto (validadores sin llamador) es decisión del dueño (D-D)
...
DO_NOT_REGRESS=REG-515 (revocación en telesalud/sala) · REG-516 (una lista de sesgo para los cuatro puntos de envío) · REG-517 (la pregunta escalada abre tarea sin depender del teléfono) · REG-518 (guardián con autotest) · REG-519 (cerrar la tarea marca la pregunta atendida) · REG-520 (sin edad no se supone adulto) · REG-521 (la huella de la receta larga no se pierde) · REG-522 (el bloqueo ARCO apaga el portal) · REG-523 (la receta ve la medicación vigente y la creatinina del expediente) · REG-524 (la misma sustancia dos veces se dice, con la suma contra el techo del catálogo) · REG-513/523 (port de #442: la vista previa mide su sitio; la sonda recorre el scroller de dentro) · REG-525 (csp-manifest corre tras el build) · REG-526 (el prompt se vigila por frases, sobre lo emitido) · REG-527 (la membresía del servidor se ejecuta contra un doble con id) · REG-528 (sanitize redacta nombres y Stripe) · REG-529 (reclamarCanal en transacción) · REG-530 (el error crudo no sale al cliente) · REG-531 (la receta no se cruza consigo misma)
```
...
3. **REG-515** — la sala de video aceptaba un enlace revocado.
4. **REG-516** — los alérgenos no llegaban a Whisper; ahora una lista para los
   cuatro puntos de envío.
5. **REG-517** — la pregunta escalada del paciente abre una tarea en
   `/pendientes` aunque no haya teléfono.
6. **REG-518** — el guardián del paciente equivocado se probaba contra un
   comentario; ahora contra sus mutantes.
7. **REG-519** — cerrar la tarea de una pregunta marca la pregunta atendida:
   el portal del paciente deja de decir «pendiente» para siempre.
8. **REG-520** — sin edad en el expediente, la receta ya no aplica topes de
   adulto a un niño en silencio: lo dice, y usa la fecha de nacimiento.
9. **REG-521** — la bitácora ya no pierde entera la huella de una receta
   larga: se acota por campo y lo omitido se declara.
10. **D-032, D-033, D-034** — el dueño decidió: la alergia crítica sólo avisa,
    la pregunta del paciente viaja completa por WhatsApp, y la cancelación
    ARCO apaga el portal. La tercera es código: **REG-522**.
11. **REG-523** — la receta ve el expediente completo, como la consulta: la
...
12. **REG-524** — «Paracetamol 500 mg» + «Tempra 1 g» pasaban renglón a
...
13. **REG-513 y REG-514** — port de PR #442, cuyos números ya había gastado
    ...
    el dueño lo cierre.
14. **REG-525, 525, 526** — test-the-test: una prueba que el CI dejaba
...
15. **REG-528 y REG-529** — seguridad: `sanitize` cumple lo que su cabecera
...
16. **REG-530** — cuarenta rutas devolvían `String(err)` al cliente; ahora un
    helper que no recibe el error, y un barrido de todas las rutas.
17. **REG-531** — la primera verificación en navegador de esta rama (arnés
