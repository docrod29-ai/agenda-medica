...
- **v1150** — REG-268. AUDITORÍA de `funcion-renal.ts` (el backlog no tenía nada reproducible: SAFE-001 y VOICE-004 ya estaban cerrados en el árbol y los EVAL-xxx siguen bloqueados por el corpus del dueño). `cockcroftGault` devolvía `Math.round(crcl)` y ESE ENTERO era el que se comparaba contra los 18 umbrales de ajuste renal: todo CrCl en [umbral−0.5, umbral) subía al umbral exacto y la alerta desaparecía, en los cuatro umbrales a la vez (30/40/50/60). Reproducido con el motor real: hombre 80a, 64 kg, creatinina 1.8 → CrCl 29.63 y **cero alertas** con metformina y nitrofurantoína en la receta — las dos son de EVITAR por debajo de 30. La pista estaba escrita en el propio archivo: `ckdEpi2021` lleva desde L6 el comentario de que NO redondea «porque cambia clasificaciones y comparaciones», y Cockcroft —el que de verdad alimenta las dosis— se había quedado fuera de esa decisión. Ahora se COMPARA con el valor completo y se ESCRIBE el redondeado: ni un mensaje ni un umbral cambió, y en pantalla se ve el mismo entero. NINGUNA cifra clínica nueva. VERIFICADO que puede ponerse rojo: restaurado el `Math.round`, 4 de 25 casos fallan.
## SIN DESPLEGAR — v1150, en rama `agent/renal/REN-001`

| v | Rama | Qué se reparó |
|---|---|---|
| **1150** | `agent/renal/REN-001` | **REG-268 — el redondeo del motor renal se comía las alertas del borde.** `cockcroftGault` devolvía `Math.round(crcl)` y ese entero era el que se comparaba contra los 18 umbrales de ajuste renal: todo CrCl en [umbral−0.5, umbral) subía al umbral exacto y la alerta desaparecía, en los cuatro umbrales a la vez. Hombre 80 a, 64 kg, creatinina 1.8 → CrCl 29.63 y cero alertas con metformina y nitrofurantoína, que son de EVITAR por debajo de 30. El redondeo pasa a la capa de presentación: se compara con el valor completo y se escribe el redondeado, que es la regla que CKD-EPI ya seguía. Ningún umbral ni mensaje cambió. |
