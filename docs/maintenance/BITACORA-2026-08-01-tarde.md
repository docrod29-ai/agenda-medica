...
- **v1151** — REG-270 (hallazgo C2/C3, el último del plan de la auditoría de 9 dimensiones). REG-192 ya había pasado por `parser-clinico.ts` —el motor que ESCRIBE LA NOTA cuando la IA devuelve 529— y añadió `padece`/`padezco` SÓLO a los negadores, no a los afirmadores. Cubrió tres formas y abrió la puerta contraria: «Niega tabaquismo, padece diabetes mellitus» pasó a BORRAR la diabetes, porque el `niega` del principio sigue alcanzando al término de la segunda cláusula. Un negador sin su afirmador gemelo no arregla la mitad: la mueve al lado que no se ve, y nadie echa de menos un antecedente que no está. Medido sobre main CON REG-192 dentro, seguían cayendo del lado afirmativo ocho formas: «no he tenido», «nunca he tenido», «jamás ha tenido», «tampoco tiene», «no sufre de», «no cuenta con», «no fuma» y «no es fumador» — las dos últimas porque ahí el término clínico ES el verbo y entre el negador y el término no queda nada que reconocer. `tabaquismoActivo` no se queda en la prosa: entra en STOP-BANG y en Caprini. Reparado con la regla que faltaba: todo verbo que entra en NEGADORES entra también en AFIRMADORES. NO se toca `negaciones.ts` — C-6 separó ese vocabulario a propósito el 7-ago. No es un comodín: «no ha mejorado su diabetes» la sigue afirmando. NINGUNA cifra clínica: es gramática del español. VERIFICADO que puede ponerse rojo: con el parser de main fallan 12 de 26. DECLARADO sin reparar: «no es diabético» sigue sin detectarse como TÉRMINO (`diabético` no está en COMORBILIDADES_DIC) — emparenta con C-7.
