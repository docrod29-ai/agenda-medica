...
## REG-308 — la hoja del paciente no llegaba a ninguna pantalla, y conectarla como estaba planeado habría violado la regla de dos actos (V7 · POSTVISIT-ENTREGA-001)
...
compuerta tendrá que vivir en el servidor.»* Éste es ese REG — y a mitad de
camino cambió de forma.

**El hecho, primera mitad**: `comoSeLoExplico`/`HojaParaElPaciente` existen
desde REG-242 y componen bien —cada línea sale de un campo que el médico ya
revisó y firmó—, y desde REG-307 sólo se entregan con la nota firmada. Pero el
único importador en producción era `consulta/[patientId]/page.tsx`: la
pantalla del MÉDICO. Se construyó una acción `instrucciones` en `/api/portal`
—mismo gate de alcance clínico que `documentos`, sólo notas `estado ===
'firmada'`, excluye internamiento— y se conectó `/mi/[token]` a ella.

**El hecho, segunda mitad — el conflicto que paró la entrega**: al fusionar
con `main` apareció `src/lib/paciente/paquete-de-visita.ts`
(`PATIENT-COMPANION-001`, REG-280/281/304), que construye el
`PatientVisitPackage` que pide V9 con precisión: **DRAFT hasta que alguien
libera, con `approvedAt` y `approvedBy`, y la pantalla `/mi/[token]` ya tiene
su pestaña «Cuidado»** enseñando —con toda intención— un estado vacío honesto:
*«Cuando tu médico libere el resumen de una consulta, lo verás aquí»*. Esa
pantalla cita `patient-facing-ai.md` §4 en su propio comentario:

> Que el médico haya firmado la nota no libera el paquete: son dos actos.

Mi acción `instrucciones` hacía exactamente lo que esa regla prohíbe: convertía
la firma —un acto medicolegal hacia el expediente— en el único requisito para
mostrarle contenido al paciente, sin ningún acto de liberación separado y
registrado. No es una preferencia de estilo: es la regla 4 completa de
`.claude/rules/patient-facing-ai.md`, dada a esta sesión como instrucción del
proyecto, y `PATIENT-COMPANION-001` ya la resolvía correctamente con un estado
vacío en vez de una entrega prematura.

**Decisión de esta sesión**: se **retiró** el cableado de `/mi/[token]` a
`<HojaParaElPaciente>` antes de fusionar — la pestaña «Cuidado» de V9 sigue
siendo la única fuente de verdad de lo que el paciente ve, y sigue diciendo la
verdad (vacío, porque nada libera paquetes todavía; falta `POSTVISIT-001`, la
pantalla del médico). Se **conserva** la acción `instrucciones` en
`/api/portal`: es de solo lectura, gated igual que `documentos` (que ya expone
medicamentos de notas firmadas para las recetas descargables — mismo patrón,
ya validado en producción), y no tiene ningún llamador todavía. Es el mismo
patrón que `validarCorreccion` en `OWNER_DECISIONS_REQUIRED.md`: motor escrito,
probado y sellado, bloqueado por una decisión que no le toca a este programa.

**La pregunta que queda abierta para el dueño** —añadida a
`OWNER_DECISIONS_REQUIRED.md`—: si `documentos` (recetas) ya expone contenido
de una nota firmada sin acto de liberación separado, ¿ese mismo estándar basta
para las instrucciones del paciente, o el companion exige el nivel más estricto
de `PatientVisitPackage` en todos los casos? Mientras no se responda, la
acción `instrucciones` queda construida y sin llamador — deliberadamente, no
por olvido.

### Qué NO cubre

- **No conecta nada a `/mi/[token]`.** Es la mitad que se retiró; ver arriba.
- **No implementa `POSTVISIT-001`** (la pantalla del médico que liberaría un
  `PaqueteDeVisita`), que es el llamador correcto tanto de `componerPaquete`
  como, potencialmente, de esta acción.
- **No arregla `proximaCita`** en la pantalla de consulta (hallazgo previo,
  sin REG propio; fuera de alcance).
- **No se ha abierto un navegador.** Sigue pendiente `NAV-NAVEGADOR-001`,
  ...
  `/dr/[clinicId]` — una ruta sin relación, por `auth/invalid-api-key`).

### Archivos

- `src/app/api/portal/route.ts` — acción `instrucciones` (sin llamador en
  producción, a propósito)
- `src/__tests__/hoja-del-paciente-llega-al-portal.test.ts` (nuevo, 7 casos) —
  prueba la acción del servidor directamente; ninguno de los 7 casos depende
  de que exista una pantalla que la llame
- `agent-state/OWNER_DECISIONS_REQUIRED.md` — la pregunta pendiente

### Compuertas

`npx vitest run` en verde (suite completa, incluida la fusión con las
REG-291…305 de `main`) · 1 fallo **preexistente y de entorno**
(`ops-timeout-y-punto-ciego` — IP no enrutable, falla distinto tras el proxy
de este contenedor) · `lint-trinquete` en el techo, sin deuda nueva ·
`npx tsc --noEmit` limpio. Probado al revés: quitar el filtro
`!n.internamientoId` de la acción hace que «una nota de INTERNAMIENTO nunca
sale, aunque sea la más reciente» falle, devolviendo la nota de UCI en vez de
la ambulatoria — confirmado antes de restaurar el filtro.

### Rollback

`git revert` del commit. No hay migración ni dato escrito que deshacer: la
acción nueva sólo LEE notas ya firmadas, no tiene llamador, y `/mi/[token]`
no se tocó respecto de lo que trajo `main`.
