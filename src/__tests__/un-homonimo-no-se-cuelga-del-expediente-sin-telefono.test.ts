/**
 * RT-001 (Panel de Lujo 2026-09, equipo rojo · ataques propios; reproducción
 * REP-080) — un homónimo que llama CON su
 ...
 *
 * ── LA REPARACIÓN ───────────────────────────────────────────────────────────
 * `elegirExpedienteParaCita` hace ahora dos cosas que antes no hacía:
 *  (1) cuenta los homónimos —nombre normalizado idéntico al de la reserva—
 *      ANTES de descartar a nadie por contradicción de teléfono; con dos, crea;
 *  (2) exige `PUNTAJE_MINIMO_PARA_FUNDIR` (65) al mejor candidato, o sea una
 *      SEGUNDA señal: misma edad (65), mismo teléfono (75) o misma fecha de
 *      nacimiento (88/95). El nombre a secas puntúa 60 y ya no alcanza.
 * `compararPacientes` NO se tocó: sigue avisando con 60 —para preguntar, ese
 * 60 vale—; lo que cambió es quién puede decidir a solas.
 *
 * NOTA: el caso «un solo candidato sin teléfono» CONTRADICE a propósito la
 * prueba «si al existente le falta el teléfono, no hay contradicción: se
 * funde» de `pacientes-duplicados.test.ts`. Esa prueba NO se borró: se
 * reescribió con una segunda señal (misma fecha de nacimiento), que es la
 * lectura que sigue siendo cierta —el paciente que vuelve y ahora sí da su
 * número—. Si el dueño decide seguir fundiendo por nombre a secas, la
 * alternativa que da RT-001 es marcar la cita «expediente elegido por nombre»
 * y enseñarlo antes de dictar — eso ya no se prueba aquí.
 *
 * ── QUÉ NO CUBRE ─────────────────────────────────────────────────────────────
...
describe('un homónimo con teléfono no se cuelga del expediente sin teléfono', () => {
  ...
  it('caso A: un solo expediente homónimo SIN teléfono no funde con la reserva que trae celular', () => {
  ...
  it('caso C: dos homónimos —uno sin teléfono, otro con teléfono contradictorio— no funde con el que quedó', () => {
    const existentes = [
