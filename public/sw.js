 ...
 */
const CACHE = 'nexusmed-v495'  // LABORATORIOS CON IA (la funcion estrella que pidio el Dr tras ver a Huli). En el expediente del paciente se adjunta un PDF o una FOTO del reporte de laboratorio; Claude vision lo TRANSCRIBE (mismo foso que el antibiograma: solo lee, no interpreta ni inventa), un motor determinista valida y agrupa por analito canonico (glucosa/creatinina/Hb/lipidos/electrolitos...), descarta valores en otra unidad, marca criticidad con el motor ya auditado, y el medico REVISA antes de guardar. Con >=2 estudios se dibujan GRAFICAS DE TENDENCIA por analito (SVG puro, banda de referencia, puntos criticos en rojo). PRIVACIDAD: el prompt prohibe devolver nombre/CURP/folio del paciente y la validacion los descarta; el registro cuelga del patientId. Reglas: laboratorios del paciente = secreto medico como las notas (solo medico/admin). En funciones NexusMED ya iguala o supera a Huli (dictado por voz, motor clinico); esto cierra el unico hueco real.
