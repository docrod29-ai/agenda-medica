...
- **v1074** — REG-192 (hallazgos C2/C3 de la auditoría). El motor de NEGACIONES leía la frase como un saco de palabras. Reproducido con el motor —no leyendo el código— sobre diecisiete respuestas negativas del habla real de consulta: **once no se veían**. «¿Padece diabetes? **Pues** no» no contaba porque `NEGATIVAS` exigía que el «no» fuera la primera palabra. Y al buscar el resto salieron CUATRO defectos más del mismo descuido —el motor no miraba DÓNDE estaban las cosas—, dos de ellos del signo contrario y peores de revisar: «No sé» contaba como negación (regla 4 al revés: ausencia de dato como dato de ausencia) y un negador negaba TODO lo que hubiera en la frase, así que «Refiere diabetes de 10 años; niega asma» marcaba **la diabetes** como negada y `corregirCertezaPorNegacion` la reclasificaba sola a *descartado* — el médico puede corregir lo que ve, no lo que el sistema le quitó de la pantalla. Además `indexOf` encontraba «asma» dentro de «pl·asma» (aviso falso en un origen que NO SE PLIEGA) y la contradicción sólo se buscaba en la primera mención, así que una nota que niega arriba y afirma abajo no levantaba nada. Fix: el negador va DELANTE del término y EN SU CLÁUSULA (la coma no corta: es el separador de las enumeraciones negadas), el término se busca como PALABRA con plural, «no sé» se mira antes que «no», y se revisan todas las apariciones. NINGUNA cifra clínica nueva: todo es vocabulario y posición; `CRONICAS` no gana ni pierde una enfermedad. Lo ambiguo («creo que no», «casi nunca») se deja SIN detectar a propósito: señalar de menos. VERIFICADO que puede ponerse rojo: revertido el módulo, fallan 23 de los 36 casos, con al menos uno de cada uno de los cinco defectos.
