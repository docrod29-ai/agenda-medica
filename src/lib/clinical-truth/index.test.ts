...
describe('Clinical Truth core', () => {
  ...
  it('rejects unsupported renderer fact IDs (NO INVENTAR boundary)', () => {
    ...
    expect(() => assertSourceFacts(truth, ['fact-1', 'invented-fact'])).toThrow(/Unsupported source facts: invented-fact/)
  })

  it('keeps signed content immutable and creates an auditable amendment version', () => {
    const truth = encounter([fact()])
    const draft = createDraftDocument({
      id: 'doc-1', encounter: truth, content: 'Original', sourceFactIds: ['fact-1'], createdAt: timestamp, createdBy: 'clinician-1',
    })
    const signed = signDocument(draft, '2026-08-17T15:01:00.000Z', 'clinician-1')
    const amended = amendSignedDocument({
      document: signed,
      encounter: truth,
      content: 'Corrected',
      sourceFactIds: ['fact-1'],
      reason: 'Clinician correction',
    ...
    expect(amended.versions[1]).toMatchObject({
      version: 2,
      status: 'amended',
      content: 'Corrected',
      supersedesVersion: 1,
      amendmentReason: 'Clinician correction',
    })
  })
  ...
  it('keeps clinician preference learning separate from clinical truth', () => {
    ...
    const preference = recordClinicianPreference({
      clinicianId: 'clinician-1',
      preferenceKey: 'document.style.assessment',
