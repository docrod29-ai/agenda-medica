/**
 * GOLDEN — REG-597. El umbral de `laboratorio-vision` (D-044) se aplica.
 *
 * Al escribirse salía ROJO (7 de 46, 15,2 %). REG-598 cerró la causa con los
 * números del médico (D-045) y hoy mide 1 de 46. La historia se conserva abajo
 ...
 *
 *  · 1-sep-2026 (REG-597): **7** de 46. Seis eran cobertura del catálogo.
 *  · 2-sep-2026 (REG-598): **1** de 46. El médico entregó el catálogo maestro de
 *    plausibilidad (D-045) y los seis analitos entraron con SUS números.
 *
...
/**
 * EL SEGUNDO NÚMERO, QUE NO SE PUEDE CALLAR — REG-599.
 *
 * Desde que la fila fuera de rango ya no se tira (§1 de D-045), «llega al panel»
 * y «entra a la serie temporal» dejaron de ser lo mismo. El eje del médico
 * (D-044) mide lo primero y hoy da CERO. Pero dos filas llegan marcadas y sin
 * gráfica, y si sólo se midiera el eje, esas dos desaparecerían del informe justo
 * cuando dejaron de desaparecer del panel.
 *
 * Eran DOS —glucosa en mmol/L y PCR en mg/dL, las dos sin factor—. REG-603 dejó
...
function medirElFoso(hojas: readonly Hoja[]): LoMedido & { filas: number; llegan: number; perdidas: number; inventadas: number } {
  ...
  for (const h of hojas) {
     ...
     *     REG-598 hizo que la unidad desambiguara el diferencial leucocitario,
     *     «Neutrófilos %» volvió a contarse como inventado.
     *  3. La tercera, sin la MUESTRA — y en cuanto REG-604 la añadió, las ocho
...
describe('EL CONJUNTO EXISTE — antes decía «no existe»', () => {
  it('10 hojas, 54 filas, y ninguna es de un paciente real', () => {
    /**
     * Eran 8 y 46. REG-604 añadió dos: un examen general de orina y un LCR, los
     ...
     * podía resolver, declarado como hueco desde REG-601.
     */
    const hojas = corpus()
...
describe('EL UMBRAL DE D-044 SE APLICA — y hoy REPRUEBA', () => {
  ...
  it('las 54 filas llegan al panel: 0 %', () => {
     ...
     *
     *  · 1-sep (REG-597): 7 de 46 fuera. 15,2 %. ROJO.
     *  · 2-sep (REG-598): 1 de 46. El médico entregó los rangos (D-045).
     *  · 2-sep (REG-599): 0 de 46 — pero NO porque se recuperara nada más, sino
     ...
     *  · 2-sep (REG-604): 0 de 54. El corpus creció con un examen general de
    ...
    expect(m.filas, 'REG-604 añadió dos hojas: orina y LCR').toBe(54)
  ...
  it('la glucosa en mmol/L ya NO se cae — y desde REG-603 se CONVIERTE', () => {
    /**
     * Hasta REG-599 esta fila desaparecía del panel: el analito estaba en el
     ...
     * como no verse.
     *
     * REG-599 la aceptó provisionalmente y la marcó, sin convertirla: el factor
     * no estaba en el catálogo del dueño y una equivalencia no se inventa.
     *
     * REG-603 no fue a buscar el factor: quitó la necesidad de tenerlo. Se
...
describe('LO QUE NO SE LE PREGUNTÓ, dicho a tiempo', () => {
  it('el analito inventado queda declarado como decisión pendiente', () => {
    /**
     * Que en la nota decidiera 0 % de alucinación (D-042) no lo decide aquí. Son
