...
## PENDIENTE DE FUSIÓN — rama `agent/safety/LAB-CENSURA-001` (PR #243)

| v | Qué se reparó |
|---|---|
| **1151** | **REG-270 — un valor de pánico CENSURADO se archivaba como NORMAL** (rama `agent/safety/LAB-CENSURA-001`). Un valor de pánico CENSURADO se archivaba como NORMAL. El laboratorio no siempre da el número: cuando el analizador se satura reporta un límite («Glucosa >400», «Plaquetas <20», «<50» en hipoglucemia severa). El prompt de visión ORDENA conservar ese signo y la IA obedecía —el dato llegaba— pero `aNumero` lo pelaba y los cortes son ESTRICTOS: `400 > 400` es falso, `50 < 50` es falso. Y no quedaba en ámbar: devolvía `evaluable: true`, «se juzgó y está bien». Pelar el signo (lo que hizo REG-036 en UCI) no bastaba porque allí los cortes son inclusivos: ahora se razona sobre el INTERVALO —«>n» ⇒ (n,∞)— y cuando cruza el umbral se declara no evaluable en vez de darse por bueno. NINGÚN umbral nuevo (siguen en C-1, sin fuente). El comparador viaja además hasta la nota, hasta la franja de críticos y desde el LIS por FHIR (`valueQuantity.comparator`, que se tiraba entero). VERIFICADO que puede ponerse rojo: 13 de 20 casos caen sin la reparación. |
