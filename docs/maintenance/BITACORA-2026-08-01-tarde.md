...
- **v1082** — REG-201 (verificando SAFE-001 antes de cerrarlo). El backlog hablaba de CUATRO parsers del campo de alergias y REG-171 unificó tres; recorriendo otra vez quién parte el campo apareció un QUINTO, y era el peor sitio posible: `hospital/cds.ts`, el punto de ORDEN — el único donde la alerta llega ANTES de firmar la indicación. Ni la «y» ni la barra separaban, así que «Niega penicilina y alérgica a sulfas» era UN fragmento, el negador de delante lo tumbaba entero y la alergia a sulfas DESAPARECÍA: al ordenar sulfametoxazol/trimetoprima, cero alertas críticas. Descartar lo frecuente y apuntar después lo que sí hay es como se escribe el campo normalmente. Y `alergiasEstructuradas` no se miraba por ninguno de los dos lados (ni la firma la aceptaba, ni el llamador la pasaba). Ahora usa `alergiasDe`, con la reacción incluida —el cruce betalactámico↔carbapenémico la necesita—. SEGUNDO hallazgo en las mismas ocho líneas: el bucle aplanaba a `critica` TODA alerta del cruce, incluida la que el motor baja a precaución a propósito (E0-15d: el carbapenémico no se bloquea con alergia a penicilina aislada), así que salía franja ROJA sobre un texto que dice «NO es contraindicación» — la pantalla deshacía la decisión del dueño. Restituida, no cambiada. NINGÚN umbral nuevo. VERIFICADO que puede ponerse rojo: 8 de 13 casos fallan sin el arreglo.
- **v1083** — REG-202. Auditando REG-201 antes de darlo por bueno: la unificación del punto de orden sobre el parser canónico se dejó atrás el NEGADOR que ese camino tenía. `hospital/cds.ts` conocía «nunca», «ausente» y `descart` a secas; el canónico no. Resultado: «nunca ha tenido reacción a penicilina» pasó a valer como ALERGIA y a sacar una crítica en el único sitio donde la alerta llega antes de firmar — la fatiga de alerta que REG-201 acababa de reducir, reintroducida por la puerta de atrás. El negador canónico es ahora la UNIÓN de los dos. La regla que queda: al fusionar dos motores que deciden lo mismo el resultado es la unión de lo que cada uno reconocía, no lo que traiga el que se quedó con el archivo. Y una palabra sólo entra al negador si NO puede encabezar el nombre de un alérgeno — ampliar sin esa condición escondería una alergia real, que es el error caro. VERIFICADO al revés (5/8 rojos).
- **v1084** — REG-203 (y con esto SAFE-001 queda cerrado). REG-201 amplió el guardián a `src/` entero, y con razón: la quinta copia vivía donde la lista no miraba. Barre lo que PARTE el campo a mano. Repasando el camino del alérgeno hasta el final aparecieron dos sitios más en la consulta que NO PARTÍAN NADA: el campo `alergias` de la NOTA QUE SE FIRMA y el sello de procedencia (tres usos, tras un ayudante local de una línea, `alergiasArray`). Los dos llamaban a `parsearAlergiasTexto` —el partidor bueno, sobre UNA SOLA de las dos fuentes—. Un paciente con la alergia sólo en `alergiasEstructuradas` salía con la alerta roja en pantalla, «Penicilina» en la receta impresa y `alergias: []` en el registro medicolegal; y el sello, al contar cero, dejaba ese dato fuera de `camposSinEvidencia`. La regla que deja: cuando un dato tiene DOS orígenes posibles, el guardián tiene que mirar QUÉ FUNCIÓN SE LLAMA, no cómo se corta el texto — un envoltorio de una línea sobre la función canónica parece inofensivo justo porque usa la función canónica. Hoy ninguna ruta de la app llena ese campo (divergencia latente), pero está en `CAMPOS_CLINICOS_PACIENTE` y cualquier importación la activa. NINGÚN criterio clínico nuevo. VERIFICADO que puede ponerse rojo: sin el arreglo fallan las tres afirmaciones de llamador.
- **v1085** — REG-204. SAFE-001 se dio por cerrado en v1084 y quedaba un camino: la FRANJA de alergias del internamiento — la que se ve todo el ingreso y es la única señal para quien NO pasa por el punto de orden (enfermería que administra, quien prescribe a mano). Con «Niega penicilina. Alérgico a sulfas» anunciaba en gris «Alergias negadas por el paciente», sobre un paciente alérgico a sulfas: partía por `/[,;\n]+/`, sin el punto era UN fragmento, empezaba por «niega» y su regla daba `negadas`. No es un aviso que falte — es el sistema AFIRMANDO LA AUSENCIA. Y POR QUÉ EL GUARDIÁN NO LA VIO: el barrido de REG-201 exige la palabra «alergia» a menos de 60 caracteres del `.split(`, en la MISMA línea; la franja copiaba el campo a `raw` y partía en la línea siguiente. El guardián en verde y la copia viva: la lección de REG-201 repetida un renombrado más tarde. Ahora `estadoAlergias` —`negadas` sólo si NO queda ningún alérgeno y hay fragmento negado; un campo vacío es «sin registro», no «no tiene»— y un SEGUNDO barrido que busca la FORMA del splitter (`.split(/[,;`) y no el nombre del campo, que un renombrado no esquiva. Reproducido copiando la lógica literalmente a un script antes de tocar nada. VERIFICADO al revés: revertidos uno a uno los tres trozos, fallan 3, 2 y 1 casos.
