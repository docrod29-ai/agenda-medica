 ...
 */
const CACHE = 'nexusmed-v489'  // LOTE AUDITORIA 2: alertas de laboratorio y opt-out de WhatsApp. (1) La alerta de valor CRITICO se comparaba a ciegas sin la unidad: un calcio de 3.5 mmol/L (hipercalcemia severa) se marcaba critico POR BAJO contra el umbral en mg/dL, o sea la alerta con la direccion clinica INVERTIDA, que es peor que no alertar; y creatinina en umol/L o hemoglobina en g/L inundaban de falsos criticos. Ahora se pasa la unidad y si no coincide con la del umbral se declara no-evaluable en vez de comparar. Ademas falsos criticos por el NOMBRE: la hemoglobina glucosilada disparaba anemia, el pH urinario acidemia, la fosfatasa alcalina fosforo, la creatinina en orina la serica — todos excluidos. NINGUN umbral se cambio (eso es decision clinica). (2) OPT-OUT DE WHATSAPP IGNORADO: la baja se guardaba con 521XX.. (el 1 de movil que antepone WhatsApp) pero el envio buscaba 52XX.., dos claves para el mismo numero, asi que quien pedia BAJA seguia recibiendo mensajes; el mismo desajuste rompia la ventana de 24h. Una sola normalizacion canonica cierra ambos.
