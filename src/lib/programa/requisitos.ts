...
export const REQUISITOS: readonly Requisito[] = Object.freeze([
  ...
  R({
    ...
    estado: 'NOT_STARTED',
    queFalta: 'Diagnóstico afinado al inspeccionarlo (sesión de REG-412; el trabajo se revirtió a propósito, ver abajo). El defecto CONCRETO no es la forma del array: es que una interconsulta NUNCA entra en el motor del bucle. Vive sólo en `Internamiento.interconsultas` y por tanto `tareasVivas`, `cabosDelPaciente` y `estadoDeAccion` no la ven; una pedida y no contestada es invisible fuera de una pestaña de un episodio. Es la misma forma que REG-252 cerró con `tareaDeResultado`. LO QUE HACE FALTA, por orden: (1) `origenId` en TareaClinica — hoy `idDerivado` sólo sabe dar identidad estable a lo que nace de una NOTA, y meter el id de la interconsulta en `notaId` rompería el enlace para todo el que lo lee esperando una nota; (2) un `TipoTarea` nuevo, `interconsulta_pendiente`; (3) crear la tarea DENTRO de la mutación de servidor `interconsulta_agregar`, que es el único sitio donde se conoce el id nuevo — `agregarInterconsulta` devuelve cadena vacía—, igual que `guardarResultados` ya crea tareas ahí; (4) cerrarla en `interconsulta_responder`. DOS DECISIONES QUE NO SON DE SOFTWARE: el PLAZO tras el cual una interconsulta sin contestar está vencida (depende de especialidad, urgencia y acuerdo del hospital: NEEDS_CLINICAL_REVIEW, y sin él `venceEn` no existe y `estaVencida` no opina), y en QUÉ grupo del worklist se enseña — `esperando_resultado` la etiquetaría mal («Esperando resultado» cuando se espera a un colega) y una octava categoría es la clase de modelo sin información que REG-404 evitó a propósito. REFERENCIA e IMAGEN son problemas aparte: la referencia sigue siendo sólo un impreso, y la imagen no tiene entidad (modalidad, lateralidad, informe, comparación con previos) — aunque el ORDEN de imagen sí entra ya en el bucle, porque `estudio_pendiente` cubre «laboratorio o gabinete». Todo esto es carril Hospital, ALPHA y no a la venta.',
  }),
  R({
