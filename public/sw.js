 ...
 */
const CACHE = 'nexusmed-v519'  // 2a OLA de auditores (consulta/expediente, finanzas/farmacia, seguridad). FIXES: (1) PERDIDA DE DATOS consulta: grabar en 2+ tramos con separacion de voces mandaba a la IA SOLO el ultimo tramo (utterances es por-blob) y la nota final perdia la 1a parte clinica; ahora en multi-tramo se usa la transcripcion completa (texto plano) y el hook limpia utterances al iniciar. (2) descartar() no borraba el audio de recuperacion en IndexedDB -> PHI residual + banner que reaparece; ya lo borra. (3) corregirConIA reemplazaba los signos vitales enteros (borraba los capturados a mano si la IA devolvia parcial); ahora merge por campo. (4) FINANZAS: registrarCobro sin idempotencia -> doble cobro concurrente inflaba el corte; ahora reserva atomica por citaId (runTransaction). (5) FARMACIA: el toast de salida mostraba la cantidad solicitada, no la aplicada tras clamp -> enganaba en controlados; ahora refleja la real. (6) REGLAS: clinics create ya no permite auto-activar entitlements/paseLibre (bypass de paywall via SDK cliente); clinic_review_requests create exige ser miembro de esa clinica (no cross-tenant).
