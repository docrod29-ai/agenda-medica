...
| 2026-08-27 | **POSTVISIT-001** | (esta rama) | El acto que faltaba: composición en el servidor de la nota FIRMADA, liberación explícita bajo `firmar`, idempotente y versionada, con retirada explícita; el paquete llega a `/mi/[token]` y hay enlace clínico para entregarlo. Cierra POSTVISIT-GATE-001 y POSTVISIT-ENTREGA-001 | **335** |
| 2026-09-02 | **PATIENT-AI-001** | (esta rama) | «ASK NEXUS»: clasificador determinista **sin modelo de lenguaje**, con el orden urgencia → acto prohibido → administrativa → cita del plan → escalar. Cuatro de las cinco clases del §2 (la quinta declarada y comprobada sin implementación). Ruta `preguntar` con alcance clínico, freno propio, escritura antes de contestar y aviso al consultorio; respuesta CONGELADA. Colección `preguntas_paciente` declarada en los tres sitios, más inventario de durabilidad y exportación ARCO. El destino «Preguntar» deja de ser un cartel. Fixture de 18 → 29 casos | **445** |
