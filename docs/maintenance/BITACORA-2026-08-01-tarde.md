...
- **v1078** — REG-196 (cierre de SAFE-001). Los tres parsers de fuera ya usaban `alergenosDe`, así que el ítem parecía hecho; siguiendo «el dato tiene que LLEGAR» aparecieron DOS llamadores más en la consulta, y uno es **el que sella la nota**. `nota.alergias` se sellaba con `parsearAlergiasTexto(patient?.alergias)`: sólo texto libre, así que una alergia en `alergiasEstructuradas` sellaba `[]`. Y de `nota.alergias` cuelga la COMPUERTA de firma (`nom004.ts`): cruce por subcadena + reactividad cruzada por familias. Reproducido con el motor real ANTES de tocar nada: «Penicilina» estructurada + cefalexina → la pantalla la pinta en rojo (lee `alergiasDe`) y la compuerta devuelve CERO errores. Peor que no mostrar nada: la pantalla invita a confiar en que la compuerta también lo sabe. Ahora la consulta sella con `alergiasDe` y `alergiasArray` recibe el paciente, no su campo de texto (de él cuelgan el manifiesto de procedencia y la compuerta de evidencia). NINGÚN umbral nuevo: fontanería de una fuente de verdad que ya existía. LATENTE hoy —ninguna ruta de escritura llena el campo estructurado— y cerrado antes de la primera importación que lo llene. VERIFICADO que puede ponerse rojo.
