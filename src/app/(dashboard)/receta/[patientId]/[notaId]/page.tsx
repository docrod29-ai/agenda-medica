...
import { revisarDosis, revisarUnidadDosis, extraerMg, extraerTomasDia, esDosisPorKg, filtrarSinReferencia, type AlertaDosis } from '@/lib/seguridad/dosis'
  ...
  const alertasDosis = useMemo(() => {
    ...
    for (const m of medicamentos) {
      ...
      const dosisPrescrita = esDosisPorKg(m.dosis)
        ...
        : cantidad(mg, 'mg', 'masa')
      // SAFE-003: en adulto se calla `sin_referencia` (ruido); en pediátrico se
      // conserva — «no tengo referencia de este fármaco» es distinto de «no hay
      // alerta» cuando la dosis va por kilo. Política única en `filtrarSinReferencia`.
      const al = filtrarSinReferencia(
        revisarDosis({ farmaco: m.nombre, dosis: dosisPrescrita, tomasDia: tomas, peso: pesoParaDosis != null ? kgMasa(pesoParaDosis) : undefined, via: m.via, edadAnios: edadPaciente }),
        esPediatrico,
      )
