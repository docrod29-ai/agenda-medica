...
## Qué cambió en esta rama — once slices, un commit cada uno (y uno para las decisiones)
...
| | |
|---|---|
| `npx vitest run` | **12 718 pasan · 1 falla** · 945 archivos (tras REG-523; la falla es `ops-timeout-y-punto-ciego`, el proxy del contenedor rechaza `10.255.255.1` al instante — entorno, no árbol; en la corrida de REG-520 pasó) |
...
| Sello | 469 archivos · 6 567 casos |
| Ledger | 317 REG · última **REG-523** |
...
`agent-state/AUSCULTA_LAST_SAFE_CHECKPOINT.md`. Lo primero: los tres del
test-the-test. El PR #442 queda absorbido aquí y lo puede cerrar el dueño. Los tres validadores
