...
Además: las dos oleadas de cierre (`crudos/Z-cierre-componentes.json`, 24 hallazgos sobre 49 componentes, 08:19; y `crudos/Z-cierre-lib.json`, 19 hallazgos sobre 113 piezas de `src/lib`, 08:23 — las dos escritas mientras se redactaba este archivo) aportan **5** `boton_muerto` más que **todavía no tienen veredicto rojo**. Van en su propia sección, con la prioridad del auditor, y no entran en el conteo de confirmados.
...
| `/configuracion › Recetas` | «Quitar» el diseño de receta | Pone `disenoMargenes` y `disenoCampos` en `undefined`; `sinUndefined` los borra del payload y `merge: true` **conserva los viejos** en Firestore: la calibración anterior reaparece. El propio comentario de `secciones-recetas.tsx:585-592` conoce el mecanismo y sólo lo aplica a un campo. Ningún `deleteField` en `app/` ni `components/` | Que `sinUndefined` convierta `undefined` en `deleteField()` (y el servidor use `FieldValue.delete()`), o normalizar en `''`/`null` todo lo que se «quita»; prueba que borre un campo y lo lea de vuelta («el dato tiene que LLEGAR») | `src/lib/firestore.ts:768` · `:779-785` · `secciones-recetas.tsx:585-592` | ZL-010 | P3 |
| *(UCI, en pausa)* Comandos de voz manos-libres | `interpretarComandoUCI` (navegar por sistemas, corregir valores, marcar pendientes) | Escritos y probados; **ninguna pantalla los interpreta** (0 llamadores). Y su patrón «cambiar … a/por …» (`:179`) tomaría contenido clínico como orden de corrección el día que se conecten | Dejar, con fecha; antes de conectar, exigir un prefijo de activación (p. ej. «comando:») | `src/lib/voz/comandos-uci.ts:160` · `:179` | ZL-009 | P3 |
| `/consultor` | Búsqueda de evidencia por PICO (`pico.ts` + `buscar-con-pico.ts`) | Escrita, tipada y probada; **ninguna ruta la llama** (la propia cabecera lo dice: «Sin callers todavía: el cableado de las rutas es E2-05»). El consultor sigue buscando con la pregunta cruda | Cerrar E2-05 o fechar el módulo como pendiente en `agent-state`; no dejarlo indefinidamente | `src/lib/evidencia/buscar-con-pico.ts:21` · `:34` | ZL-017 | P3 |
