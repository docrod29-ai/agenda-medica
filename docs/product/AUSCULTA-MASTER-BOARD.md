...
| **P1 internos abiertos** | **1** (P1-14, **a medias** desde REG-421: la parte del repositorio está reparada, lo que queda es trabajo de este repositorio con nombre; ver abajo. Más 1 `BLOCKED_EXTERNAL`: P1-6. La confirmación en consola de que los índices terminaron de construirse sigue siendo externa, pero ya no bloquea el código) |
...
| **P1-14** | **A MEDIAS (REG-421).** El worklist ya no devuelve 200 **arbitrarias**: `tareasVivas` pide `orderBy('creadaEn')` con el índice `tareas_clinicas(estado, creadaEn)`, así que el recorte se lleva a las MÁS NUEVAS y una tarea vieja no puede caerse. Los otros tres sacrificios de la misma pared —lista de espera, citas del paciente, resumen de notas— quedan reparados en el mismo cambio. **Lo que sigue abierto es lo que P1-14 pedía de verdad**: «las más urgentes». Ordenar por urgencia en el servidor necesita un índice `(estado, prioridad, creadaEn)` **y** un campo NUMÉRICO de peso — `prioridad` guarda texto y en orden alfabético `alta` iría antes que `critica`, o sea que Firestore ordenaría al revés de lo que dice la palabra. Eso ya **no es `BLOCKED_EXTERNAL`**: es trabajo de este repositorio, con nombre. **Lo que sí sigue siendo externo** —y no lo cierra ningún commit— es mirar la consola de `nexomed-agenda` y ver los índices `Enabled`: el despliegue del 31-ago (ejecuciones #11 y #12 del botón, paso `firestore:rules,firestore:indexes` en `success`) **envió** siete de los nueve, y la construcción de un índice compuesto es **asíncrona** — `deploy` devuelve al ENVIAR, no al terminar, y un índice puede quedarse en `Building` horas o caer en `Error` mucho después del `success`. Los dos que añadió REG-421 (`waitlist(estado, createdAt)` y `clinic_invitations`) no se han enviado todavía. |
