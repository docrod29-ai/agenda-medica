...
| **Regresiones cerradas** | **14** (REG-513 … REG-526) |
| **Arneses nuevos** | **4** |
...
- **Finanzas ya no esconde dinero.** `<main>` medía 390 px de ancho con 685 px
  ...
  a lado, que es la comparación para la que existe el par. **(REG-521)**
...
- **El diálogo que confirma un borrado ya no se dispara solo.** `confirm()`
  ...
  navegación ni siquiera ocurrió. **(REG-524)**
- **Las tardes ya no envejecen a nadie.** A partir de las 18:00 hora del
  ...
  tarde es justo cuando el médico repasa la jornada. **(REG-525)**
- **La edad del paciente ya no se queda vieja** (REG-513), **la alergia no se
  dice dos veces** (REG-514), **la fecha se escribe igual en todo el producto**
  —nueve pantallas la enseñaban en ISO— (REG-516), **el expediente dice quién
  puso cada diagnóstico** (REG-519), y **el riel de la agenda mira el reloj**
  (REG-518).
- **Cuando la IA no está, el error no nombra al proveedor**: quince rutas se lo
  contaban al médico. **(REG-517)**
- **Dos defectos de contraste que sólo existían en tema claro** (REG-520).
...
El cambio de mayor alcance es el de `ultimaVezVisto` (REG-525), que afecta a
