 ...
 */
const CACHE = 'nexusmed-v471'  // MODULO 2, LOTE 2 — cierre. (1) Cancelar desde el menu rapido NO borraba el evento de Google Calendar: el paciente lo seguia viendo y se presentaba (el borrado y el modal si lo hacian). (2) El filtro de medico se quedaba pegado si ese medico se daba de baja: el selector deja de dibujarse con un solo medico activo pero el filtro seguia aplicandose — agenda vacia todos los dias y sin control en pantalla para quitarlo. (3) El modal reescribia las banderas de recordatorio con el valor congelado al abrirlo: si el cron enviaba mientras estaba abierto, guardar las volvia a false y el paciente recibia el aviso DOS veces. (4) /api/public/availability leia la coleccion COMPLETA de citas en cada clic del portal, sin auth. (5) Borrar una cita cobrada dejaba el cobro huerfano y descuadraba el corte de caja. (6) La cita sin expediente ligado se creaba en silencio con toast verde. (7) Reagenda del portal marca la cita como desincronizada de Google (no se adivina el calendario). (8) Backfill idempotente para reconstruir los contadores historicos.
