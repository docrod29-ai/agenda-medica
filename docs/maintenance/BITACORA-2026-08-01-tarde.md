...
- **v1151** — REG-270. Auditando `uci-dosis-sin-numero` (el motor que avisa de una dosis que perdió su número al transcribirse) apareció el defecto CONTRARIO: la lista de números en letra que valen como cantidad iba 1…12, saltaba a 15, a 20 y de ahí a las decenas — faltaban el 13, el 14, el 16-19 y TODO el 21-29. «Metoprolol veinticinco miligramos» salía marcado como dosis SIN cantidad, con la cantidad delante en la misma frase, y por `corrector-vigilado` levantaba `requiereConfirmacion`: la compuerta le pedía al médico que confirmara lo que había dictado bien. El hueco cae sobre la posología MÁS repetida de la consulta (25 mg de metoprolol, espironolactona, captopril, hidroclorotiazida; 18 unidades de glargina). Las decenas con «y» se salvaban por accidente —en «treinta y cinco» la palabra previa es «cinco»—, y por eso el hueco pasó dos revisiones sin verse: sólo caían los numerales de UNA sola palabra. Segunda vez que este mismo módulo falla por enumeración incompleta (la primera fue el signo menos del balance hídrico). NINGUNA cifra clínica cambia: cambia qué cuenta como número escrito en letra en español, y el motor sigue detectando sin completar ninguna dosis. Reproducido con el motor real antes de tocar nada y verificado que el golden se pone rojo (6 de 12 sin el arreglo).
