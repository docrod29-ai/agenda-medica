...
| **P1 internos abiertos** | **1** (P1-14, a medias: ver abajo — dejó de ser `BLOCKED_EXTERNAL` con REG-417; más 1 `BLOCKED_EXTERNAL`: P1-6) |
...
| **P1-14** | **A MEDIAS (REG-417).** El worklist ya no devuelve 200 **arbitrarias**: `tareasVivas` pide `orderBy('creadaEn')` con el índice `tareas_clinicas(estado, creadaEn)` ya declarado, así que el recorte se lleva a las MÁS NUEVAS y una tarea vieja no puede caerse. **Lo que sigue abierto es lo que P1-14 pedía de verdad**: «las más urgentes». Ordenar por urgencia en el servidor necesita un índice `(estado, prioridad, creadaEn)` **y** un campo NUMÉRICO de peso — `prioridad` guarda texto y en orden alfabético `alta` iría antes que `critica`, o sea que Firestore ordenaría al revés de lo que dice la palabra. Ya no es `BLOCKED_EXTERNAL`: es trabajo de este repositorio, con nombre. |
