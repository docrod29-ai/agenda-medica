...
| 4 | `POSTVISIT-001` | ✅ **cerrada** | *(ver checkpoint)* |
...
> `POSTVISIT-GATE-001` y `POSTVISIT-ENTREGA-001` **cerraron** el 10-ago con
> `POSTVISIT-001` (REG-306, REG-307). En su lugar quedan dos P2 declarados —
> `POSTVISIT-VERSION-002` y `POSTVISIT-DOSIS-003`—, que no bloquean.
...
| unit + integration (`npx vitest run`) | ✅ salvo 1 fallo de entorno declarado (`ops-timeout`) |
