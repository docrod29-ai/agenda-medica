...
export type Lectura =
  ...
  | { readonly legible: false; readonly porQue: string }
...
export function leerEsearch(cuerpo: unknown): Lectura {
  if (!cuerpo || typeof cuerpo !== 'object') {
    return { legible: false, porQue: 'La respuesta de esearch no era un objeto JSON.' }
  }
  const c = cuerpo as Record<string, unknown>
  /* NCBI pone el error DENTRO del 200, tanto arriba como en `esearchresult`. */
  if (typeof c.ERROR === 'string' && c.ERROR) {
    return { legible: false, porQue: `esearch contestó 200 con un error dentro: ${c.ERROR}` }
  }
  const res = c.esearchresult
  if (!res || typeof res !== 'object') {
    return { legible: false, porQue: 'La respuesta de esearch no traía `esearchresult`.' }
  }
  const r = res as Record<string, unknown>
  if (typeof r.ERROR === 'string' && r.ERROR) {
    return { legible: false, porQue: `esearch contestó 200 con un error dentro: ${r.ERROR}` }
  }
  if (!Array.isArray(r.idlist)) {
    return { legible: false, porQue: 'La respuesta de esearch no traía la lista de identificadores.' }
  }
