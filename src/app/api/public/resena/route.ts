...
export async function POST(req: NextRequest) {
  ...
  // RATE-LIMIT (endpoint público, sin auth): por IP, contra la adivinanza del
  // token por fuerza bruta. Mismo criterio que public/booking.
  const ip = req.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || 'sin-ip'
  const limIp = await limitarOResponder(`resena:ip:${ip}`, 8, 3600, 'Demasiadas solicitudes. Intenta más tarde.')
