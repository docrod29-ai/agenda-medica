...
## SIN DESPLEGAR — v1150, en rama `agent/expediente/vocabulario-clinico`

- **v1150** — REG-268 (auditoría del motor de temporalidad, ítem EVAL-002 del backlog). Los DOS vocabularios clínicos del expediente —`CRONICAS` y `AGUDAS_FRECUENTES`— se buscaban con `texto.includes(forma)`: «obeSIDAd» y «neceSIDAd» daban VIH, «plASMA» daba asma, «preDIABETES» daba diabetes, «coleCISTITIS» daba infección urinaria, «Klebsiella pNEUMONIAe» daba neumonía y «ENFISEMAtosa» daba EPOC. MEDIDO sobre sus 6 000 frases: 68 casaban sólo por dentro de otra palabra, 55 falsas. El ruido era lo de menos: `corregirCertezaPorNegacion` RECLASIFICA, así que un paciente que NIEGA LA OBESIDAD hacía que un VIH dictado por el médico saliera marcado como `descartado` — un diagnóstico apagado por una palabra que nadie dijo, en la consulta de un infectólogo. Nuevo `vocabulario-clinico.ts`: un solo buscador para los dos motores, frontera de palabra por los dos lados y detrás sólo el plural (única flexión que deja la misma palabra). Las 13 coincidencias legítimas —miocardiopatía, neurocirugía, postinfarto— se DECLARAN como formas: no se pierde ninguna. NO entra el «derrame pleural → EVC»: casa como palabra entera, es la lista y no el buscador, y lo repara el PR #239 de otra corrida de la misma noche. VERIFICADO que puede ponerse rojo: con `indexOf` fallan 14 de 21 casos.
