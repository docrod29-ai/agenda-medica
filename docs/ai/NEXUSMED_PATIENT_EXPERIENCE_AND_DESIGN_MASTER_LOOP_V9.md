...
| 2026-08-09 | **NAVIGATION-001** *(abierta, no cerrada)* | *(esta sesión)* | `NAV-AGENDA-001`: la consulta usa `useSmartBack` como las otras diez pantallas, y la agenda guarda fecha, filtro y búsqueda en la URL. Y `proximoSeguimiento`, que REG-193 arregló en uno de tres caminos: el que faltaba **borraba** la copia buena al salir. **Falta**: scroll fuera de la consulta, filtro del expediente, y las seis comprobaciones que piden navegador | **289** |
