    ...
    return (
      <div style={layoutStyle}>
        <div style={cardStyle}>
          ...
          <p style={{ fontSize: 14, color: 'var(--text3)', marginBottom: 16, lineHeight: 1.6 }}>
    ...
    return (
      <div style={layoutStyle}>
        <div style={{ ...cardStyle, maxWidth: 720, textAlign: 'left' }}>
          ...
          <pre style={{ whiteSpace: 'pre-wrap', fontSize: 12, lineHeight: 1.65, color: 'var(--text2)' }}>
  ...
  return (
    <div style={layoutStyle}>
      <div style={{ ...cardStyle, maxWidth: 560, textAlign: 'left' }}>
            ...
            <div style={{ display: 'grid', gap: 8, marginBottom: 18 }}>
              {(['acceso', 'rectificacion', 'cancelacion', 'oposicion', 'revocacion'] as ArcoTipo[]).map(t => (
                <button
                  ...
                  style={{
                    padding: '12px 14px', textAlign: 'left', background: 'var(--s2)',
                    border: '1px solid var(--border)', borderRadius: 'var(--r-card)', cursor: 'pointer',
        ...
        {paso === 'formulario' && (
          <>
            <button onClick={() => setPaso('info')} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 13, marginBottom: 12 }}>
              ← Volver
            </button>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              ...
              <div>
                <label htmlFor="arco-descripcion" style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 4 }}>
                ...
                <textarea
                  ...
                  style={{
                    width: '100%', padding: '8px 10px', borderRadius: 6, border: '1px solid var(--border2)',
                    background: 'var(--s2)', color: 'var(--text)',
                    fontSize: 13, fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box',
                  }}
                />
                <div style={{ fontSize: 10.5, color: 'var(--text3)', textAlign: 'right', marginTop: 2 }}>{descripcion.length}/1000</div>
              </div>
              <div style={{
                padding: 10, background: 'var(--warn-bg)', borderRadius: 6, fontSize: 12, color: 'var(--warn-text)',
...
function Field({ label, value, onChange, type = 'text', placeholder, maxLength }: { label: string; value: string; onChange: (v: string) => void; type?: string; placeholder?: string; maxLength?: number }) {
  ...
  return (
    <div>
      <label htmlFor={id} style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 4 }}>{label}</label>
      <input
        id={id}
...
const cardStyle: React.CSSProperties = {
  background: 'var(--s1)', border: '1px solid var(--border)', borderRadius: 'var(--r-modal)',
  padding: 24, maxWidth: 440, width: '100%',
  boxShadow: 'var(--sombra-menu)', textAlign: 'center',
}
