...
## REG-270 — la respuesta no es su primera palabra (versión por asignar)

**El número, por TERCERA vez, lo pone quien fusiona.** Esta rama nació sin número porque
había 14 PRs reclamando «REG-192 · v1074». Se le asignó REG-213 / v1095 y, para
cuando llegó a fusionarse, **otro PR ya se había llevado ese número** (REG-213 es
hoy «¿de dónde sacó la IA esto?») y `main` iba por v1149 — una versión de caché
por debajo de la desplegada es peor que no tocarla. Así que aquí NO se reclama
versión de service worker: se la pone quien despliegue. Es la M-2 de
`OWNER_DECISIONS_REQUIRED.md`, y van dos veces que hace falta.

**Qué queda de esta rama después de reconciliar con `main`** — REG-192 trajo
`respuestaNiega()` con limpieza del ruido de turno, `NO_ES_NEGACION` para «no sé»
y `NEGATIVAS_SIN_NO`. Eso es mejor que lo que traía esta rama para lo mismo y se
adopta entero. Lo que sigue siendo de aquí, comprobado contra el árbol de `main`:

**1. «Nada más la diabetes» descartaba las dos.**

```
condicionesNegadas('¿Tiene diabetes o presión alta? Nada más la diabetes.')
  → ['diabetes', 'hipertensión arterial']
```

Es una afirmación **parcial** leída como negación: descartaba incluso la que se
acababa de afirmar, y `corregirCertezaPorNegacion` bajaba a `descartado` una
hipertensión `confirmado`. Y la limpieza de ruido de REG-192 lo **agrava**: quita
el «pues» y deja «nada más la diabetes» empezando por «nada», así que llegan aquí
frases que antes ni se miraban. Es la misma trampa que `NO_ES_NEGACION` documenta
para «no sé», por el otro lado.

**2. «No, sí tengo».** El paciente se desdice a media frase y el motor se quedaba
con la primera palabra. Gemelo de «no sé»: mirar el arranque no basta.

**3. El pretérito y las conjugaciones que faltaban.** `No padeció diabetes.`
devolvía `positivas: ['Diabetes mellitus tipo 2']`. Estaban `padece` y `padezco`;
faltaban `padecía/padeció`, `sufre/sufro/sufría/sufrió` y `negó/negaron` — y
`negó` fallaba doble, porque `\b` no cierra tras la «ó».

**4. El guardián del reconocedor, que nadie había tocado.** Sin `padece/sufre`,
un corrector que convirtiera «no padece diabetes» en «padece diabetes» no volteaba
ninguna marca. Y su cierre `(\s|$)` **no cierra en un punto**, así que perder el
«no» al final de una frase —donde acaban tantas— no se marcaba.

**5. La ventana de `estaNegado`, sin normalizar.** Se llama por dos caminos, uno
con el texto crudo, y por ése ninguna negación acentuada casaba. Normalizarla
sólo para `NEGADORES` dejaba a `AFIRMADORES` mirando el crudo: la misma frase
daba dos respuestas según llevara tilde. Ahora se normaliza una vez, al entrar.

**Lo que NO se metió** — «Qué va» ya lo trae `NEGATIVAS_SIN_NO` de `main`; aquí
se había dejado fuera por miedo a que empezara una frase. Se acepta el criterio
de `main`, que es el que ya está probado en el árbol.

**Lo que NO hace, y queda para el médico** — Con «no sé», el motor deja de
afirmar que el paciente negó y ahí se detiene: la condición que el extractor
cosechó de la PREGUNTA conserva su certeza. La tercera salida —«el paciente dice
que no lo sabe», enseñada en pantalla— sigue abierta como SAFE-005.

**Comprobado que puede ponerse rojo** — Sobre el árbol de `main`, sin los cambios
de esta rama, el golden falla; el detalle de cuántos casos y cuáles está en la
cabecera del propio archivo, que se reescribió al reconciliar.

**Golden** — `src/__tests__/asi-se-niega-en-la-consulta.test.ts`.
