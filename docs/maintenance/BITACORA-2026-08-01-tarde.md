...
- **v1151** — REG-270. Probando el caso del TITULAR de REG-192 contra los motores reales de `main` —ya fusionado, con su golden en verde— seguía sin avisar: `contradicciones()` devolvía `[]` con el paciente negando la diabetes y la nota diagnosticándosela. `primeraMencionSinEscudo` juzgaba cada aparición con los 60 caracteres previos EN CRUDO, y la mención de abajo tiene el «niega» de la LÍNEA DE ARRIBA dentro de esos 60. Ahora el contexto se recorta en el punto, el salto de línea y el punto y coma. EN LOS DOS PUNTOS NO, y ésa fue mi primera versión: el encabezado ES el escudo («Antecedentes personales patológicos: neumonía en 2019»), así que cortar allí hacía avisar del antecedente BIEN escrito en vez del diagnóstico — lo cazó el golden de REG-192 al correr la suite entera. `NO_AFIRMA` para que el recorte no estrene fatiga de alerta. NINGÚN cambio clínico. VERIFICADO que puede ponerse rojo (7 de 16). NO se mezcla la frontera de palabra («plasma» contiene «asma»): queda como SAFE-008.
