...
describe('tareasCerradasRecientes() — módulo, no consulta inline', () => {
  it('existe y filtra por estado === cerrada, nunca por cancelada', () => {
    ...
    expect(fn).toContain("pedirListado(clinicId, 'cerradas', tope)")
    const servidor = leer('src/app/api/tareas/listar/route.ts')
    expect(servidor).toContain("body.modo === 'cerradas'")
    expect(servidor).toContain("cola.where('estado', '==', 'cerrada')")
