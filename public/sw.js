 ...
 */
const CACHE = 'nexusmed-v504'  // DOS ARREGLOS DE LO QUE REPORTO EL DR CON CAPTURA/VIDEO. (1) DOS ALERTAS DE ALERGIAS en el expediente: yo introduje una segunda al agregar la tarjeta de resumen (v496), encima del banner que ya existia. Se deja UNA sola, la de arriba (mas prominente, se ve primero), y se le pone la logica lista: rojo solo si hay alergias REALES, neutro si estan negadas (rojo cuando no hay alergias desgasta la senal). La tarjeta de resumen ya no repite alergias. (2) DESCARTAR AUDIO NO PERSISTIA: el boton 'Descartar audio guardado' llamaba a reset(), que solo borra de IndexedDB la clave de la sesion ACTUAL (recoveryKeyRef); al recargar y encontrar audio huerfano esa ref esta vacia, asi que no se borraba nada y el audio reaparecia en cada recarga. Nueva descartarRecovery(clave) que borra por clave explicita.
