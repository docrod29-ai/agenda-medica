 ...
 */
const CACHE = 'nexusmed-v479'  // MODULO 4 (hospital) — P0 de seguridad del paciente + cardiometabolico. (1) SE PODIA ADMINISTRAR UN FARMACO A UN PACIENTE YA EGRESADO: 'Administrar' era la unica accion del MAR sin la guarda de egresado, y egresar no cierra las indicaciones (quedan activas para siempre); el servidor lo aceptaba. (2) Y una indicacion SUSPENDIDA tambien: el servidor no comprobaba 'activa', asi que si el medico suspendia mientras enfermeria tenia el modal abierto, la dosis se registraba igual. (3) El MAR atribuia la administracion al MEDICO TITULAR: 'por' salia de config.nombreMedico, un documento COMPARTIDO por toda la clinica, no del usuario en sesion. Ahora el servidor sella autor y hora. (4) El campo 'identidad verificada' se ponia true con solo tildar una casilla, sin escanear el brazalete — dato inventado en el registro medicolegal. (5) NEWS2 marcaba parcial solo si faltaba conciencia u oxigeno: un score de 1 de 7 parametros salia 0/riesgo bajo/completo. (6) Signos vitales SIN limite (misma raiz que la lentitud de la agenda). (7) Una alerta critica que no se enviaba mostraba toast VERDE. Ademas: biomarcadores lipidos (apoB, Lp(a), no-HDL, remanente) y nomenclatura MASLD.
