/**
 * ASE-004 (Panel de Lujo 2026-09, auditor AS-expedientes; reproducción
 * REP-039) — un Excel con «Nombre», «Apellido
 ...
 *
 * ── LA REPARACIÓN ───────────────────────────────────────────────────────────
 * `SINONIMOS` reconoce las piezas del nombre (apellido/apellidos/apellido
 * paterno/apellido materno, y sus equivalentes en inglés) como `CampoImportado`
 * —no como campos nuevos del expediente: `Patient` sigue teniendo UN nombre— y
 * `construirFilas` los COMPONE en el orden de una identidad mexicana. Y para
 * que el hueco no vuelva a ser silencioso, `columnasDescartadas` devuelve las
 * columnas con dato que no alimentan nada, que es lo que /migracion pinta en
 * rojo antes de dejar importar.
 *
 * ── QUÉ NO CUBRE ─────────────────────────────────────────────────────────────
...
describe('los apellidos en columnas separadas llegan al nombre completo', () => {
  ...
  it('«Nombre / Apellido paterno / Apellido materno» → «Ernestina Quiroga Balbuena»', () => {
    expect(importar(
  ...
  it('«Nombre / Apellidos» → «Ernestina Quiroga Balbuena»', () => {
    expect(importar(['Nombre', 'Apellidos', 'Teléfono'], ['Ernestina', 'Quiroga Balbuena', '5550101010'])).toBe('Ernestina Quiroga Balbuena')
  })

  it('la columna de apellidos no se descarta en el mapeo (deja de ser null)', () => {
