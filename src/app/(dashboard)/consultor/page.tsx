      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--bg)', borderTop: '1px solid var(--border)', padding: '12px 16px' }}>
        <div style={{ maxWidth: 820, margin: '0 auto', display: 'flex', gap: 8, alignItems: 'flex-end' }}>
          ...
          <button onClick={() => preguntar(pregunta)} disabled={cargando || !pregunta.trim()}
            style={{ flexShrink: 0, width: 44, height: 44, borderRadius: 12, border: 'none', cursor: cargando || !pregunta.trim() ? 'default' : 'pointer', background: cargando || !pregunta.trim() ? 'var(--s3)' : 'var(--nexus-solido)', color: cargando || !pregunta.trim() ? 'var(--text3)' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
