...
- **v1151** — REG-271. Una CMI es un INTERVALO, no un número (decisión E0-15c del Dr., la que gobierna `interpretarCMI` desde REG-044) — y `cmiDe` devolvía el número PELADO y tiraba el operador, así que los fenotipos de Gram positivos leían el MISMO panel sin él mientras el motor de puntos de corte sí lo respetaba. Reproducido con `interpretarAntibiograma` antes de tocar nada: neumococo con penicilina «>2» no meníngea imprimía «CMI 2 ≤2 → **tratable con penicilina parenteral a dosis altas**» —el número que niega la frase, al lado—; SARM con vancomicina «>2» caía en «sospecha de hVISA, límite alto de S» en vez de eficacia reducida; E. faecium con daptomicina «>4» no decía NADA. Y al revés, un tamiz de gentamicina «<500» declaraba HLAR y hacía abandonar la sinergia β-lactámico + aminoglucósido en endocarditis. `cmiDe` DESAPARECE —mientras exista una forma de pedir «el número» alguien la volverá a usar—: `cmiConCensuraDe` + tres predicados en positivo (`cmiAlcanza`, `cmiSupera`, `cmiNoPasaDe`) que devuelven `false` cuando no se sabe, y `cmiIndeterminadaEn` que lo DICE y pide dilución. NINGÚN punto de corte nuevo y nada se sube a R (sería inventar en la otra dirección). Con valor exacto no cambia nada. VERIFICADO que puede ponerse rojo: 8 de 20 casos fallan sin el arreglo.
