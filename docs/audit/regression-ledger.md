...
## REG-270 — la respuesta se leía por su primera palabra (v1151)
...
**Golden** — `src/__tests__/la-negacion-se-lee-entera.test.ts` (16 casos
estáticos, 31 ejecutados).

**Al fusionar con `main`** — REG-192 (v1074) reemplazó el bucle de
`contradicciones` por `primeraMencionSinEscudo`, que mira **todas** las
apariciones y no sólo la primera. Se toma esa versión, que es mejor, y la
negación pegada viaja con ella en `ESCUDO_DE_LA_NOTA`: el módulo compartido
admite un solo patrón, y cada una de las dos trae su propio anclaje, así que la
alternancia no las mezcla.
