...
| P-006 | Voice + Note engine con provenance | **CASI HECHO — corregido tras leer el código.** Mi primera lectura fue injusta: `lib/expediente/procedencia.ts` YA hace provenance POR CAMPO, y bien. No la declara: la **deriva de evidencia** — si el dato coincide con la extracción y trae `source_quote`, es `dictado` (y se puede leer la frase exacta); si coincide sin cita, es `ia`; si no coincide, lo escribió el médico. Además registra `confirmado` por campo, con un detalle fino: no compara índices entre las dos listas porque se desfasan al rechazar un ítem, y dar por confirmado un diagnóstico con el visto bueno de OTRO sería un dato falso en el registro. **HECHO (v892)**: el vocabulario ya distingue `calculado` e `importado`. Dejó de ser un matiz en cuanto v891 metió datos de monitor: `manual` significa «lo escribió el médico», y sellar así un signo que midió un aparato es afirmar algo falso en el registro. Hay puente entre la `fuente` de una toma y el sello de la nota, y una fuente desconocida NO se degrada a `manual` — devuelve `null`, porque inventar un autor es peor que no tener uno. |
