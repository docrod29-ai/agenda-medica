...
- **v1074** — REG-192. Auditando el motor de temporalidad (EVAL-002 pedía corpus) salió un defecto en los DOS motores que contrastan dictado y nota: buscaban el término con `indexOf` y, si esa PRIMERA aparición venía bien escrita, hacían `break`. La nota real hace las dos cosas — el negativo pertinente arriba, en el resumen, que es lo que el prompt le ORDENA escribir, y el diagnóstico arrastrado abajo, en la lista de `textoDeLaNota` —, así que la de arriba callaba a la de abajo: **cuanto mejor redactada la nota, más ciega la defensa**. Es el caso que fundó el motor (3-ago: «¿diabetes o presión alta? No.» → nota con las dos). Y escapaba POR SUERTE cuando alguna forma más larga del vocabulario («diabetes mellitus») casaba con la segunda mención; con «hipertensión» no hay suerte y quedaba mudo. `primeraMencionSinMarca` compartido: todas las apariciones de todas las formas, en orden de nota. NO toca la ventana de 60 caracteres ni el vocabulario. MEDIDO de paso: el corpus de 6 000 frases del Dr. NO sirve de oro para temporalidad — 0 disparos, son órdenes y cifras, no interrogatorio. Y queda un hallazgo aparte sin reparar (SAFE-002): `YA_ES_ANTECEDENTE` acepta «previo» a secas, así que «sin trauma previo» delante de un diagnóstico lo silencia.
