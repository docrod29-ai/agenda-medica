...
- **v1074** — REG-192 (SAFE-002). El paciente que NEGABA la alergia quedaba alérgico: «Niega alergias a penicilina **y sulfas**» devolvía `['sulfas']`, porque el negador se escribe una sola vez —en el primer fragmento— y el alcance se perdía en el separador. Reproducido con el motor real: el cruce contestó `critica — «Alergia a sulfas y se prescribe Trimetoprima/Sulfametoxazol»`. De ahí salían la alerta que apaga Firmar, el recuadro rojo de la receta que va a la farmacia y el sello de una nota inmutable; y al médico le dejaba la única salida que este repositorio documentó como el fallo a evitar: BORRAR el texto del expediente. La negación ahora alcanza a toda la enumeración y se corta con un fin de frase o con una palabra que afirme, para NO llevarse por delante la alergia real escrita tras una negada (el caso ganado el 4-ago). NO cambia qué bloquea la firma. Y apareció la QUINTA copia del partidor del campo —`hospital/cds.ts`—, que se le escapó al guardián de REG-144 porque sólo miraba `consulta` y `uci`: traía el mismo defecto más otros dos (no partía por «/» ni por « y », y no leía `alergiasEstructuradas`). Ya usa el canónico; sus negadores de más («nunca», «ausente») se subieron al canónico. VERIFICADO al revés por separado: 6 casos en rojo sin `alergias.ts`, 3 sin `cds.ts`.
