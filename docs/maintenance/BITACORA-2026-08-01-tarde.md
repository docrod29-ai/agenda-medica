...
- **v1074** — REG-192 (hallazgos C2+C3). «No padece alergia a penicilina», en el campo de alergias, quedaba registrado como UN ALÉRGENO LLAMADO ASÍ: como el cruce busca el fármaco DENTRO del texto del alérgeno, esa cadena contiene «penicilina» y disparaba la alerta crítica que APAGA EL BOTÓN DE FIRMAR — en el paciente que acababa de negar esa alergia, y con la única salida de borrar el texto del expediente (el desenlace exacto que `alergias.ts` se escribió para evitar, por la puerta de al lado). En el mismo dictado «No padece diabetes» salía como ANTECEDENTE POSITIVO, y los antecedentes se arrastran. LA CAUSA: **cuatro** listas de negadores, una por archivo, ninguna al tanto de las otras — cada una creció el día que un defecto la tocó A ELLA y el verbo añadido no llegaba a los otros tres; «padece», con el que se contesta el interrogatorio en México, sólo lo conocía uno. La deriva iba TAMBIÉN AL REVÉS: `padece` faltaba en los AFIRMADORES, así que «niega tabaquismo, padece diabetes» BORRABA una diabetes real mientras «…tiene diabetes» funcionaba. Y el motor de contradicción sólo entendía «No.» a secas: «pues no», «fíjese que no», «para nada», «tampoco» —como se contesta de verdad— se perdían enteras. Nuevo `negadores.ts` con el vocabulario UNA VEZ; el ANCLAJE no se comparte (es política de cada sitio: el campo de alergias exige que la negación ABRA el fragmento) y `sin` suelto se queda fuera del grupo común («sin control de la diabetes» habla de una diabetes que SÍ existe). NINGUNA cifra ni política nueva: es vocabulario. VERIFICADO que puede ponerse rojo revirtiendo los CINCO cambios uno por uno.
