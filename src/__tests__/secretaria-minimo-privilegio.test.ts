...
describe('GP11 — el borde de datos niega notas clínicas a la secretaria', () => {
  ...
  it('las notas del expediente se leen y escriben sólo detrás de isMedico', () => {
    ...
    const bloque = reglas.slice(inicio, fin)
    expect(inicio).toBeGreaterThan(-1)
    expect(bloque).toContain('allow read: if esMedicoDelPaciente(clinicId, docId)') // D-057: isMedico + alcance del paciente
    expect(bloque).toContain('allow create: if esMedicoDelPaciente(clinicId, docId)')
    expect(bloque).toContain('allow update: if esMedicoDelPaciente(clinicId, docId)')
    expect(bloque).toContain('allow delete: if esMedicoDelPaciente(clinicId, docId)')
