...
- **v1151** — REG-270 (EVAL-002 del backlog). El motor de TEMPORALIDAD no tenía corpus: sus únicos casos eran los de quien lo escribió, sacados de las mismas expresiones regulares que probaban — pasaban por construcción. Escrito el corpus oro (57 frases de consulta mexicana, sintéticas, etiquetadas A MANO antes de volver a mirar el código) y corrido contra el motor real: 15 fallos con la suite entera en verde. `meses?` es «mese» con una ese opcional y NUNCA casó «mes» — «hace un mes tuvo neumonía» invisible, un mes entero de historia clínica borrado por una `?` puesta por analogía con «años». Faltaban la pasiva («fue operado»), el infinitivo compuesto («refiere haber tenido»), «el año pasado», «a los N años» y «de la infancia». Y el falso positivo que importa: marcaba como pasado el PADECIMIENTO ACTUAL («hace tres días inició con fiebre y tiene neumonía»), que es la frase más frecuente de toda la consulta — el motivo de consulta siempre se dicta con cuánto lleva. Ahora el verbo de estado vence a la marca de tiempo pero NO al verbo en pasado, para no perder el titular del motor. «Refiere» NO cuenta como presente: es un verbo de decir, no de estado, y meterlo apagaría el interrogatorio dirigido entero. Gramática, no medicina: cero umbrales clínicos nuevos. VERIFICADO que puede ponerse rojo, revirtiendo las tres causas por separado.
