...
describe('E0-07 · el registro no puede MENTIR sobre el código (por MÉTODO y por ARGUMENTO)', () => {
  ...
  it('el ANALIZADOR no pasa por vacío (si se rompe, todo lo de arriba pasaría sin comprobar nada)', () => {
    ...
    // abierto.
    expect(llamadas.length).toBe(97)   // +1 el 2026-08-09: `paciente/paquete`, un POST con su `verificarCapacidad('firmar')`; +1 el 2026-08-04: `arco/oponerse`   // +1 el 2026-08-04: `superadmin/incidentes`   // +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
...
describe('E0-07 · propiedad heredada de E0-06, ahora expresada en capacidades', () => {
  ...
  it('el control de que la comprobación anterior NO pasa por vacío', () => {
    ...
    /* `paciente/paquete` (V9 POSTVISIT-001) entra a la lista: relee la nota
       FIRMADA para componer lo que el paciente va a leer. Es lectura de PHI
       clínico y va bajo `firmar` — médico y admin, ningún rol no clínico. */
  ...
  it('las rutas que tocan la IDENTIDAD del paciente están congeladas (segundo nivel de PHI)', () => {
    ...
    expect(conPaciente).toEqual([
      ...
      /**
       * V9 · POSTVISIT-001. Toca `patients` para dos cosas y ninguna más: leer
       * la nota FIRMADA de la que compone el paquete, y escribir el paquete
       * liberado bajo ese mismo paciente. Va con `firmar` —médico y admin—
       * porque liberar es aprobación clínica: el consultorio le dice al
       * paciente «esto es lo que quiero que leas».
       */
      'paciente/paquete',
      'portal',
