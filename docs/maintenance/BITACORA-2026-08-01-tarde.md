...
- **v1077** — REG-195. Verificando la rama de REG-192/193/194 con los motores reales antes de darla por buena: el caso del TITULAR seguía sin avisar, con el arreglo puesto y su golden en verde. `mencionSinDisculpa` juzgaba cada aparición con los 60 caracteres previos EN CRUDO, y la segunda mención —la del diagnóstico— tiene el «niega» de la LÍNEA DE ARRIBA dentro de esos 60: se recorrían todas las apariciones y se descartaban todas. Es palabra por palabra lo que `VENTANA_ATRAS` ya temía («una disculpa ajena taparía una afirmación real»). Ahora el contexto se recorta en el fin de apartado (punto, salto de línea, punto y coma, dos puntos; la coma NO, que es la que enumera lo negado). Con el recorte llega `NO_AFIRMA`, porque al dejar sola la segunda mención «glucosa para descartar diabetes» habría estrenado un falso positivo de alta frecuencia sobre un aviso que no se pliega. NINGÚN cambio clínico. VERIFICADO que puede ponerse rojo (7 de 15).
