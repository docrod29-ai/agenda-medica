          ...
          <div style={{ display: 'grid', gap: 6 }}>
            {factores.map(f => (
              <div key={f.uid} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 10, background: 'var(--s2)', borderRadius: 6 }}>
                ...
                <button onClick={() => remover(f.uid)} style={{ background: 'transparent', border: '1px solid color-mix(in srgb, var(--red) 30%, transparent)', color: 'var(--red)', borderRadius: 6, padding: '4px 10px', fontSize: 11.5, cursor: 'pointer' }}>
                  Quitar
                </button>
              </div>
            ))}
