...
describe('E0-07 · el registro no puede MENTIR sobre el código (por MÉTODO y por ARGUMENTO)', () => {
  ...
  it('el ANALIZADOR no pasa por vacío (si se rompe, todo lo de arriba pasaría sin comprobar nada)', () => {
    ...
    // abierto.
    expect(llamadas.length).toBe(97)   // +1 el 2026-08-09: `paciente/paquete`   // +1 el 2026-08-04: `arco/oponerse`   // +1 el 2026-08-04: `superadmin/incidentes`   // +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
    expect(rutasConGuardia).toBe(80)   // +1 el 2026-08-09: `paciente/paquete`   // +1 el 2026-08-04: `arco/oponerse`   // +1 el 2026-08-04: `superadmin/incidentes`   // +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
    ...
    expect(conVocabulario).toBe(54)   // +1 el 2026-08-09: `paciente/paquete`; +1 el 2026-08-04: `arco/oponerse`; +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
  })
...
describe('E0-07 · propiedad heredada de E0-06, ahora expresada en capacidades', () => {
  ...
  it('el control de que la comprobación anterior NO pasa por vacío', () => {
    ...
    // `paciente/paquete` entra el 2026-08-09: lee la nota FIRMADA —medicamentos y
    // estudios— para componer lo que el paciente va a leer. Está bajo
    // `clinico.escribir`, la misma capacidad con la que se escribe el expediente.
    expect(conPHI).toEqual(['arco/acceso', 'arco/cancelar', 'clinic/exportar', 'clinic/importar', 'expediente/exportar/[patientId]', 'fhir/paciente/[patientId]', 'hospital/mutar', 'paciente/paquete', 'portal', 'uci/estancia'])
  })
  ...
  it('las rutas que tocan la IDENTIDAD del paciente están congeladas (segundo nivel de PHI)', () => {
    ...
    expect(conPaciente).toEqual([
      ...
      'clinic/importar',
      /**
       * +1 el 2026-08-09 (REG-309). El cron de recordatorios acuña el token con
       * el que el paciente entra a la sala de SU videoconsulta, y para eso lee
       * `portalTokenVersion` — **un entero, y nada más**: ni nombre, ni
       * teléfono, ni una línea clínica.
       *
       * No lleva capacidad porque no tiene sesión: la protege `CRON_SECRET`, que
       * en producción **falla cerrado** (sin secreto configurado, 503). Y sólo
       * paga esa lectura cuando la cita es una teleconsulta, así que la agenda
       * presencial de un consultorio no toca la colección.
       */
      'cron/reminders',
      /**
      ...
      /**
       * Lee `proximoSeguimiento` del expediente del paciente para el bloque «su
       * próxima cita» del paquete, y nada más de su identidad: ni nombre, ni
       * teléfono. Va con `clinico.escribir` porque el resto de lo que lee —la
       * nota firmada— sí es secreto médico.
       */
      'paciente/paquete',
      'portal',
