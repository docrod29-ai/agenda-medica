...
describe('E0-07 · el escaneo encuentra rutas de verdad', () => {
  it('hay 81 rutas en disco (un guardián que no encuentra archivos pasa vacío y no protege nada)', () => {
    ...
    // 98 → 99 el 2026-08-09: `paciente/paquete` (V9 POSTVISIT-001) — el médico
    // libera lo que su paciente puede leer de la consulta. GET dice si ya se
    // entregó (`clinico.leer`); POST libera, y liberar es un acto de aprobación
    // clínica, así que pide `firmar`.
...
describe('E0-07 · el registro no puede MENTIR sobre el código (por MÉTODO y por ARGUMENTO)', () => {
  ...
  it('el ANALIZADOR no pasa por vacío (si se rompe, todo lo de arriba pasaría sin comprobar nada)', () => {
    ...
    // abierto.
    // 96 → 98 y 79 → 80 el 2026-08-09: `paciente/paquete` (V9 POSTVISIT-001)
    // entra con DOS métodos y DOS llamadas —una por handler—, que es el par
    // correcto: una ruta de dos métodos con un solo guardián significaría que
    // uno de los dos entra sin comprobar nada.
    ...
    // +2 el 2026-08-09: las DOS llamadas de `paciente/paquete` nacen hablando
    // capacidades (`clinico.leer` y `firmar`).
    expect(conVocabulario).toBe(55)   // +1 el 2026-08-04: `arco/oponerse`; +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
  })
...
describe('E0-07 · propiedad heredada de E0-06, ahora expresada en capacidades', () => {
  ...
  it('el control de que la comprobación anterior NO pasa por vacío', () => {
    ...
    // `paciente/paquete` entra a la lista: lee la nota FIRMADA del paciente para
    // componer lo que se le entrega, y sus notas anteriores para saber qué
    // medicación cambió. Es lectura de PHI clínico, y va bajo `firmar` (POST) y
    // `clinico.leer` (GET) — ningún rol no clínico las tiene.
  ...
  it('las rutas que tocan la IDENTIDAD del paciente están congeladas (segundo nivel de PHI)', () => {
    ...
    expect(conPaciente).toEqual([
      ...
      /**
       * Compone y libera el paquete de la visita. Toca `patients` porque todo
       * lo del paciente cuelga de ahí —su nota firmada, sus notas anteriores y
       * la subcolección donde queda el paquete—, y el `patientId` va en el
       * CAMINO: no se compara después, así que no se puede olvidar.
       */
      'paciente/paquete',
      'portal',
