...
| **P1 internos abiertos** | **0** (más 2 `BLOCKED_EXTERNAL`: P1-6, P1-14 — **P1-14 recalificado el 31-ago**: el despliegue ya se ejecutó; lo que falta es confirmar en la consola que los índices terminaron de construirse) |
...
| **P1-14** | `tareasVivas` sigue devolviendo **200 arbitrarias** de N. Sigue `BLOCKED_EXTERNAL` —el índice se crea fuera del repositorio— pero desde REG-352 **con el artefacto listo**: `firestore.indexes.json` lo declara y `docs/ops/INDICES-DE-FIRESTORE.md` reúne los cuatro módulos que hoy están peor por no tenerlos (worklist, lista de espera, citas del paciente, resumen de notas). Antes vivían en comentarios sueltos y nadie podía saber cuántos faltaban. **El despliegue YA SE EJECUTÓ** el 31-ago-2026, en las ejecuciones #11 y #12 del botón (`firestore:rules,firestore:indexes`, paso en `success`). Y esto **no lo cierra**, por una razón que no es burocrática: la construcción de un índice compuesto es **asíncrona** — `deploy` devuelve al ENVIAR, no al terminar, y un índice puede quedarse en `Building` durante horas o caer en `Error` mucho después del `success`. **Lo que falta ahora no es desplegar: es mirar la consola** del proyecto `nexomed-agenda` y comprobar que los índices de `firestore.indexes.json` figuran `Enabled`. Hasta entonces `tareasVivas` puede seguir devolviendo 200 arbitrarias sin que nada aquí lo note. |
