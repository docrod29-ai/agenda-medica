...
describe('E0-07 · el registro no puede MENTIR sobre el código (por MÉTODO y por ARGUMENTO)', () => {
  ...
  it('el ANALIZADOR no pasa por vacío (si se rompe, todo lo de arriba pasaría sin comprobar nada)', () => {
    ...
    expect(conVocabulario).toBe(54)   // +1 el 2026-08-09: `paciente/paquete` nace hablando capacidades   // +1 el 2026-08-04: `arco/oponerse`; +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
  })
...
describe('E0-07 · propiedad heredada de E0-06, ahora expresada en capacidades', () => {
  ...
  it('el control de que la comprobación anterior NO pasa por vacío', () => {
    ...
    // `paciente/paquete` entra el 2026-08-09: lee la nota FIRMADA para componer
    // lo que se le entrega al paciente. Es lectura de PHI clínico, bajo `firmar`.
  ...
  it('las rutas que tocan la IDENTIDAD del paciente están congeladas (segundo nivel de PHI)', () => {
    ...
    expect(conPaciente).toEqual([
      ...
      /**
       * Escribe bajo `patients/{id}/paquetes_visita` lo que el médico le
       * entrega al paciente de su consulta (V9 · POSTVISIT-001). Toca su
       * identidad por definición —el paquete cuelga de su expediente— y va con
       * `firmar`: liberar es un acto de aprobación con identidad profesional
       * detrás, no una acción de mostrador.
       */
      'paciente/paquete',
      'portal',
