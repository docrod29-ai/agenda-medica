...
describe('E0-07 · el registro no puede MENTIR sobre el código (por MÉTODO y por ARGUMENTO)', () => {
  ...
  it('el ANALIZADOR no pasa por vacío (si se rompe, todo lo de arriba pasaría sin comprobar nada)', () => {
    ...
    // abierto.
    expect(llamadas.length).toBe(97)   // +1 el 2026-08-04: `arco/oponerse`   // +1 el 2026-08-04: `superadmin/incidentes`   // +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`; +1 el 2026-08-10: `expediente/paquete-visita`
    expect(rutasConGuardia).toBe(80)   // +1 el 2026-08-04: `arco/oponerse`   // +1 el 2026-08-04: `superadmin/incidentes`   // +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`; +1 el 2026-08-10: `expediente/paquete-visita`
    ...
    expect(conVocabulario).toBe(54)   // +1 el 2026-08-04: `arco/oponerse`; +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`; +1 el 2026-08-10: `expediente/paquete-visita`
  })
  ...
  it('el avance se cuenta DEL REGISTRO, no de la prosa del expediente', () => {
    ...
    expect(resumenActivacion(METODOS_POR_RUTA)).toEqual({
      declarados: 63, activos: 36, pendientes: 27,   // +1 el 2026-08-04 `arco/oponerse`: nace ACTIVA (verificarCapacidad 'administrar', sin pendiente). +1 el 2026-08-03 `clinic/exportar-excel`: nace ACTIVA (verificarCapacidad, sin pendiente). +1 `calendar/ocupado` y +1 `seguridad/csp-estado`: los dos nacen ACTIVOS (verificarCapacidad, sin pendiente). +1 el 2026-08-10 `expediente/paquete-visita`: nace ACTIVA (verificarCapacidad 'clinico.escribir', sin pendiente)
    })
...
describe('E0-07 · propiedad heredada de E0-06, ahora expresada en capacidades', () => {
  ...
  it('el control de que la comprobación anterior NO pasa por vacío', () => {
    ...
    // `expediente/paquete-visita` entra a la lista: lee diagnósticos y
    // medicación de `notas` firmadas para componer el paquete de la visita
    // (POSTVISIT-001) — PHI clínico por definición.
  ...
  it('las rutas que tocan la IDENTIDAD del paciente están congeladas (segundo nivel de PHI)', () => {
    ...
    expect(conPaciente).toEqual([
      ...
      /**
       * POSTVISIT-001 — compone el paquete de la visita desde `patients/{id}`
       * (notas, config del consultorio) y lo persiste bajo el mismo paciente.
       * Va con `clinico.escribir` por lo mismo que el expediente completo: lee
       * diagnósticos y medicación de una nota firmada.
       */
