 ...
 */
const CACHE = 'nexusmed-v492'  // SEGURIDAD WHATSAPP: token del canal y firma del webhook. (1) El token permanente de Meta / api_key de 360dialog se guardaba EN CLARO en el doc raiz de la clinica, que cualquier miembro puede leer; con ese token se envian mensajes desde el numero de la clinica sin pasar por la app (una recepcionista, o un XSS en el panel, se lo lleva). El propio proyecto ya declaraba que el token nunca debe viajar en claro fuera del gestor de secretos y la implementacion lo contradecia. Ahora vive en clinics/{id}/secretos/whatsapp (regla read/write:false, solo Admin SDK); todos los lectores/escritores son server-side, migracion perezosa sin downtime al primer envio. (2) El webhook de Meta era FAIL-OPEN: sin META_APP_SECRET configurado devolvia firma-valida=true, asi que cualquiera con un phone_number_id inyectaba mensajes falsos (agendar citas espurias, disparar WhatsApp a costa de la clinica). Ahora fail-closed: se rechaza. REQUIERE que el Dr agregue META_APP_SECRET en Vercel.
