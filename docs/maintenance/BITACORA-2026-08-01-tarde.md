...
- **v1074** — REG-192. Encontrado leyendo los dos guardianes de arriba abajo para armar el corpus oro de temporalidad (EVAL-002): `contradicciones` y `desajustesTemporales` buscaban el padecimiento con `indexOf` a secas, o sea SÓLO su primera aparición, y evaluaban la disculpa («niega …», «antecedente de …») sobre ella y sólo sobre ella. Y la primera aparición es justo la del apartado de ANTECEDENTES, donde está bien escrito: **cuanto mejor redactada estaba la nota arriba, más ciego se quedaba el guardián abajo**. Un «niega hipertensión arterial» en antecedentes tapaba el «se continúa losartán por su hipertensión arterial» del plan; un «antecedente de neumonía en 2023» tapaba la «Impresión diagnóstica: neumonía» de hoy. Y la mención de abajo es la que se arrastra: `textoDeLaNota` pone el resumen primero y el DIAGNÓSTICO ESTRUCTURADO después — el que se copia a la receta y a la consulta siguiente (misma lección que `[object Object]`). Nuevo `mencion-en-la-nota.ts`: un solo buscador que recorre TODAS las apariciones, en orden de lectura, para los dos motores; la ventana de 60 deja de estar duplicada. NINGÚN criterio nuevo: la misma ventana y la misma expresión de disculpa, sólo que aplicadas todas las veces. MÉTODO: con diabetes el defecto se tapa por casualidad (el vocabulario trae «diabetes mellitus» y su primera aparición sí es la mala) — hubo que buscar hipertensión, donde la segunda mención repite la misma palabra, para reproducirlo de verdad. De paso, `descartar` faltaba en `NIEGA_EN_LINEA` (estaba `descarta`): «se solicita HbA1c para descartar diabetes» habría empezado a BLOQUEAR LA FIRMA al juzgar todas las menciones. VERIFICADO que puede ponerse rojo.
- **v1076** — REG-194. «No pues sí, desde hace años» se leía como NEGACIÓN: en el habla mexicana el «no» de arranque es muletilla y lo que sigue es un sí rotundo, pero `NEGATIVAS` ancla en `^no` y se quedaba con él. Verificado con el motor real: `condicionesNegadas('¿Padece diabetes? No pues sí, desde hace años.')` devolvía diabetes NEGADA, y de ahí `descartado` sobre una crónica que el paciente ACABABA DE AFIRMAR. Es la familia de «no sé» un escalón peor: allí se fabricaba una ausencia sobre una duda, aquí sobre una afirmación explícita. Una entrada más en `NO_ES_NEGACION`, que tiene UN solo consumidor — ningún vocabulario compartido se ensancha, que es la lección que dejó escrita la primera versión de REG-192. Declarado lo que cuesta: sin acentos «sí» y «si» se confunden, así que «No, si yo nunca he tenido nada» deja de contar como negación (se pierde un aviso; al revés se fabricaría el negativo). Comprobado en rojo: revertido el regex, caen 6 casos.
- **v1077** — REG-195 (hallazgo C2/C3, el último del plan de la auditoría de 9 dimensiones que quedaba abierto). «No padece diabetes» salía como ANTECEDENTE POSITIVO en `parser-clinico.ts` — que NO es el módulo de REG-193/194: aquéllos son el lado del dictado, éste es el motor que ESCRIBE LA NOTA cuando la IA devuelve 529. Reproducido con el motor real antes de tocar nada, y más ancho que la línea de la auditoría: «no padece», «no padezco», «no tengo», «no he tenido», «nunca he tenido», «jamás ha tenido», «tampoco tiene», «no sufre de», «no cuenta con», «no fuma» y «no es fumador» salían TODAS positivas. No es que no detectara la negación: `extraerComorbilidades` tiene dos casillas y lo que no cae en `negadas` cae en `positivas`. Importa porque un antecedente crónico inventado cambia el riesgo quirúrgico y SE ARRASTRA a todas las notas siguientes; y `tabaquismoActivo` no se queda en la prosa: entra en STOP-BANG y en Caprini, así que el paciente que dijo que no fuma puntuaba como fumador. DOS defectos más al reproducir: (1) el error CONTRARIO — «niega tabaquismo, padece diabetes mellitus» daba la diabetes por negada porque `padece` no cerraba la negación anterior; (2) «no fuma» — cuando el término clínico ES el verbo no queda nada entre el negador y el término. NO se comparte el vocabulario con `negaciones.ts`, y es a propósito: el dueño decidió separarlos el 7-ago (C-6) después de que el compartido fabricara una negación que el paciente nunca dijo. Tres consumidores, tres preguntas, tres vocabularios; aquí se ensancha SÓLO el que estaba corto. No es un comodín: «no ha mejorado su diabetes» la sigue afirmando. NINGUNA cifra clínica: es gramática del español. VERIFICADO que puede ponerse rojo: 18 de 26 casos fallan sin el arreglo. DECLARADO sin reparar: «no es diabético» sigue sin detectarse como TÉRMINO (`diabético` no está en COMORBILIDADES_DIC) — backlog VOC-001, emparenta con C-7.
...
| **1075** | **REG-193 — el lado del dictado leía la negación como se escribe, no como se contesta.** v1074 reparó la nota; el dictado se quedó igual. **Uno**: «pues no», «fíjese que no», «que yo sepa, no», «para nada» no contaban como negación — y la variante «que yo sepa no» estaba declarada, la rompía **la coma**. El paciente contesta que no y la nota le pone la crónica: el caso del 3-ago con una palabra delante. **Dos**: «no sé», «no me acuerdo», «no me han checado» sí contaban, y marcaban la condición `descartado` — que es una **afirmación de ausencia** puesta en boca de quien sólo dijo que no lo sabe; un paciente con diabetes sin diagnosticar contesta exactamente así. **Tres, el más caro**: el negador se aplicaba a la **frase entera**, así que «Niega tabaquismo, tiene diabetes en tratamiento» borraba una diabetes activa del panel de entidades. La reparación **no ensancha el vocabulario compartido** —eso ya fabricó una negación falsa en v1074, y el aviso está escrito encima del regex—: lo reutiliza tal cual y le cambia el **alcance**, anclándolo al término de al lado. El dictado usa ya el mismo `apariciones()` que la nota, así que la frontera de palabra de v1074 («plasma» no es «asma») vale para los dos. La enumeración hereda el negador de su cabeza («niega diabetes, hipertensión y dislipidemia») — **eso lo cazó el golden viejo, no yo**: con adyacencia estricta, la nota mejor escrita era la que disparaba el aviso. **Qué queda para el Dr.**: el vocabulario de negación de la NOTA —«negó», «sin X», «X (-)» siguen disparando el aviso contra una nota correcta— es C-6, y no lo amplío por mi cuenta porque es justo lo que costó el revert de v1074; y si «el paciente no sabe» debe verse en pantalla, C-8. |
