/**
 * GOLDEN — REG-599. Primero se normaliza la unidad; después se juzga el número.
 *
...
describe('EL DEFECTO QUE NINGÚN RANGO PODÍA CAZAR — PCR en mg/dL', () => {
  ...
  it('y ahora se convierte a 840 y AUN ASÍ se marca', () => {
    /**
     * REG-603 le puso conversión (mg/dL → mg/L es escala del SI, ×10). Así que
...
describe('EL ORDEN DEL §28 — normalizar, y DESPUÉS juzgar', () => {
  ...
  it('la vitamina D en nmol/L, igual, con el factor literal del §6', () => {
    ...
    // REG-603: el 2,496 ya no se cita, se REPRODUCE desde C27H44O2.
  ...
  it('el ORIGINAL nunca se pierde (§27.1)', () => {
    ...
    // REG-603: la fuente ya no es su documento, es el cálculo. Su 1,58 pasó a
    // ser el testigo que lo comprueba, y vive en el golden de REG-603.
...
describe('LO QUE NO SE INVENTA', () => {
  it('la glucosa en mmol/L YA se convierte — pero calculada, no tecleada (REG-603)', () => {
    /**
     ...
     *
     * REG-603 no fue a buscar el número: quitó la necesidad de tenerlo. El
  ...
  it('TODA conversión que existe dice de dónde sale su factor', () => {
     ...
     * conversiones venían de ahí. REG-603 las calcula, así que ahora la fuente
...
describe('AL REVÉS POR EL OTRO LADO — no se pasa de frenada', () => {
  ...
  it('la unidad ausente se sigue JUZGANDO como la canónica — pero ya se dice', () => {
    /**
     * REG-602 movió esto a medias, y las dos mitades importan:
     *
...
describe('EL DATO TIENE QUE LLEGAR — la pantalla enseña el estado', () => {
  ...
  it('el aviso ámbar se enciende con el ESTADO, no sólo con `noEvaluable`', () => {
     ...
     */
    /**
     * REG-602 le añadió una excepción: `MISSING_UNIT` NO pinta ámbar, porque es
  ...
  it('y el motivo es TEXTO VISIBLE, no un `title` (REG-581)', () => {
    /**
