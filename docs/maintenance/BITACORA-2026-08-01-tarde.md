...
- **(sin versión, número por asignar)** — hallazgos C2+C3 de la auditoría. NO se reclama v1074: hay 14 PRs abiertos que ya se llaman así. Los TRES motores que leen el dictado para decidir si un antecedente está negado usaban un vocabulario escrito mirando la nota redactada («niega diabetes»), no el habla del consultorio. Fallaba en LAS DOS DIRECCIONES: «No padece diabetes.» entraba al parser local y salía **«Antecedentes: Diabetes mellitus tipo 2.»** —`padece`, `sufre` y `negó` no estaban, y `negó` fallaba doble porque `\b` de JavaScript no considera letra a la «ó»—; y al revés, «¿Tiene diabetes? NO SÉ, doctor.» contaba como negación y degradaba a `descartado` una diabetes confirmada. En medio, cómo se contesta de verdad: «Pues no», «Fíjese que no», «Mmm, no», «Este… no» no contaban como respuesta negativa. Reparado en las tres rutas (parser-clinico, negaciones, guardian-sustituciones), no en una — lección de REG-180/187. «Qué va» se dejó FUERA a propósito: puede empezar una frase y aquí un falso positivo descarta un antecedente real. LO QUE NO HACE: con «no sé» deja de afirmar que se negó y ahí se detiene; la condición cosechada de la PREGUNTA conserva su certeza. La tercera salida —«el paciente dice que no lo sabe», en pantalla— queda abierta como SAFE-005. SE SOLAPA con el PR #245, que repara mejor la mitad de esto (saca el vocabulario a un `negadores.ts` compartido); lo propio de aquí es el «no sé», la frontera de palabra con tilde y la ventana sin normalizar. VERIFICADO que puede ponerse rojo: 17 de 20 casos fallan sin el arreglo; los 3 verdes son los controles.
