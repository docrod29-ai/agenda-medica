 ...
 */
const CACHE = 'nexusmed-v525'  // COBROS endurecidos (auditoria adversarial de mi propio trabajo de la noche): (1) ABONO parcial ya NO marca la cita como saldada -> no desaparece del worklist con saldo pendiente. (2) No se puede cobrar una cita en CORTESIA (guard simetrico) ni exentar una ya cobrada. (3) REGLAS: marcar cortesia (cobroExento false->true) EXIGE exentoPor==uid + motivo (anti-fraude, el borde real; la validacion del cliente no basta). (4) cancelarCobro ahora es TRANSACCIONAL (antes el 2o write con catch podia dejar la cita sin liberar). (5) cobro a una cita BORRADA ya no se pierde (tx.update NOT_FOUND) — se registra igual. (6) 'Quitar cortesia' ahora tiene UI (distintivo clicable) — era reversible pero sin boton. (7) cobrar desde Citas ya pasa estadoActual (no retrocede finalizada->atendida). (8) colision de cama en traslado usa el servicio destino con fallback al actual. Superadmin: bypass de trial y override de modulos DOCUMENTADOS (logica de negocio, requieren decision del Dr).
