...
export const REQUISITOS: readonly Requisito[] = Object.freeze([
  ...
  R({
    ...
    estado: 'PROVEN',
    evidencia: 'REG-407 lo cerró en la consulta: selector de los CUATRO tipos en cada fila, con etiqueta accesible, bloqueado en nota firmada, y elegirlo marca tipoOrigen: medico — la única vía por la que un diagnóstico pasa a estar firmado por una persona. REG-524 corrigió el censo: esta entrada pedía «la misma elección en las otras superficies» y contra el árbol no se sostenía —hospital y UCI no tienen Diagnostico[] sino una cadena libre, y el expediente enseña notas ya FIRMADAS, donde un selector sería una segunda puerta de escritura—. Construirlo habría sido construir el defecto con el censo dando la orden. De paso apareció lo que sí estaba roto: la receta y la orden elegían el principal con `find(definitivo) ?? dxs[0]`, y ese respaldo no mira tipo, así que un «embarazo descartado» salía impreso como el motivo de la receta; ahora hay una sola puerta (diagnosticoQueSeImprime) que usa estaVigente y no rellena nada si nada califica. REG-547 cerró las DOS preguntas que quedaban, las dos del médico dueño: (D-034) firmar con diagnósticos tipados por el dictado AVISA y NO BLOQUEA —se descartaron bloquear la firma y bloquear sólo el que se imprime—, con guardián nuevo que cae si alguien lo convierte en bloqueo o si quita el aviso; y (D-035) `diagnosticoIngreso` de hospital/UCI SIGUE siendo cadena libre, DIFERIDO mientras Hospital sea ALPHA y no esté a la venta.',
  ...
  R({
    ...
    estado: 'PROVEN',
    evidencia: 'CORRECCIÓN DEL CENSO (REG-403): tres de los cuatro campos que esta entrada pedía YA EXISTÍAN, en el sitio correcto y con otro nombre — `revisado` es `estado: cerrada`, `revisadoPor` es `cerradaPor` y `revisadoEn` es `cerradaEn`. Ponerlos en el panel es lo que la arquitectura prohíbe y `laboratorio/firestore.ts` lo tiene escrito bajo el título «DÓNDE VIVE REVISADO»: crearía una segunda fuente de verdad del mismo hecho. Construirlos habría sido construir el defecto con el censo dando la orden. El cuarto sí faltaba y se cerró: nada registraba que un valor crítico se hubiera comunicado, así que «lo vi» y «localicé a alguien» eran el mismo gesto. REG-548 cerró las DOS preguntas que quedaban, las dos del médico dueño: (D-036) un crítico NO vence —se ofrecieron 1 h, 4 h, 24 h y ninguno—, así que la pregunta al cerrar es la única defensa y hay guardián que cae si alguien la quita; y (D-037) cuentan las cuatro vías, incluido un mensaje enviado, advertido de que puede morir sin acuse (REG-535/438). Se guarda CUÁL fue, en un campo que pregunta una sola cosa —de qué manera consta— para no repetir REG-521, y el progreso del resultado lo distingue: «(hablado)» / «(por mensaje)». El trinquete de conexión cazó que el lector quedaba sin llamador y se conectó ahí.',
  ...
  R({
    ...
    estado: 'PARTIAL',
    queFalta: 'REG-391 puso el interruptor y REG-397 el instrumento (el vigilante cuenta pausadas y rendidas y las distingue). REG-535 cerró la pantalla del dead-letter: contar era el primer paso y no el trabajo — con un número no se ve de qué paciente era, ni qué decía, ni por qué murió, ni se puede reintentar. Ahora se listan y se pueden devolver a la cola, con la MISMA puerta que /entregas (`mensajeria.enviar`) y avisando ANTES del botón de que reintentar puede DUPLICAR el mensaje: desde la cola no se distingue si no llegó nunca o si llegó y se perdió el acuse. Los intentos se reinician y las pausas se conservan, porque cuentan cosas distintas. CORRECCIÓN DEL CENSO (REG-541): esta entrada decía que el mensaje REACTIVO del bot «se pierde y no queda en ninguna cola». Que no se encole es una DECISIÓN argumentada —reintentar fuera de la ventana de 24 h exige plantilla aprobada en Meta, trámite del dueño— y sí quedaba constancia: `registrarNoEntregado` escribe desde el helper `send()`, que cubre sus 36 llamadas. El defecto real era peor: `whatsapp_no_entregados` tenía UN ESCRITOR Y CERO LECTORES. Declarada en los tres sitios, respaldada, cerrada al cliente… e invisible, mientras la cabecera del módulo prometía que «un fallo registrado se puede VER». Y REG-535 había puesto una pantalla llamada «No entregados» que, con la cola limpia y el bot fallando, afirmaba que no se había perdido nada. Cerrado: lector con tope que LANZA en vez de devolver [], las dos listas por la misma ruta y la misma capacidad, los del bot SIN botón (no se pueden reintentar, y prometerlo haría que el médico dejara de llamar), y «ninguno» ahora exige que las dos estén vacías. FALTA: (1) encolar y reintentar de verdad, que espera la plantilla aprobada del dueño; (2) una clave de idempotencia de punta a punta con el proveedor, que es la única defensa REAL contra el duplicado (hoy la defensa es que lo decida una persona informada); (3) archivar o descartar lo que el médico decida no reintentar.',
