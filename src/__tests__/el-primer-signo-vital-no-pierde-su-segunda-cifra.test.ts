 ...
 * Tres cambios, porque el defecto tenía tres patas:
 *   1. `vacio` se deriva de `hayAlgoQuePerder` (la lista canónica), así que las
 *      dos preguntas ya no pueden discrepar — es lo que mide esta prueba.
 *   2. La restauración marca `autoRestRef` también cuando NO hay respaldo, y
 *      nunca repone un espejo escrito por su propio montaje (`espejoPropioRef`).
 *   3. `useGrabacionVoz` devuelve un objeto memoizado, así que el efecto deja de
 *      correr en cada commit.
 * Las patas 2 y 3 no las mide esta prueba (ver «qué no cubre»): las midió el
 * equipo rojo en vivo y su regresión sería visible en el mismo tecleo.
 *
 * ── QUÉ NO CUBRE ─────────────────────────────────────────────────────────────
