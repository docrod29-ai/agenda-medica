...
const MODELOS = (process.env.ASR_MODELOS_DIARIZACION || 'universal-3-5-pro,universal-2').split(',')
...
async function diarizar(d: Dialogo): Promise<Utterance[]> {
  ...
  const sub = await fetch(`${AAI}/transcript`, {
    ...
    body: JSON.stringify({
      ...
      // Igual que producción tras la v1032: el proveedor RETIRÓ `speech_model`
      // y devuelve 400 con cualquier valor, incluido el alias «best».
      speech_models: MODELOS,
      speaker_labels: true,
