...
/**
 * Un escudo que no cabe en una expresión regular.
 *
 * Las negaciones tienen dos marcas: la que niega la frase entera («niega
 * diabetes») y la que niega **un término pegado a ella** («no es diabético»).
 * La segunda no se puede escribir como un regex suelto sin dejar que un «no es»
 * ajeno —el de «no es fumador, tiene diabetes»— tape una afirmación real, así
 * que se decide con código. Se recibe el texto previo **ya plegado**, igual que
 * lo recibiría el regex.
 */
export type Escudo = RegExp | ((antesPlegado: string) => boolean)

const escuda = (escudo: Escudo, antesPlegado: string) =>
  typeof escudo === 'function' ? escudo(antesPlegado) : escudo.test(antesPlegado)
 ...
 *   avisar. Si es un regex, sin bandera `g`: se evalúa muchas veces y
 *   `lastIndex` haría que fallara una de cada dos.
...
export function primeraMencionSinEscudo(
  ...
  for (const idx of [...apariciones].sort((a, b) => a - b)) {
    ...
    if (escuda(escudo, sinAcentos(antes))) continue
