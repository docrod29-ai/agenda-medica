 ...
 */
const CACHE = 'nexusmed-v462'  // AUDITORIA MODULO 0, LOTE 2. (1) Nunca mas pantalla en blanco sin salida: si Firestore no responde, ClinicContext ahora PROPAGA el error en vez de tirarlo, y el layout muestra que paso con botones Reintentar y Cerrar sesion (antes era return null: ni spinner ni error ni forma de salir, e indistinguible de no tener consultorio). (2) La receta ya NO se imprime con formato por defecto en silencio: si falla la lectura de config, salia sin membrete, sin firma y sin cedula (invalida para NOM-004) sin avisar; ahora aviso visible y impresion/descarga bloqueadas en receta, orden, nota y referencia. (3) Cobertura de tests en las 3 superficies de seguridad que estaban al 0%: auth-server, superadmin y rate-limit.
