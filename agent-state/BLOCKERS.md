...
| B-12 | **`axe` y el barrido visual sobre la aplicación corriendo** (V9 · A11Y-GATE-001, NAV-NAVEGADOR-001) | Las variables de Firebase en este entorno. `npm run build` compila y luego falla al recolectar datos de página con `auth/invalid-api-key`: el contenedor no las tiene, así que la aplicación no se puede levantar ni recorrer con un navegador | Lo que se puede leer del código está cubierto (cero controles sin nombre en la superficie del paciente, trinquete de deuda de estilo). **Contraste real en pantalla, orden y trampa de foco, objetivo táctil de 44×44 y las seis comprobaciones de `NAV-NAVEGADOR-001` siguen sin medir** — y dos de esas seis pueden convertir un P2 en P0 |
