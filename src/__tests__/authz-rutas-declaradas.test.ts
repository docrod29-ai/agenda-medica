...
describe('E0-07 · el registro no puede MENTIR sobre el código (por MÉTODO y por ARGUMENTO)', () => {
  ...
  it('el ANALIZADOR no pasa por vacío (si se rompe, todo lo de arriba pasaría sin comprobar nada)', () => {
    ...
    // abierto.
    expect(llamadas.length).toBe(100)   //   // +1 el 2026-09-06 (ASE-009, Panel de Lujo): `pacientes/fundir` — juntar dos expedientes repetidos no existía en el producto y el único camino era fingir una solicitud ARCO de cancelación; nace ACTIVA (verificarCapacidad 'administrar', sin pendiente).  // +1 el 2026-09-05 (REG-523): `expediente/pregunta-atendida` (verificarCapacidad 'clinico.escribir', una llamada, un POST);   // +2 el 2026-08-27 (POSTVISIT-001): `expediente/paquete-de-visita` (verificarCapacidad 'firmar') y la rama `clinico` de `portal/link`, que cobra `firmar` para emitir un enlace con secreto médico dentro;   // +1 el 2026-08-04: `arco/oponerse`   // +1 el 2026-08-04: `superadmin/incidentes`   // +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
    expect(rutasConGuardia).toBe(82)   // +1 el 2026-09-06 (ASE-009): `pacientes/fundir`   // +1 el 2026-09-05 (REG-523): `expediente/pregunta-atendida`   // +1 el 2026-08-27: `expediente/paquete-de-visita`   // +1 el 2026-08-04: `arco/oponerse`   // +1 el 2026-08-04: `superadmin/incidentes`   // +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
    ...
    expect(conVocabulario).toBe(57)   // +1 el 2026-09-06 (ASE-009): `pacientes/fundir` ('administrar' como literal)   // +1 el 2026-09-05 (REG-523): `expediente/pregunta-atendida` ('clinico.escribir' como literal)   // +2 el 2026-08-27 (POSTVISIT-001): `expediente/paquete-de-visita` y la rama `clinico` de `portal/link`   // +1 el 2026-08-04: `arco/oponerse`; +1 el 2026-08-03: `clinic/exportar-excel`; +1 el 2026-08-02: `calendar/ocupado`; +1 `seguridad/csp-estado`
  })
  ...
  it('el avance se cuenta DEL REGISTRO, no de la prosa del expediente', () => {
    ...
    expect(resumenActivacion(METODOS_POR_RUTA)).toEqual({
      declarados: 65, activos: 38, pendientes: 27,   //   // +1 el 2026-09-06 (ASE-009, Panel de Lujo): `pacientes/fundir` — juntar dos expedientes repetidos no existía en el producto y el único camino era fingir una solicitud ARCO de cancelación; nace ACTIVA (verificarCapacidad 'administrar', sin pendiente).  // +1 el 2026-09-05 (REG-523) `expediente/pregunta-atendida`: nace ACTIVA (verificarCapacidad 'clinico.escribir', sin pendiente). +1 el 2026-08-27 `expediente/paquete-de-visita`: nace ACTIVA (verificarCapacidad 'firmar', sin pendiente). +1 el 2026-08-04 `arco/oponerse`: nace ACTIVA (verificarCapacidad 'administrar', sin pendiente). +1 el 2026-08-03 `clinic/exportar-excel`: nace ACTIVA (verificarCapacidad, sin pendiente). +1 `calendar/ocupado` y +1 `seguridad/csp-estado`: los dos nacen ACTIVOS (verificarCapacidad, sin pendiente)
    })
...
describe('E0-07 · propiedad heredada de E0-06, ahora expresada en capacidades', () => {
  ...
  it('el control de que la comprobación anterior NO pasa por vacío', () => {
    ...
    // +1 el 2026-08-27 (POSTVISIT-001): `expediente/paquete-de-visita` lee la nota
    ...
    // +1 el 2026-09-06 (ASE-009): `pacientes/fundir` MUEVE las notas del
    // expediente absorbido al superviviente. Es lectura y escritura de PHI
    // clínico —la nota firmada entera, copiada verbatim para que su hash siga
    // valiendo— y está bajo `administrar`, como `arco/cancelar`, que es su
    // vecina más cercana: la que decide si un expediente se suprime.
    expect(conPHI).toEqual(['arco/acceso', 'arco/cancelar', 'clinic/exportar', 'clinic/importar', 'expediente/exportar/[patientId]', 'expediente/paquete-de-visita', 'fhir/paciente/[patientId]', 'hospital/mutar', 'pacientes/fundir', 'portal', 'uci/estancia'])
  })
  ...
  it('las rutas que tocan la IDENTIDAD del paciente están congeladas (segundo nivel de PHI)', () => {
    ...
    expect(conPaciente).toEqual([
      ...
      /**
       * +1 el 2026-08-27 (POSTVISIT-001 · REG-335). Lee `patients/{id}` para
       * sacar las ALERGIAS canónicas con `alergiasParaImpreso` —la misma
       * primitiva del impreso del médico— y las congela en el paquete que el
       * paciente va a leer. Toca identidad y toca lo clínico, y por eso va con
       * `firmar`: es la ruta que decide qué se le enseña al paciente como
       * aprobado por su médico.
       *
       * No baja el listón: quien puede llamarla ya puede sellar la nota de la
       * que este paquete se compone.
       */
      'expediente/paquete-de-visita',
      /**
       * +1 el 2026-09-05 (REG-523). Escribe UN campo —`atendidaEn`— en
       * `patients/{id}/preguntas_paciente/{preguntaId}`, y lo hace desde el
       * servidor porque la colección está cerrada al navegador a propósito.
       * Bajo `clinico.escribir`: quien puede cerrar la tarea clínica puede
       * dar por atendida la pregunta que la abrió; el mostrador no.
       */
      'expediente/pregunta-atendida',
       ...
       * sobrevive y cuál queda marcado— y mueve todo lo clínico que cuelga del
       * absorbido. Va bajo `administrar`, como las tres de ARCO: juntar dos
       * historias, o juntar por error las de dos personas distintas, es una
       * decisión del responsable del expediente y no del mostrador.
       *
       * No baja el listón: quien puede llamarla ya puede SUPRIMIR un expediente
       * entero por `arco/cancelar`, que es estrictamente más destructivo. Y
       * ésta no borra nada — el absorbido queda marcado `fusionadoEn`.
       */
      'pacientes/fundir',
      /**
      ...
      'public/booking',
      /**
       * REG-519: la tercera consumidora del magic-link, y la que quedó fuera
