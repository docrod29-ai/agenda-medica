...
## REG-270 — «no es diabético» no era una negación, y «negó» tampoco (v1151)
...
mexicana. De doce, siete no se reconocían («pues no», «fíjese que no», «para
nada», «qué va», «tampoco», «nel», «no es diabético»), y una se reconocía sin
deber: «¿Tiene diabetes? No sé» salía como **negada**, y
`corregirCertezaPorNegacion` bajaba la condición a `descartado`.

**LO QUE ESTA ENTRADA YA NO RECLAMA** — Entre que esto se escribió y que se pudo
fusionar, `main` cerró su propio REG-192 sobre el mismo módulo y **llegó más
lejos en la mitad de la respuesta**: `RUIDO_ANTES_DE_LA_RESPUESTA` quita marcas
de turno («—», «Paciente:») además de muletillas; `NO_ES_NEGACION` defiende el
«no sé»; `NEGATIVAS_SIN_NO` cubre «tampoco», «para nada», «qué va», «en
absoluto»; y `NIEGA_POSPUESTO` caza «Diabetes no.», que aquí ni se había visto.

Así que **C3 es de `main`, no de esta rama**, y su implementación es la que
queda: se descartó la de aquí en vez de imponerla, porque imponerla habría hecho
perder la marca de turno y la negación pospuesta. Escribirlo importa: dos
reparaciones del mismo defecto compitiendo terminan con la peor ganando por
llegar más tarde.

**LO QUE SÍ APORTA, Y NO ESTABA EN `main`** — todo del lado de la frase, no de la
respuesta:

1. **`NIEGA_EL_TERMINO` — «no es diabético», «nunca ha tenido asma».**
   `NIEGA_EN_LINEA` sólo entendía «no tiene». No se pueden meter ahí sin más
   porque esas marcas se buscan en **cualquier** posición de la frase, y en «no
   es fumador, tiene diabetes de diez años» el «no es» habla del tabaco: leerlo
   como negación **borraría un antecedente real**, que es el daño en la
   dirección cara. Por eso exigen **adyacencia** — entre la marca y el término
   sólo cabe la enumeración que la propia marca niega («no es diabético ni
   hipertenso»)— y se deciden en `niegaAlTermino`, con un bucle y no con una
   alternación repetida, que es como se escribe un regex que se cuelga.
2. **«negó» no cazaba.** `nieg[ao]` no cubre «negó» —la «ó» no es «o»— y la
   frase se leía como una afirmación.
3. **El escudo de `contradicciones` pasó a ser doble** (`ESCUDO_DE_LA_NEGACION`),
   para que «la nota dice no es diabético» también silencie el aviso. Como eso
   no cabe en un regex, el parámetro `escudo` de `mencion-en-la-nota.ts` admite
   ahora también una función (`Escudo`); la temporalidad sigue pasando el suyo
   sin cambiar una línea.
4. Dos guardas de precisión: «nada más» es «sólo» y no niega, y «nel».

**EL `\b` QUE MUERDE DOS VECES** — `main` documentó que en JavaScript `\w` es
ASCII, así que tras una vocal acentuada **no hay límite de palabra**. Al añadir
`neg[oó]` aquí se cayó en esa misma trampa sin verla: el `\b` que cierra el
grupo entero de `NIEGA_EN_LINEA` hacía que «Negó diabetes» siguiera sin cazar
**aunque la alternativa estuviera escrita**. Se descubrió porque el golden se
puso rojo, no leyendo el regex. El cierre es ahora `(?![a-záéíóúñ])`, que además
sigue impidiendo que «negociación» cuente como una negación.

**Por qué le importa a un paciente** — El interrogatorio se dicta **nombrando**
las enfermedades en la pregunta y el extractor las cosecha; este motor es la
única defensa determinista contra que el paciente que dijo que no salga con
crónicas que no tiene, y el antecedente se arrastra a todas las notas
siguientes. En sentido contrario, tratar «no es fumador, tiene diabetes» como
negación borraría una diabetes real.

**Ninguna cifra clínica, ningún umbral** — Es vocabulario, no criterio: la lista
de crónicas no cambió y sigue valiendo que lo que no está en ella no se vigila.

**Qué NO hace** — No detecta la duda **como** duda: «¿Tiene diabetes? No sé»
deja de negar, pero nadie avisa de que quedó una pregunta abierta, y si la nota
afirma la diabetes a partir de ese «no sé» este motor no lo ve. Hace falta un
canal de «declarado incierto» que hoy no existe. Anotado como **NEG-002** en
`agent-state/BACKLOG.json`, con la decisión que lleva dentro —si el aviso
bloquea la firma o sólo se declara— marcada como del médico dueño.

**Qué queda para el médico** — El motor sólo afirma que dictado y nota se
contradicen. Un paciente puede negar una diabetes que sí tiene documentada;
cuál de las dos vale es una decisión clínica suya.

**Comprobado que puede ponerse rojo** — Devolviendo el escudo a una sola marca,
tres casos del golden se ponen rojos; quitando `NIEGA_EL_TERMINO`, los de la
negación pegada al término. Los casos de la costura llevan las dos menciones
deliberadamente **separadas**: la cita que se le enseña al médico abarca 100
caracteres y, con las menciones pegadas, la prueba pasaba sin distinguir nada.
Ocurrió al escribirla y por eso se dice aquí.

**Golden** — `src/__tests__/el-no-de-la-consulta-no-es-el-del-formulario.test.ts`
(22 casos declarados, 34 ejecutados).

**Renumerado dos veces** — Fue REG-192/v1074 y después REG-216/v1098; `main`
fusionó un REG-192 y luego un REG-216 distintos mientras esto esperaba. Sólo
cambian números.
