 ...
 */
const CACHE = 'nexusmed-v515'  // AUDITORIA multi-agente (agenda + whatsapp + impresion). FIXES: (1) AGENDA: el modal generaba huecos con el horario del MEDICO pero validaba el conflicto y el servidor con el horario de la CLINICA -> citas validas rechazadas cuando el doctor trabaja distinto a la clinica; ahora ambos usan el horario del medico (AppointmentModal.tsx + api/appointments carga el doc del doctor). (2) AGENDA: el recordatorio de MISMO DIA nunca llegaba a citas confirmadas (al mandar el de 24h pasaban a 'recordatorio-enviado' y salian de la query); ahora ese estado se incluye. (3) WHATSAPP: la respuesta 'SI' a una oferta de lista de espera se perdia en silencio porque las sesiones del bot usaban claves de telefono distintas (webhook con wa_id 521..., waitlist con digitos sin lada); ahora TODAS las sesiones se indexan por telefono normalizado (52 + 10 digitos) como id del doc -> convergen y el hueco se agenda. (4) IMPRESION: Word sin cedula ya marca [FALTA CEDULA PROFESIONAL]; carta de referencia ya no inventa 'Chihuahua, Chih.'; recetaConfig se recalcula al llegar unicoMedico. Tests 1265.
