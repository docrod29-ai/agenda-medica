...
import { buscarEvidencia, buscarEvidenciaMulti, type ArticuloPubMed } from '@/lib/evidencia/pubmed'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const fuentes = articulos.map((a, i) => `[${i + 1}] ${a.tipo ? `[${a.tipo}] ` : ''}${a.revista} ${a.anio} · PMID ${a.pmid}\n${a.titulo}\n${a.resumen.slice(0, 1200)}`).join('\n\n')
