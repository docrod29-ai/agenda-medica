...
- **v1074** — REG-192 (EVAL-002: el corpus oro que este motor nunca tuvo). `desajustesTemporales` buscaba cada padecimiento en la nota con un `indexOf` suelto, UNA sola vez: como la nota le llega en texto plano (`resumen` + diagnósticos + secciones pegados con `\n`), **la primera mención decidía por el documento entero**. Si esa primera venía bien escrita como antecedente, la comprobación se daba por satisfecha. Y ése es el orden real de NOM-004 —«Antecedentes personales patológicos» va antes que «Plan de tratamiento»—, así que **cuanto MEJOR estructurada estaba la nota, menos vigilada quedaba**: una neumonía correctamente archivada como antecedente de 2019 tapaba el levofloxacino que la nota recetaba hoy por esa misma neumonía. REPRODUCIDO con el motor real: la misma nota con dos líneas intercambiadas avisaba en un orden y callaba en el otro. Ahora se recorren TODAS las apariciones. NINGÚN umbral nuevo (ventana de 60 y vocabulario, los de antes). El golden es además el corpus oro que EVAL-002 pedía, y deja DECLARADAS y medidas las 6 formas verbales sin cobertura («se recuperó de», «cursó con», «fue operado de»…) como trinquete que sólo puede bajar — no se añaden aquí porque cada forma nueva ensancha el riesgo de avisar de más. Queda TEMP-001: la frase compuesta («tuvo neumonía hace tres años y sigue con diabetes») pierde las dos porque un PRESENTE veta la oración entera.
