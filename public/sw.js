 ...
 */
const CACHE = 'nexusmed-v474'  // MODULO 3, LOTE 2 — cierre. (1) RESERVAS DE PAGINACION AL ALZA tras medir el render real: la hoja 1 con membrete subido reservaba 52mm contra ~69 reales, y el pie 26mm contra ~30 mas el aviso legal. Quedarse corto no da una hoja fea: el bloque de firma se coloca con marginTop:auto dentro de una hoja con overflow:hidden, asi que sin espacio libre TERMINA FUERA DE LA HOJA — receta impresa sin firma ni cedula. Ademas el formato propio firma CADA hoja y solo se reservaba en la ultima. (2) FOLIO ESTABLE derivado de la nota: era Date.now() y se reinventaba en cada carga, asi que reimprimir daba OTRO folio y dos papeles del mismo acto circulaban con identificadores distintos. (3) Sin cedula ya no imprime 'Cedula Prof. —' (parece un guion de maquetacion, no la ausencia de un dato obligatorio) y se avisa antes. (4) El respaldo de impresion cuando la ventana emergente esta bloqueada imprimia la receta al 40% y recortada; ahora se avisa y no se imprime nada. (5) El membrete del Word con URL relativa salia roto. 8 tests nuevos del invariante 'ningun medicamento se pierde'.
