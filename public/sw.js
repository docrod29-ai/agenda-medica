 ...
 */
const CACHE = 'nexusmed-v511'  // AUDITORIA CONSULTA - LOTE 2 (arreglos seguros). (1) GUARDIAN DE 1MB TAMBIEN AL CREAR LA NOTA: estaba solo en updateNota; una nota ya grande en su PRIMERA escritura (dictado largo + diarizado + entidades) fallaba con el error crudo de Firestore. Ahora avisa claro y el respaldo local conserva todo. (2) HUELLA DE AUDITORIA en Word y PDF de la ORDEN medica (antes solo Imprimir dejaba rastro): una orden entregada como Word/PDF no quedaba en bitacora — se puede responder 'que decia la orden del dia X'. Pendiente de lote 3 (documentado, requiere sesion para probar): streaming de la nota, paralelizar el ensamble Opus+GPT en Maxima, identidad multi-medico en el impreso, persistir rolesHablante.
