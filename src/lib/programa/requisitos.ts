...
export const REQUISITOS: readonly Requisito[] = Object.freeze([
  ...
  R({
    ...
    estado: 'PARTIAL',
    queFalta: 'REG-426 cerró la mitad que faltaba: los otros DOS programas en vuelo ya no son invisibles. El tablero de Ausculta custodia 78 requisitos y está bien vigilado, pero custodiaba UN programa de TRES — V9 (experiencia del paciente) y V10 (excelencia visual) viven en agent-state/ y el tablero no los mencionaba, así que «quedan N accionables» era cierto del censo y falso del producto. Ahora se cuentan en docs/product/PROGRAMAS-EN-VUELO.md, con guardián. NO se fusionan a propósito: V10 es el carril de Product Excellence y el §20 del directivo prohíbe rehacer su trabajo. Al reconciliar V9 se midieron sus 10 abiertos UNO POR UNO contra el árbol y CINCO ya estaban hechos sin marcar. FALTA: la prosa de cada WS sigue a mano y puede envejecer (lo que ya no puede es el estado ni el conteo de los tres programas).',
    artefactos: [
