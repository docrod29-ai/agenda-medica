...
| RTC-18 | P2 | RT-15 + medición 14-ago | **CONFIRMADO CON NÚMEROS.** `ClinicalSpine` con silueta de filtro: el elemento longitudinal de §7 se rinde como fila de píldoras igual a los filtros de /pacientes; corta a media palabra sin affordance de scroll. La medición de píldoras encontró que **`/expediente` es el único outlier del producto**: 8 píldoras en TRES filas y 270px del primer pliegue (5 sin dato), mientras cuatro de seis superficies tienen CERO y la fila de `/pacientes` (3, con conteos) informa. Las tres filas hacen tres trabajos distintos —riel del Spine, filtro de la historia, chips de diagnósticos— vestidos igual | Forma propia para el Spine (indicador de posición, fade de corte), NO borrar píldoras: la medición dice que el problema es la silueta compartida, no la cantidad. Acta: `docs/design/capturas/v15-pildoras/medicion.json` |
