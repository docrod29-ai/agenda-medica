...
{
    dataClass: 'team-chat-display-names',
    sourcePath: 'members',
    systemOfRecord: 'firestore', mutable: true, signedOrImmutable: false,
    restoreAllowed: 'libre', retentionClass: 'configuracion', containsPHI: false,
    referenceDependencies: [],
    integrityInvariant: 'El nombre que cada miembro del equipo eligió para el chat interno. No es la membresía —ésa es `clinic_members`, de nivel raíz— sino cómo se le ve en el chat: perderlo deja conversaciones firmadas por identificadores.',
  },
  {
    dataClass: 'physician-working-memory',
    sourcePath: 'memoria_medico',
    systemOfRecord: 'firestore', mutable: true, signedOrImmutable: false,
    restoreAllowed: 'solo-si-falta', retentionClass: 'clinica', containsPHI: true,
    referenceDependencies: ['encounter-signed-note', 'patient-demographics'],
    integrityInvariant: 'Es una PROYECCIÓN de las notas firmadas, no una fuente. Por eso `solo-si-falta`: un resumen de ayer escrito encima del de hoy le enseña al médico un contexto clínico que ya no es el del paciente, y lo hace con la autoridad de algo que el sistema afirma. Si falta, se vuelve a derivar de las notas.',
  },
  {
    dataClass: 'whatsapp-outbox',
    sourcePath: 'whatsapp_outbox',
    systemOfRecord: 'firestore', mutable: true, signedOrImmutable: false,
    restoreAllowed: 'nunca', retentionClass: 'operativa', containsPHI: true,
    referenceDependencies: ['patient-demographics'],
    integrityInvariant: 'Mensajes PENDIENTES DE ENVIAR. Es la única clase de este inventario cuya restauración no escribe un dato sino que dispara un ACTO hacia fuera: devolver la cola de ayer manda hoy mensajes que el paciente ya recibió, o que se cancelaron a propósito. `nunca`, y no por prudencia — mandar mensajes reales requiere autorización explícita del dueño.',
  },
  {
    dataClass: 'whatsapp-contacts-and-consent',
    sourcePath: 'whatsapp_contacts',
    systemOfRecord: 'firestore', mutable: true, signedOrImmutable: false,
    restoreAllowed: 'solo-si-falta', retentionClass: 'clinica', containsPHI: true,
    referenceDependencies: ['patient-demographics'],
    integrityInvariant: 'Lleva el CONSENTIMIENTO para escribirle a alguien por WhatsApp. Restaurar encima puede devolver un consentimiento que el titular ya retiró, y el retiro no deja rastro en el archivo viejo: el respaldo no sabe lo que pasó después de generarse. Por eso sólo se escribe si falta.',
  },
  {
    dataClass: 'whatsapp-delivery-status',
    sourcePath: 'whatsapp_status',
    systemOfRecord: 'externo', mutable: false, signedOrImmutable: true,
    restoreAllowed: 'solo-si-falta', retentionClass: 'auditoria', containsPHI: true,
    referenceDependencies: ['patient-demographics'],
    integrityInvariant: 'Acuse de entrega de cada mensaje: la prueba de si al paciente le llegó el aviso. El sistema de registro es el proveedor, no nosotros. Se conserva y no se sobrescribe, igual que el resto de los acuses.',
  },
  {
    dataClass: 'whatsapp-channel-events',
    sourcePath: 'whatsapp_events',
    systemOfRecord: 'firestore', mutable: false, signedOrImmutable: false,
    restoreAllowed: 'nunca', retentionClass: 'operativa', containsPHI: false,
    referenceDependencies: [],
    integrityInvariant: 'Conexiones y desconexiones del canal. Describe el estado de una integración VIVA: restaurar los eventos de ayer sobre el canal de hoy cuenta una historia de conectividad que no ocurrió.',
  },
]
...
export const FUERA_DE_LA_RUTA_DE_LANZAMIENTO: Record<string, string> = {
  ...
  'internamientos.registros': 'Hospital/UCI en ALPHA. Registros del episodio hospitalario.',
  uci_copilot_feedback: 'Hospital/UCI en ALPHA. Lo que el médico respondió a las sugerencias del copiloto de UCI: es materia prima de EVALUACIÓN, no expediente. Su régimen se fija cuando UCI entre a la ruta de lanzamiento.',
