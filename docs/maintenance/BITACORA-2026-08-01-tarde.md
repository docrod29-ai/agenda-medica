...
- **v1075** — REG-193. Probando el caso del TITULAR de REG-192 contra los motores reales de `main` —ya fusionado, con su golden en verde— seguía sin avisar: el paciente niega la diabetes, la nota se la pone como diagnóstico y `contradicciones()` devolvía `[]`. `primeraMencionSinEscudo` juzgaba cada aparición con los 60 caracteres previos EN CRUDO, y la mención de abajo tiene el «niega» de la LÍNEA DE ARRIBA dentro de esos 60: se recorrían todas las apariciones y se escudaban todas. Es palabra por palabra lo que `VENTANA_DEL_ESCUDO` ya temía («un escudo ajeno taparía una afirmación real»). Ahora el contexto se recorta en el fin de apartado (punto, salto de línea, punto y coma, dos puntos; la coma NO, que es la que enumera lo negado). Con el recorte llega `NO_AFIRMA`, porque al dejar sola la segunda mención «glucosa para descartar diabetes» habría estrenado un falso positivo de alta frecuencia sobre un aviso que no se pliega. NINGÚN cambio clínico. VERIFICADO que puede ponerse rojo (7 de 16). NO se mezcla aquí la frontera de palabra («plasma» contiene «asma»): queda como SAFE-008 en el backlog.
