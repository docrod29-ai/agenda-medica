...
- **v1074** — REG-192. Encontrado construyendo el corpus sintético del motor de temporalidad (EVAL-002): veinte términos que un internista dicta a diario y que NO son ninguna canónica; TRECE devolvieron una. Los dos motores de vocabulario clínico —negaciones (v985) y temporalidad (v1027)— buscaban con `includes`: `sida` vive dentro de «obesidad», «necesidad» y «densidad»; `asma` dentro de «plasma»; `cistitis` dentro de «colecistitis». Y negaciones no sólo avisa, RECLASIFICA: reproducido con el motor real, «Niega necesidad de oxígeno suplementario» pasaba un VIH confirmado a `descartado` — el diagnóstico que sostiene el tratamiento entero en la consulta de un infectólogo, arrastrándose a todas las notas siguientes. Nuevo `indiceDeTermino()` con frontera de palabra, una sola definición para los dos motores; devuelve el ÍNDICE y no un booleano porque la ventana de 60 caracteres de `contradicciones` se leía en el sitio de «obesidad». El plural se tolera a propósito («fracturas», «infartos»). «Derrame» retirado del vocabulario: avisaba de un ictus en cada derrame pleural; se exige «derrame cerebral» y el coloquial DEJA DE VIGILARSE, declarado. NINGUNA cifra clínica tocada: esto es gramática. VERIFICADO que puede ponerse rojo (11 de 17 casos).
