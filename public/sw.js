 ...
 */
const CACHE = 'nexusmed-v465'  // AUDITORIA MODULO 0, LOTE 5 (integridad de datos) — cierra el modulo. (1) PACIENTE DUPLICADO: la lista se cachea 30s y solo la invalida la pestana que escribe, asi que un alta en la tablet no se veia en la laptop y se creaba el mismo paciente dos veces, partiendo su historial; ahora se relee sin cache y se pregunta antes de duplicar. (2) SESIONES DE WHATSAPP DUPLICADAS: dos mensajes seguidos creaban dos documentos y la conversacion saltaba entre ellos perdiendo lo ya capturado; id derivado del telefono = duplicado imposible. (3) ALTA DE CONSULTORIO ATOMICA en /api/clinic/crear: dos pestanas en /setup creaban dos consultorios y el segundo pisaba la membresia del primero, dejando uno huerfano y facturable. (4) /setup ya muestra el error en vez de solo quitar el spinner, y deja de tirar especialidad y telefono.
