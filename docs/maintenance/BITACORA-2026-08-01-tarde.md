...
- **v1152** — REG-276 (hallazgo del barrido crudo, repasado contra el código de hoy). Los CUATRO ítems de STOP-BANG que se preguntan de viva voz —ronquido, somnolencia diurna, apneas presenciadas, hipertensión— se marcaban con SÓLO mencionar el término. El único guardián era el literal `!/niega (hipertension|hta)/`: dejaba pasar «niega presión alta», «sin hipertensión», «no tiene hipertensión» y «descarta HTA», y los otros tres ítems no tenían ninguno. Un varón de 58 que NIEGA las cuatro salía 5/8 — riesgo Alto, e imprimía polisomnografía, neumología y minimizar opioides; ahora 1/8 — Bajo. El motor de negación (`estaNegado`) ya existía y Caprini lo usaba DOS FUNCIONES MÁS ABAJO en el mismo archivo: no faltaba el motor, faltaba usarlo. El ronquido lleva negador propio porque su término es un verbo («no ronca») y `NEGADORES` cubre sustantivos; se resuelve en el módulo, sin ampliar `NEGADORES` para todo el expediente. Lo negado se escribe `false` y NO enciende la escala en la nota (`capturado()` descarta los `false`). NINGÚN umbral nuevo. VERIFICADO que puede ponerse rojo: 13 de 21 casos caen sin el arreglo. SEGUNDA VUELTA tras revisión independiente que CORRIÓ el motor: la v1 miraba una sola aparición y «Niega presión alta. Hipertensión en tratamiento con losartán» dejaba de puntuar al hipertenso documentado — regresión de la propia reparación, en la dirección contraria, en los CUATRO ítems. Ahora se miran TODAS las apariciones y gana la afirmación (ausencia de dato no es dato de ausencia). Además «pero» cierra la cláusula negativa como el punto —único cambio al motor compartido, arbitrado por las 6 949 pruebas restantes en verde— y el negador PEGADO («nunca ronca») se resuelve aparte de NEGADORES. NO reparado y DECLARADO con prueba de estado conocido: el formato pregunta-respuesta («¿Ronca fuerte? No.»), que falla igual en main y ya lo trabajan NEG-001/NEG-002/SAFE-003 — queda como SAFE-007 en vez de reimplementarse por tercera vez. Golden 21→31; rojo por partida doble: 18/31 contra main y los 6 nuevos contra la v1.
