...
- **v1074** — REG-192 (hallazgos C2+C3). MEDIDO con el motor real: de SIETE formas de decir que no, cazaba UNA. El motor exigía que la respuesta EMPEZARA por «no» y delante viene el guion de turno o una muletilla: «Pues no», «Fíjese que no», «Tampoco», «Diabetes no» y hasta «— No.» a secas se perdían — cada una es una enfermedad negada que la nota podía afirmar sin avisar. Además `parser-clinico.ts` tenía una SEGUNDA lista más pobre (sin «no padece»): «No padece diabetes» entraba como antecedente POSITIVO y contaminaba el STOP-BANG de la preoperatoria (mismo patrón que REG-177). LA TRAMPA de la reparación: al limpiar el guion, «— No sé» se vuelve «no sé» y contaría como negación; la guarda falló a la primera con `\bs[eé]\b` porque en JS `\w` es ASCII y «é» no cierra palabra — cazaba «no se» y fallaba con «no sé», la forma que se escribe.
- **v1075** — REG-193 (hallazgo D3, MÁS GRAVE de lo reportado: no era una dep olvidada, el dato no estaba en el respaldo EN ABSOLUTO). `proximoSeguimiento` sólo se persistía al FIRMAR: teclearla y recargar la borraba, y si era lo ÚNICO escrito (consulta de control de 2 min) el sistema decidía que no había nada que guardar. Reparado en los CUATRO sitios: payload del respaldo, deps, las dos condiciones de contenido, y la restauración (guardarla sin reponerla habría sido peor: parecería conservarse y al abrir estaría vacía). Alimenta la tarea «agendar seguimiento» y el contador de vencidos — un paciente sin fecha no reaparece en ninguna lista, sin error ni aviso. MISMO fallo que ya se pagó con `estudiosOrden` y `preop` en esas mismas deps: la lección no se había generalizado.
- **v1076** — REG-195 (P0, REPORTADO POR EL DR EN VIVO: «tengo el plan hecho, borro medicamentos y me borras el plan de la nota»). CAUSA: al firmar, si la IA marcó líneas con `[IA — no dictado]`, el diálogo ofrecía «Quitarlas y firmar» — y EL PLAN ES JUSTAMENTE LO QUE LA IA REDACTA (el médico lo dicta en prosa, no palabra por palabra). Fallaba en las TRES mitades: (1) no decía QUÉ quitaba, sólo «3 líneas»; (2) no se podía deshacer, aunque `snapshotUndo` existía desde hacía versiones; (3) «Quitarlas y firmar» NO FIRMA, hace return — así que el médico creía haber cerrado la nota, se le borraba el plan y al volver a firmar la firmaba SIN plan. Reparado: `lineasSugeridas()` las enseña con la sección delante, se guarda punto de deshacer, y el botón dice sólo «Quitarlas». + REG-194: la regla «el LLM no calcula» sólo regía en UCI.
- **v1077** — REG-196 (P0, REPORTADO EN VIVO con captura: «Nota de Primera Vez» FIRMADA con encabezados SUBJETIVO/OBJETIVO/EVALUACIÓN/PLAN). DOS causas: (1) de los 13 tipos de nota, `primera_vez` y `alta_consulta` NO tenían instrucción de formato en el prompt — sin una, el modelo escribe la que le sale y en documentación médica es SOAP; (2) al reprocesar sin cambiar de tipo la base eran las secciones QUE YA HABÍA EN MEMORIA, así que una clave de otro tipo no salía nunca. Nuevo `seccionesDelTipo()` que devuelve exactamente las del tipo conservando lo escrito, y lo que no encaja se devuelve APARTE (no se borra: perder prosa dictada para arreglar el formato sería peor). GUARDIÁN: una prueba recorre los 13 tipos y falla si alguno queda huérfano.
- **v1078** — REG-197 (hallazgo F1). MEDIDO con el propio motor: de 3 alucinaciones clínicas, 2 pasaban INVISIBLES — y las 2 eran las peligrosas («diabetes mellitus tipo 2 CON NEFROPATÍA ESTADIO 4» se contaba como acierto). 3 causas: `some()` en vez de proporción (bastaba UNA palabra compartida para absolver todo el valor); `if (campo in oro.esperado) continue` saltaba los campos correctos, así que lo inventado PEGADO a un dato bueno era invisible por construcción; y `v.includes(ov)` absolvía cuando significa lo contrario. Nuevo `sinSustento()` por proporción con umbral de método declarado (1/3). LECCIÓN: un arnés que sólo caza lo fácil mide la tranquilidad, no el riesgo — y es el instrumento con el que se juzga si la IA mejora.
- **v1079** — REG-198 (UX-001 del backlog del loop, score 70 — el pendiente más alto). «QUITAR DE LA NOTA» NO QUITABA NADA DE LA NOTA: el botón rojo del panel de revisión sacaba el id de `aprobados`, que sólo se guarda como metadato de auditoría. El médico veía un diagnóstico mal extraído, pulsaba «Quitar», el renglón se tachaba en pantalla… y el diagnóstico SEGUÍA en la nota que firmaba. Un control que miente sobre lo que hizo es PEOR que no tenerlo: sin él habría borrado el renglón a mano. Nuevo `quitar-de-la-nota.ts` con punto de deshacer. Las alergias quedan FUERA a propósito: viven en el expediente, no en la nota — borrarlas desde una consulta las quitaría de todas las futuras. Mismo patrón que REG-195, misma pantalla, mismo día.
- **v1080** — REG-199 (TRACE-001). El hallazgo era MEDIO FALSO y hay que decirlo: `CAMPOS_NO_SELLADOS_V3` YA documentaba que `transcripcionMotor` queda fuera del sello, con su razón (sellarlo cambiaría el hash de todo lo firmado → falsa alarma de REG-060). Lo que sí mentía era la PANTALLA: `noCubre: []` y `cubreTodo` derivado de «¿es la última versión?» — al médico se le decía que el sello cubre el contenido íntegro. Contar una limitación hacia dentro y ocultarla hacia fuera es peor que no documentarla. Ahora la cobertura se DERIVA de la lista de exclusiones y `cubreTodo` significa «no queda nada fuera». TRES pruebas certificaban la afirmación falsa (mismo patrón que el test que exigía la línea rota). Y el golden cazó un bug que introduje al repararlo: una nota SIN sello daba `cubreTodo: true` porque su lista de exclusiones está vacía. Subir a v4 queda como decisión del dueño (D-08).
- **v1081** — REG-200 (EVAL-002 del backlog). CORPUS ORO del motor de temporalidad: existía el motor, no la vara de medir. Medido sobre 26 frases de consulta mexicana: 16 aciertos, 10 fallos, TODOS del mismo tipo (pasado no detectado) y CERO falsos positivos — el motor erraba siempre del lado seguro, como declara su doc. Se le escapaban las formas más corrientes: «le dio hepatitis» (la mexicana de enfermar), «había tenido» (pluscuamperfecto), «fue diagnosticada» (pasiva), «ya no toma»/«dejó de»/«suspendieron» (CESE — distingue fármaco vigente de suspendido), «salió del hospital», «antes fumaba», «solía tener». Ampliado → 30/30 sin falsos positivos. Corpus de 32 frases sellado. «Ya no toma metformina» leído como vigente deja un fármaco fantasma en la lista contra la que se cruzan alergias e interacciones.
- **v1082** — REG-201 (§18 del charter V7). `docs/clinical-safety/` existía con CERO archivos — junto con otras 8 carpetas del §4.1: el patrón «escrito y sin conectar» aplicado a la documentación. Nuevo REGISTRO-DE-PELIGROS.md con DIEZ peligros en el formato exacto del charter (peligro·causa·daño·afectados·severidad·controles·pruebas·riesgo residual·responsable·aprobación). NINGUNO es hipotético: los diez ocurrieron y citan su REG. LO QUE NO SE HIZO A PROPÓSITO: la probabilidad NO se estima (se registra cuántas veces ocurrió — un número inventado de probabilidad es justo la cifra que este proyecto no se permite), y ninguna aprobación se marca (aceptar un riesgo clínico residual es del médico, no del sistema que lo produce; las 10 quedan en ☐). GUARDIÁN de 108 casos que verifica que los archivos de prueba CITADOS EXISTAN — un registro que cita una prueba renombrada declara un control que no está. Y el documento DECLARA SUS HUECOS: paciente equivocado, fuga entre consultorios, inyección en el dictado, embarazo/pediatría/renal.
- **v1083** — REG-202 (§N3 del charter). SALA DE DATOS: `docs/data-room/` estaba vacía. Índice con 8 secciones marcadas VERIFICADO/PARCIAL/NO EXISTE/DEL DUEÑO. Verificado hoy con su comando: 40 dependencias directas, NINGUNA copyleft fuerte (`scripts/data-room/licencias.mjs`, corrible por el comprador). EL GUARDIÁN ME CAZÓ DOS VECES: (1) escribí «49 REG» cuando eran 48 — cifra inflada, sin querer, en el documento cuya primera línea dice «nada de tracción falsa»; (2) al añadir mis propias pruebas el sello creció y el documento quedó desfasado EN EL ACTO — no fue descuido, es ESTRUCTURAL: un documento con cifras tecleadas miente el día que el repo crece. Por eso ahora se DERIVAN con `actualizar-cifras.mjs`, y lo que no puede derivar (el total de pruebas) lo AVISA en vez de adivinarlo. Y el propio guardián estaba mal a la primera: su patrón cazaba la NEGACIÓN («ni hospitales clientes»), o sea impedía declarar la ausencia — un guardián que impide declarar un hueco empuja a callarlo. DECLARADO en vez de callado: pentest externo NO realizado, métricas de negocio NO existen, sin validación clínica formal, ninguna aprobación de riesgo firmada. El bloqueo nº1 no es técnico: el código está en cuenta personal, no de la sociedad.
- **v1084** — REG-203 (§5.1 del charter). ADR: `docs/decisions/` estaba vacía. Cuatro escritos, todos de decisiones YA TOMADAS (no propuestas): ADR-001 una fuente de verdad por entidad clínica (de REG-034/035/171, los cuatro parsers de alergias), ADR-002 el LLM no calcula (REG-194), ADR-003 el sello tiene versión propia (REG-060, REG-199), ADR-004 tres niveles de aviso (REG-181). LAS TRES REGLAS del directorio: (1) las alternativas descartadas SE ESCRIBEN — un ADR sin ellas documenta un hecho consumado, no una decisión, y es lo que impide que alguien reproponga lo mismo en seis meses; (2) las consecuencias NEGATIVAS también — callarlas hace que el coste se descubra tarde y parezca un defecto (ADR-004 escribe la suya: «plegar es esconder»); (3) una decisión CLÍNICA del médico NO es un ADR — meterla aquí haría parecer que el sistema decidió algo que no le corresponde. Guardián de 28 casos: verifica que los archivos citados EXISTAN y que ninguno fije un umbral clínico disfrazado de arquitectura. Y el guardián de la sala de datos volvió a saltar al crecer el sello: se le añadió el comando de arreglo AL MENSAJE, para no rediagnosticarlo cada vez.
- **v1085** — REG-204 (§H6 del charter). LA PUERTA DE LIBERACIÓN: los nueve ceros que bloquean publicar una versión (paciente equivocado, error de medicación/unidad/negación SILENCIOSO, cita fabricada, orden activa no confirmada, acceso entre consultorios, pérdida de datos, pago duplicado) vivían sólo en el charter — o sea, FUERA del código que tienen que bloquear. Al mapearlos contra la suite salió lo contrario de lo que esperaba: **los nueve YA tenían cobertura**, ninguno estaba desnudo. Lo que faltaba no era protección sino la DECLARACIÓN de que esa protección es una puerta, y el guardián que comprueba que siga en pie. DOS QUEDAN DECLARADOS DÉBILES en vez de pintarse de verde: (1) «paciente equivocado» es el PRIMERO de la lista del charter y el peor cubierto — lo protegen controles reales pero DERIVADOS (`deEstePaciente`, `clinicId`), ninguno escrito pensando en ese peligro, y no hay corpus adversarial que intente a propósito meter la nota de un paciente en el expediente de otro; (2) el aislamiento entre consultorios corre en cada PR pero prueba LO QUE SE LE PIDIÓ probar, sin equipo rojo independiente (§5.16). La palabra que hace el trabajo es SILENCIOSO —seis de los nueve la llevan—: un error que el sistema avisa es trabajo hecho; el que persigue esta puerta es el que se lee igual que un dato correcto dentro de un documento firmado con cédula. Y dentro del propio documento queda escrito lo que NO significa: un cero no dice que el error no pueda ocurrir, dice que sobre los casos que ALGUIEN PENSÓ EN ESCRIBIR no ocurrió sin avisar. Guardián de 15 casos: que sean nueve, que ninguna prueba citada haya desaparecido, que los débiles no se pinten verdes y que nadie borre esa última frase. Son las DOS formas de perder una protección sin enterarse —que se borre y que se ahueque—; el sello ya cubría la segunda, faltaba la primera.
- **v1086** — REG-205 (§H7 del charter). DE QUÉ SE ENFERMA ESTE SISTEMA: los 53 REG del ledger tenían cada uno su prueba de regresión, pero NADIE los había mirado juntos. Contados por familia de causa raíz sale lo que ninguno dice solo. LA MÁS GRANDE: «escrito, probado y sin conectar», **9 de 53** — el módulo estaba bien, sus pruebas pasaban, y el sistema fallaba igual porque NO CORRÍA donde tenía que correr (sobredosis después de firmar, «Quitar de la nota» tocando un metadato de auditoría, los motores viendo la receta de hoy en vez del paciente entero, el 80,6 % de las correcciones tirado, el bucle que nunca aprendió). Implicación práctica: una prueba unitaria verde es compatible con las nueve — sólo las cazan las que recorren EL CAMINO. LA SEGUNDA: «el sistema se contradice a sí mismo», 8 de 53, donde NINGUNA de las dos partes está mal por separado, así que ninguna revisión de una pieza las encuentra: es lo que justifica los guardianes de coherencia. La taxonomía es CÓDIGO y no una tabla, porque una tabla a mano envejece en silencio: el guardián compara contra el ledger y FALLA si un REG se queda sin familia — este mismo REG-205 lo estrenó, se puso rojo al añadirlo hasta clasificarlo. Y el guardián de módulos huérfanos cazó, en cuanto lo escribí, el módulo que documenta que «escrito y sin conectar» es la familia más grande: no es ironía, es la prueba de que sigue mirando. ESCRITO DENTRO: el conteo sólo cuenta lo ENCONTRADO — una familia pequeña puede serlo porque es rara o porque nadie la busca, y las dos se ven idénticas; la sospechosa es «fuga entre consultorios», 4 casos, los 4 hallados auditando a propósito.
- **v1087** — REG-206 (§4.1). LA ARQUITECTURA, MEDIDA EN VEZ DE DIBUJADA: `docs/architecture/` era la última carpeta técnica vacía. Existía el mapa en CLAUDE.md y existían los ADR, pero NADIE había comparado el diagrama con el grafo de imports REAL. Medido sobre 734 archivos y 1 314 aristas: **0 dependencias invertidas, 0 ciclos, 0 módulos de lib atados a una pantalla**. El resultado limpio no era el esperado y se dice tal cual — ni un solo motor clínico está atado a la interfaz, que es lo que le impediría correr desde una ruta de API (por donde entran los motores cuando algo se automatiza). LA GRIETA ENCONTRADA: `types/` no era una hoja, dos archivos traían código en tiempo de ejecución desde lib. CERRADA la de `hospital.ts` —re-exportaba un VALOR con alias y tenía UN SOLO consumidor, que ahora importa de la fuente única; la comodidad de un alias no valía una arista invertida—. DECLARADA la de `clinical-quantity.ts`: usa `num()` de lib/uci y HACE BIEN (es la fuente única de la coma decimal mexicana); lo que está mal es DÓNDE VIVE — es un módulo de dominio completo alojado en `types/` por herencia, y moverlo toca a todos sus consumidores: se decide, no se cuela en una madrugada. POR QUÉ IMPORTA: hoy no hay ciclo, pero es por ahí por donde aparecería el primero (lib→types→lib), y un ciclo no siempre rompe — cuando rompe deja un `undefined` en un módulo que se lee perfecto, o sea UNA CIFRA QUE NO SALE en vez de un error que salte. Los `import type` no cuentan (TS los borra): contarlos daría violaciones inexistentes, el mismo detalle que tapó cuatro huérfanos durante meses en v1019. Guardián de 8 casos: el valor no es certificar el cero, es que quien lo rompa se entere EN SU PR y no seis meses después. ESCRITO DENTRO lo que NO dice: que la dirección sea correcta no dice que los límites estén en el sitio correcto — un lib enorme con todo dentro cumpliría las tres reglas y seguiría siendo un nudo.
- **v1088** — REG-207 (§4.1 y §N5). EL GUARDIÁN DE AFIRMACIONES NO MIRABA DONDE MÁS IMPORTA: `claims-guard` vigila landing/precios/demo —cuatro archivos de src— y NO vigilaba `docs/`, que es lo que lee un comprador en diligencia debida. De los dos públicos el segundo es el que TIENE CONSECUENCIAS: un visitante que lee una exageración se encoge de hombros; un comprador que verifica UNA y la encuentra falsa deja de creerse el resto del paquete, incluido lo que sí está medido. AL ENCENDERLO: seis afirmaciones sobre terceros SIN UNA SOLA FUENTE en `docs/COMPETITIVE_ANALYSIS.md`, bajo una columna titulada literalmente «Por qué somos superiores» — «Nadie con esa granularidad», «Pocos en LATAM», «Casi nadie en EHR cloud», «nadie lo expone visualmente». LA REGLA: un documento competitivo tiene dos clases de frase y sólo una se sostiene — lo que hacemos NOSOTROS (verificable, con su comando) y lo que hace un TERCERO (necesita fuente y fecha, o no se escribe). El documento viejo NO se borró: se le puso aviso de alcance y se reescribió la tabla para decir sólo qué hace este producto y dónde comprobarlo — borrar habría escondido que alguna vez se afirmó, y esconderlo es lo que hace que se vuelva a afirmar. DOCUMENTO NUEVO `docs/competitive/EL-FOSO.md`: separa lo que ES foso (corpus mexicano medido, motores deterministas, 55 REG con causa raíz, guardianes) de LO QUE NO LO ES, escrito para no engañarnos — usar un modelo de frontera, transcribir voz, tener agenda/receta/expediente, «UX bonita», el número de funciones: casi todo lo que un usuario VE no es foso. Y EL GUARDIÁN ME CAZÓ DOS VECES, las dos la misma lección: (1) cazó el documento escrito para DENUNCIAR esas frases, porque las cita — arreglo: afirmar es distinto de citar, lo entrecomillado no cuenta; (2) me impedía escribir «No somos los únicos» y «Ninguno de los ceros está medido», dos declaraciones de hueco SOBRE NOSOTROS. Es el error exacto del guardián de la sala de datos en v1083: un guardián de honestidad que castiga la honestidad empuja a CALLAR el hueco en vez de escribirlo.
- **v1089** — REG-208 (§4.1, última carpeta). EL CAMINO DEL MÉDICO: los 7 pasos del paciente a la nota firmada, con una prueba que lo RECORRE de verdad desde `src/app/` siguiendo imports. Existe porque la familia de defecto más grande —9 de 56— es «escrito y sin conectar» y LOS NUEVE TENÍAN PRUEBA UNITARIA EN VERDE. LO IMPORTANTE DE ESTE REG NO ES EL DOCUMENTO SINO LO QUE PASÓ AL MEDIR: la primera cifra fue «87 módulos fuera del camino» y ERA FALSA. El lector tenía CUATRO cegueras, todas del mismo tipo —veía texto donde tenía que ver código—: (1) `import type` contado como dependencia; (2) `await import()` y `dynamic(() => import())` invisibles, con lo que los paneles clínicos de Next parecían fuera → 87 a 70; (3) rutas relativas `./x` ignoradas, con lo que el INTERIOR de cada motor parecía fuera —los 19 archivos del antibiograma— → 70 a 29; (4) comentarios leídos como código → 29 a 35. Y la cuarta trajo la peor: al quitar comentarios EL ORDEN IMPORTABA — un comentario de línea que dice «la lógica vive en src/lib/inmuno/*» tiene una barra-asterisco que ABRE UN BLOQUE FALSO, y el limpiador se comía los seis imports del motor de inmunocomprometido: aparecía desconectado ESTANDO MONTADO en la consulta. LECCIÓN: un instrumento que declara desconectado un motor clínico que sí corre es PEOR que no tenerlo — manda a buscar donde no hay nada y desacredita las veces que acierta. Antes de reportar una cifra, comprobar el instrumento con un caso cuya respuesta ya se conoce. RESULTADO REAL: 500 módulos, 471 alcanzables, 29 no — y 26 de esos 29 YA estaban declarados como huérfanos aceptados. Dos instrumentos independientes convergiendo es la mejor señal que puede dar una medición. Los otros 3 son islas de dos: importados, pero por un módulo que tampoco corre. Trinquete en 29: puede bajar, no subir.
- **v1090** — REG-209 (§B5 del charter, IA clínica). BENCHMARK DE NÚMEROS Y UNIDADES CRÍTICAS: la POLÍTICA existía (los pares que jamás se autocorrigen: mg/mcg, PEEP/PIP, derecha/izquierda) pero LA MEDICIÓN NO. Escribí 20 formas reales de dictar una cifra en un consultorio mexicano y las pasé por el pipeline: resolvía 14, fallaba en CINCO maneras de decir una fracción. LAS DOS PELIGROSAS NO PIERDEN EL DATO, LO REDUCEN A UN VALOR PLAUSIBLE: «dos coma cinco miligramos» salía «2 coma 5 mg» y el motor de dosis lee 2 mg (20 % menos); «un gramo y medio» salía «1 g y medio» y lee 1 g (un tercio menos). EN MÉXICO LA COMA ES EL SEPARADOR DECIMAL AL DICTAR y «y medio» es la forma normal de decir dosis y media — ninguna estaba cubierta. POR QUÉ ES EL PEOR TIPO DE FALLO: una cifra que DESAPARECE se nota (campo vacío, hay guardianes); una que se convierte en OTRA CIFRA CREÍBLE no la ve nadie — ni el médico al releer, ni el motor, ni el sello. Mismo patrón que el pH «7.30 y 5» y la metformina «852 veces al día». REPARADO: «coma» se trata como «punto» (la guarda que ya existía —número delante Y número detrás— es lo que impide romper «el paciente está en coma», y está probado); mitades habladas sólo con unidad de FÁRMACO detrás, con «media hora», «a medio camino», «la media de la serie» y «dos veces y media» probadas INTACTAS — sin esos cuatro casos una regla demasiado ávida pasaría igual y su daño no se ve hasta que ensucia una nota real. DECLARADO SIN ARREGLAR: «punto cinco miligramos» sin el cero (ambiguo con «el punto tres del plan») y «cinco décimas» (aritmética, no gramática) — quedan como casos del corpus que documentan el límite, no como huecos.
- **v1091** — REG-210 (§B8, IA clínica). ¿A QUIÉN LE PASÓ? El tercer eje: ya estaban medidos el «¿sí o no?» (negación) y el «¿cuándo?» (temporalidad), faltaba el «¿a quién?». En una consulta buena parte de lo que se dice sobre enfermedades NO ES DEL PACIENTE: «mi mamá tuvo cáncer de mama», «mi papá murió de un infarto». Metido como antecedente PERSONAL deja una historia clínica IMPECABLEMENTE REDACTADA afirmando un cáncer que el paciente nunca tuvo, firmada con cédula — no se ve raro, por eso es de los peores. Y EL ERROR AL REVÉS CUESTA IGUAL: cuando el familiar sólo es QUIEN LO CUENTA («mi esposa dice que ronco»), el síntoma es del paciente y mandarlo a antecedentes familiares BORRA UN DATO REAL. LA TRAMPA QUE VOLVIÓ A COSTAR: escribí el patrón con `\b` al final y en JavaScript `\w` es ASCII — la «á» no cuenta como letra, así que `\b` detrás de «mamá» NO ENCUENTRA LÍMITE DE PALABRA. El motor reconocía «mi abuela» y NO «mi mamá» ni «mi papá», las dos formas más frecuentes: media función muerta y la otra media funcionando, lo peor para darse cuenta. Es el mismo fallo del motor de negación con «no sé»; ahora usa `(?![\p{L}])` y hay un caso del corpus dedicado. HECHO: motor determinista (con formas coloquiales «jefa», «apá» — un motor que sólo conoce «madre» y «padre» falla justo con quien habla con más confianza), regla 19-ter del prompt, y CONECTADO a la barra como origen nivel «revisa»: el motor dice de quién es la frase, quién decide dónde va el antecedente es el médico. LOS GUARDIANES SALTARON TRES VECES SEGUIDAS y ninguno me dejó entregarlo a medias: el de módulos huérfanos (el motor no estaba conectado — la familia de defecto nº1), el de la versión del prompt (huella nueva sin bumpear) y el de la barra (exige declarar el nivel de todo motor nuevo). Corpus oro de 25 casos.
- **v1092** — REG-211 (§B6, IA clínica). EL CUARTO EJE: ¿con cuánta seguridad lo dijo? El charter pide distinguir presente/ausente/posible/probable/incierto/histórico/condicional/no mencionado, y sin este motor los OCHO colapsaban en DOS: dicho o no dicho. «Creo que me dijeron que tenía anemia» quedaba como «Anemia» — lo que el paciente ofreció como DUDA queda en el expediente como DIAGNÓSTICO, y a partir de la segunda consulta ya nadie sabe que era una duda. Cuatro matices porque cambian qué hacer con el dato: duda / posibilidad / referido / aproximado. SALVEDAD que evita el aviso inútil: si el paciente TRAE LA CONSTANCIA («aquí traigo la biometría») ya no es duda — un aviso que salta teniendo el papel en la mano se aprende a cerrar. Por omisión AFIRMADO, no al revés: marcar de más llenaría la nota de dudas inventadas. Y EL MISMO DEFECTO POR TERCERA VEZ EN LA NOCHE: `\b` detrás de acento no dispara (en JS `\w` es ASCII). Ya había costado con «no sé» (negación) y «mamá» (experienciador), y hoy con «quizá». SÍNTOMA SIEMPRE IGUAL Y EL PEOR POSIBLE: media función viva y media muerta — «quizas» sí, «quizá» no; nada revienta, nada avisa. UN COMENTARIO EN CADA ARCHIVO NO BASTÓ: el tercero se escribió con la lección ya escrita dos veces. Ahora es una PRUEBA que revisa los seis motores de lenguaje — y encontró TRES MÁS en el acto en el módulo escrito una hora antes: «a mí», «me salió», «padecí» y «sentí» tampoco disparaban. El guardián marca de más a propósito: en un motor de español hablado `(?![\p{L}])` es correcto siempre, así que exigirlo uniforme elimina la clase entera de fallo.
- **v1094** — REG-212 (IA clínica, composición). CADA MOTOR ACERTABA SOLO Y JUNTOS MENTÍAN: los cuatro ejes ya tenían su corpus oro cada uno, pero NADIE HABÍA PROBADO SI SE ESTORBAN. Pasé nueve frases que tocan varios ejes a la vez por los tres motores y dos fallaron. (1) UNA FRASE CON DOS DUEÑOS: «yo no tengo diabetes PERO mi mamá sí» — la negación decía bien «diabetes negada» y el experienciador decía «toda la frase es del familiar». Analizada entera se perdía LO QUE DE VERDAD DICE: que el paciente la niega Y que la mamá sí la tiene. Dos datos distintos, de dos personas distintas, en catorce palabras. Ninguna prueba de pieza podía encontrarlo porque el fallo NO ESTÁ EN NINGUNA PIEZA: está en que las dos leen la frase entera cuando la frase cambia de sujeto a mitad. FIX: cortar también en los conectores que cambian de sujeto (pero, aunque, en cambio, mientras que, sin embargo), con un caso que comprueba que SIN conector no parte de más. (2) LA DUDA QUE VA AL FINAL: «mi mamá no tuvo cáncer, creo» — el patrón exigía «creo QUE» y en español hablado la duda se POSPONE muchísimo; la frase entera pasaba como afirmada. Se exige coma o final delante para no cazar «creo un recordatorio», y está probado. LO QUE ENSEÑA SOBRE CÓMO PROBAR: los cuatro corpus que ya existían son correctos y no habrían encontrado ninguno de los dos. Es la familia «escrito y sin conectar» un nivel más arriba: no es que el motor no corra, es que CORRE SIN VER LO QUE EL OTRO VE.
- **v1095** — REG-213 · SUP-001 del loop competitivo. ¿DE DÓNDE SALIÓ ESTO? Cierra la distancia MÁS GRANDE frente a Abridge (Linked Evidence) y es el §B10 del charter propio, que llevaba sin hacer desde que se escribió. LA PREGUNTA: frente a una nota generada la duda del médico no es «¿está bien redactada?» sino «¿DE DÓNDE SALIÓ ESTO?», y hasta hoy la única forma de contestarla era VOLVER A OÍR LA CONSULTA ENTERA — y un médico que tiene que reescuchar veinte minutos para comprobar una línea no comprueba: FIRMA. Ahora cada afirmación se enlaza al fragmento del dictado que la sostiene, con su posición para resaltarlo, y lo que ninguno sostiene se declara CON LAS PALABRAS QUE NADIE DIJO. LO QUE NO HACE, escrito dentro: no dice que sea VERDAD, dice que hay un fragmento que la sostiene; elimina la frase que NADIE DIJO, no todos los errores. Y NO ELIGE POR EL MÉDICO: cuando no hay respaldo lo dice en vez de enlazar al fragmento menos malo — un enlace inventado daría por comprobado justo lo que hay que comprobar. POR QUÉ NO SE LE PIDE AL MODELO QUE CITE SUS FUENTES: un modelo que inventa una frase también puede inventar de dónde la sacó, y entonces el enlace CERTIFICA la alucinación en vez de delatarla. EL HALLAZGO CARO fue el falso positivo: la primera medición marcaba «cefalea» (←dolor de cabeza), «colecistectomía» (←me operaron de la vesícula) y «madre» (←mi mamá) como inventadas, SIENDO TRADUCCIONES CORRECTAS que un médico hace al redactar. Un aviso que las señala se aprende a cerrar en dos consultas — y entonces deja de proteger de lo que sí importa, que EN ESA MISMA NOTA era una «nefropatía diabética estadio 4» inventada de cero. La tabla de sinónimos es ESTRICTAMENTE LINGÜÍSTICA: «dolor de cabeza» ES cefalea; «cada mañana» NO es «cada 24 horas» —eso es interpretar la pauta— y se sigue marcando, con su caso en el corpus. Es también el tipo de activo que no viene con ninguna API: la tabla del español que se habla en un consultorio mexicano.
- **v1096** — REG-214 · P0 CLÍNICO. LA ALERGIA ESTRUCTURADA NO LLEGABA A LA COMPUERTA DE FIRMA. Reproducido con los motores reales: paciente con «Penicilina» SÓLO en el campo estructurado + prescripción de cefalexina → lo que se sellaba daba 0 alergias y la compuerta 0 errores; con el lector correcto, 1 alergia y «[Contraindicado] beta-lactámicos». EL BETALACTÁMICO SE FIRMABA SOBRE UN ALÉRGICO CON EL AVISO A LA VISTA EN LA PANTALLA — porque la pantalla lee `alergiasDe` (y pinta el rojo) y lo que se sella leía `parsearAlergiasTexto`, que sólo mira el TEXTO LIBRE. De `nota.alergias` cuelga la compuerta de nom004: cruce por subcadena y reactividad cruzada por familias; con la lista vacía los dos callan. FAMILIA: dos lecturas del mismo campo (REG-034/035/171, ADR-001) — había dos lecturas y LA QUE GOBERNABA LA SEGURIDAD ERA LA CIEGA. QUIÉN LO ENCONTRÓ: la RUTINA AUTÓNOMA del Master Loop, en su rama `agent/safety/SAFE-001-sello-de-la-nota`. Su diagnóstico era correcto y su caso real; SU REPARACIÓN NO: usaba `alergenosDe`, que devuelve string[], cuando la compuerta espera AlergiaEstructurada[] y hace `al.alergeno.toLowerCase()` — con cadenas sueltas revienta o queda ciega por otra puerta. La correcta es `alergiasDe`. Por eso hay un caso del corpus dedicado AL TIPO que devuelve el lector: aquí el tipo ES parte del arreglo. LECCIÓN DE OPERACIÓN: la rutina trabaja sobre `main` y esta sesión sobre `agent/pagos/PAY-001` — dos carriles que nunca se cruzaron, y su trabajo llevaba un día parado sin llegar a producción. Además los números de REG COLISIONARON (su REG-202 es otro defecto que el mío) porque cada uno numeró sin ver al otro. PENDIENTE: revisar sus otras tres ramas (dos de negación, una de temporalidad) y unificar el carril y la numeración.
- **v1097** — REG-215 · §F3 DEL CHARTER (reconciliación de medicamentos). EL PACIENTE DECÍA «EL LOSARTÁN YA LO DEJÉ» Y LA LISTA SEGUÍA IGUAL PARA SIEMPRE, porque nada convertía «lo dejé» en un cambio. POR QUÉ ES SEGURIDAD Y NO ORDEN: de esa lista cuelgan el cruce de interacciones, el de alergias, el motor de dosis y la receta — desactualizada NO ES UN DATO VIEJO, es un motor de seguridad razonando sobre un paciente que no existe. NO CORRIGE LA LISTA SOLO: abre una tarea con dueño y la deja hasta que un humano decida, porque el paciente puede equivocarse, el reconocedor puede transcribir mal el nombre y SUSPENDER UN ANTICOAGULANTE ES UN ACTO MÉDICO (§C3: no elegir la verdad automáticamente). La forma del dato lo garantiza: no hay campo «aplicar», y hay un caso del corpus que lo comprueba. TRES FILTROS porque un worklist que se llena se abandona en una semana —y entonces tampoco se ve la tarea que sí importaba—: familiar, duda, y lo que el médico receta HOY. Los tres reutilizan experienciador y certeza: NINGÚN PARSER NUEVO del mismo texto. LO QUE COSTÓ MEDIR: dos formas reales del habla se escapaban y son LAS MÁS FRECUENTES — «me la subieron a 850» (el patrón exigía unidad y el paciente no la dice) y «me quitaron el losartán» (exigía el pronombre). Seis de ocho a la primera. Y UNA CORRECCIÓN MÍA: el motor de tareas del §F YA EXISTÍA y es bueno (seis estados, máquina de transiciones con veredicto, escalamiento, y `tareasDeNota` conectado desde antes); mi auditoría anterior lo dio por inexistente y era FALSO. Esto no construye un segundo motor: añade el tipo al que ya había. Tres casos del corpus comprueban EL CABLE.
- **v1099** — REG-216 y REG-217, los DOS reportados por el Dr. con captura, trabajados con tres agentes en paralelo. (1) DIECINUEVE DIAGNÓSTICOS EN UNA NOTA con el mismo cuadro tres veces bajo el MISMO CIE-10 (R59.1 ×3, D64.9 ×2, N39.0 ×2). Dos causas: la fusión CONCATENABA y sólo descartaba el repetido si el texto era idéntico letra por letra —ignorando el código, que estaba ahí y que el propio prompt obliga a rellenar—; y EL MULTIPLICADOR, el pase en vivo cada 15 s / 18 palabras = ~40 pasadas por consulta, cada una re-redactando los síndromes distinto. Eso explica también los DOS CUADROS MEZCLADOS. No se arregla reemplazando: la fusión existía porque reemplazar BORRABA el diagnóstico que el médico capturó a mano. La distinción correcta no es viejo contra nuevo, es LO QUE PUSO LA IA contra LO QUE PUSO EL MÉDICO. Más regla 7-bis: 3 a 6 diagnósticos, una entrada por CIE-10, los hallazgos de laboratorio NO son diagnósticos, lo crónico del historial no se repite. (2) LA NOTA SALÍA HUECA con el dictado entero delante: la regla 15 ORDENABA escribir «No referido» y la 1-bis LO PROHÍBE — y el guardián de contradicciones no lo cazaba porque esas frases NO ESTABAN EN SU LISTA; vivió meses. El mecanismo que lo hacía irreparable: la primera pasada en vivo ocurre cuando apenas se dictó la ficha de identificación, rellenaba todas las obligatorias con huecos, y la guarda `if (enVivo && s.value?.trim())` los daba por contenido — NINGUNA pasada posterior podía corregirlos. Y LO PEOR: la compuerta de firma sólo comprueba que no esté en blanco, así que «No referido.» LA PASABA: la nota hueca quedaba FIRMABLE CON CÉDULA. Reparado en tres capas (regla corregida, saneo `seccionEsHueco` en los dos sitios y ANTES de la guarda —el orden ES el arreglo—, y el guardián ampliado). La distinción delicada: «No referida.» es hueco pero «No refiere fiebre ni disnea» es un NEGATIVO PERTINENTE que la regla 16 pide; por eso se compara la sección ENTERA, no por contención. CONSECUENCIA PARA LA TAXONOMÍA: «el sistema se contradice a sí mismo» pasó a ser la familia MÁS GRANDE (10 de 65), adelantando a «escrito y sin conectar».
- **v1100** — REG-218 «Algo se atoró en esta pantalla» (iPhone del Dr., con captura). HONESTIDAD PRIMERO: la excepción CONCRETA no se pudo leer — vive en `/superadmin/errores` y la llave de administrador sólo está en Vercel; descargar TODOS los secretos de producción a disco para leer un mensaje no es un cambio justo por ese valor. Lo reparado NO se declara como la causa de ese fallo: son tres huecos confirmados leyendo el código que PUEDEN producir ese síntoma y que son defectos por sí mismos. (1) LA GUARDA ESTABA EN UNO DE CUATRO: `setSignos(n.signosVitales ?? {})` la tenía y secciones, diagnósticos y medicamentos NO — una nota vieja sin el campo deja el estado en `undefined` y el siguiente render revienta en `.map`. (2) `Array.isArray` VALIDA EL CONTENEDOR, NO LOS ELEMENTOS: un `null` dentro o un elemento de un esquema anterior pasa entero y truena igual; y es el MISMO momento de la sesión que el aviso de audio guardado. REGLA QUE SALE: restaurar nunca debe poder tumbar la pantalla — se descarta ESE elemento, no la lista; perder un renglón dudoso es infinitamente mejor que perder la consulta con el paciente delante. (3) «REINTENTAR» NO PODÍA FUNCIONAR para el error más probable en celular: la consulta carga ocho piezas bajo demanda y hay service worker; en red inestable —o JUSTO DESPUÉS DE UN DESPLIEGUE— la descarga falla, y `reset()` re-renderiza el mismo árbol con el trozo todavía ausente. El médico pulsa, ve lo mismo, y concluye que la app se rompió. Ahora se reconoce, el título cambia a «Falta bajar una parte de la aplicación», el botón principal RECARGA, y se registra con origen propio `boundary:consulta:chunk` — sin eso «Algo se atoró» son cinco fallos distintos bajo un mismo mensaje. PENDIENTE DEL DR: mirar el campo `origen` de ese día en /superadmin/errores; parte el problema en dos por sí solo.
- **v1101** — REG-219 · §D1 DEL CHARTER (ciclo de vida del medicamento). EL ANTIBIÓTICO DE HACE UN MES SEGUÍA «VIGENTE»: nadie comparaba la duración con el calendario, así que «7 días» prescrito hace un mes seguía alimentando el cruce de interacciones, el de alergias y el motor de dosis. Es el daño de REG-215 POR OTRA PUERTA — allá lo decía el paciente, aquí lo dice el calendario; en los dos casos, motores de seguridad razonando sobre un paciente que no existe. POR QUÉ «PROBABLEMENTE TERMINADA» Y NO «TERMINADA»: el sistema NO SABE si el paciente lo terminó, sabe que la duración escrita ya pasó — pudo suspenderlo por un efecto adverso, alargarlo por otro médico, o no surtirlo nunca. Marcarlo terminada sería afirmar un hecho clínico que nadie comprobó (§D1: «no lo marques completado en silencio»). LO CRÓNICO NO CADUCA: la metformina de un diabético no termina a los 30 días, y marcarla llenaría el worklist de tareas falsas cada mes. MARGEN DE GRACIA porque el paciente rara vez empieza el mismo día —surte la receta al día siguiente— y avisar el día exacto produce tareas que se cierran sin mirar. ANTE LA DUDA SE CALLA: duración incontable, fecha ilegible o futura → no venció; el error caro es decirle al médico que suspenda algo que el paciente debe seguir tomando. Y UNA CORRECCIÓN MÍA en el mismo despliegue: sellé ese archivo con 10 casos cuando el contador del sello ve 9. Recontar TODOS los sellos con la regla exacta del gate encontró además dos que habían CRECIDO (e0-12 y firestore-rules-guard) y nadie había subido. El sello sólo protege si el número es el que el gate cuenta, no el que yo creo.
...
enfermedad en la pregunta, así que entraba entero: «¿Tuvo tuberculosis? **No.**»
dejaba una mención pasada de tuberculosis, con la pregunta como cita. Y «**no
tuvo** tuberculosis» también, porque `NIEGA_EN_LINEA` sólo conocía el presente.

Importa porque el aviso que sale de ahí dice «esto se dijo en pasado», que es la
frase con la que uno mueve una condición a antecedentes — y llega por las dos
puertas, la barra de la consulta y el extractor de entidades. **Una negación
convertida en antecedente es historia clínica fabricada**: un «nunca tuve
tuberculosis» acaba escrito como tuberculosis pasada y cambia cómo otro médico
lee un PPD dentro de seis meses.

Al revés faltaba lo legítimo: «¿Ha tenido neumonía alguna vez? Sí, hace tres
años» devolvía **nada** — la pregunta dice *qué* y la respuesta dice *cuándo*.
Ahora la pregunta sólo cuenta con su respuesta delante y sólo si es un sí claro;
el par se juzga entero y el presente sigue mandando dentro de él. `respuestaA()`
sale a `negaciones.ts` para que los dos motores lean el mismo turno.

Es el hueco hermano del que costó tres versiones reparar allí, y el encabezado de
`temporalidad.ts` dice que este motor viene a evitarlo: reutilizó su
**vocabulario**, no su **emparejado de pregunta y respuesta**.

Encontrado trabajando `EVAL-002`: el motor no tenía corpus y las frases del
interrogatorio nunca se le habían puesto delante. Golden de 22 casos; revertido,
fallan 17.

### Lo que trajo la fusión con main

`REG-192` y esta tanda tocan los mismos dos módulos y **no se pisan**: aquél
repara DÓNDE se busca el término en la nota; ésta, QUÉ se cosecha del dictado. Se
toma su `primeraMencionSinEscudo` tal cual.

De su lado vino también `respuestaNiega()`, mucho más rico que la comprobación de
negación que traía esta rama —muletillas, «no sé», «tampoco», el «no» pospuesto—.
Se adopta el suyo y se retira el propio: `respuestaAfirma()` se escribe como su
espejo y **reutiliza su mismo limpiador de ruido**, para que las dos mitades del
interrogatorio se lean con el mismo criterio. Ahí queda decidido, de una vez,
qué significa «creo que»: si «creo que no» cuenta como negación, «creo que sí»
cuenta como afirmación. Dos motores que discrepen en eso son la divergencia que
esta reparación viene a quitar.

Y con él vino su trampa, que picó igual: `s[ií]\b` **no caza «Sí, hace tres
años»** porque `\w` es ASCII y entre «í» y «,» no hay límite de palabra. Cuarta
vez en dos días — «no sé», «mamá», «quizá» y ahora «sí». Se cierra con la misma
anticipación negativa y tiene su caso en el golden.
...
Ninguno de esos PRs es basura: son defectos distintos y reales del mismo módulo.
El problema es que para cuando una rama se fusiona, su número ya está tomado.
Renumerar al final es un parche; la decisión —repartir la secuencia, serializar
el loop, o derivar el número al fusionar— es del dueño y queda como `O-5`.

### Notas de la corrida (contenedor sin credenciales)

- `ops-timeout-y-punto-ciego.test.ts` cuenta con que `10.255.255.1` se trague la
  conexión; el proxy de este contenedor la rechaza en el acto. Falla igual sobre
  el árbol limpio y **pasa en los runners de CI**. Anotado como `OPS-002`.
- `npm run build` sin las `NEXT_PUBLIC_FIREBASE_*` muere recogiendo
  `/dr/[clinicId]`. Con valores de relleno compila entero.
